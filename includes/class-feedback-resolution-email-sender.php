<?php
/**
 * Sends a branded notification when user feedback is resolved.
 *
...
class ResolutionEmailSender {

	public const META_SENT_AT = '_feedback_resolution_email_sent_at';

	/**
	 * Notify the feedback author once that their feedback has been resolved.
	 *
	...
	public function send( int $feedback_id ): array {
		...
		$resolution_summary = trim( (string) get_post_meta( $feedback_id, StatusService::META_RESOLUTION_SUMMARY, true ) );
		if ( $resolution_summary === '' ) {
			return [ 'status' => 'missing_resolution_summary' ];
		}
		...
		$body_html       = sprintf(
			'<p style="margin:0 0 16px;color:#0f172a;font-size:16px;line-height:1.7;">%1$s</p><p style="margin:0;color:#0f172a;font-size:16px;line-height:1.7;">Bedankt voor het delen van je feedback. We hebben deze opgelost:</p>%2$s%3$s<p style="margin:0;color:#0f172a;font-size:16px;line-height:1.7;">De oplossing staat nu in Rondo. Via de knop hieronder kun je de feedback nog eens bekijken.</p>',
			esc_html( $greeting ),
			$title_html,
			$resolution_html
		);
		$html            = EmailTemplate::render(
			[
				'preheader'   => $subject,
				'eyebrow'     => 'Feedback',
				'heading'     => 'Je feedback is opgelost',
				'body_html'   => $body_html,
				'cta_url'     => $feedback_url,
				'cta_label'   => 'Bekijk je feedback',
				'footer_html' => '<p style="margin:0;color:#64748b;font-size:13px;line-height:1.6;">Deze e-mail is automatisch verstuurd nadat je feedback als opgelost is gemarkeerd.</p>',
			]
		);
		...
		do_action( 'rondo_feedback_resolution_email_sent', $feedback_id, $email );
