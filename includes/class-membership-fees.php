...
class MembershipFees {
	 ...
	 * reference back to this MembershipFees for fee calculation and
	 * settings lookups until phases 216-217 extract those too.
	 *
	 ...
	 * do not have to know about the wiring. Phase 216/217 will swap the
	 * internal MembershipFees collaborator for FeeCalculator /
	...
	public function family_grouping(): FamilyGroupingService {
		if ( $this->family_grouping === null ) {
			$this->family_grouping = new FamilyGroupingService( $this );
		}
	...
	public function calculate_fee_with_family_discount( int $person_id, ?string $season = null ): ?array {
		...
		$family_grouping = $this->family_grouping();
		$family_key      = $family_grouping->get_family_key( $person_id );
		...
		$groups         = $family_grouping->build_family_groups( $season );
