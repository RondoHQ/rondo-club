...
class PRM_Monica_Import {
    ...
    public function register_routes() {
        register_rest_route('prm/v1', '/import/monica', [
            'methods'             => WP_REST_Server::CREATABLE,
            'callback'            => [$this, 'handle_import'],
            'permission_callback' => [$this, 'check_import_permission'],
        ]);

        register_rest_route('prm/v1', '/import/monica/validate', [
            'methods'             => WP_REST_Server::CREATABLE,
    ...
    public function validate_import($request) {
        ...
        if (strpos($sql_content, 'INSERT IGNORE INTO `contacts`') === false) {
            return new WP_Error('invalid_format', __('Invalid Monica SQL export format. File must contain contacts table.', 'personal-crm'), ['status' => 400]);
        }
    ...
    public function handle_import($request) {
        ...
        if (empty($monica_url)) {
            return new WP_Error('missing_url', __('Monica instance URL is required for photo import.', 'personal-crm'), ['status' => 400]);
        }
    ...
    private function import_contact_tags(int $post_id, string $monica_id): void {
        $contact_tags = array_filter($this->tables['contact_tag'] ?? [], function($ct) use ($monica_id) {
    ...
    private function import_contact_fields(int $post_id, string $monica_id): void {
        $fields = array_filter($this->tables['contact_fields'] ?? [], function($f) use ($monica_id) {
    ...
    private function import_addresses(int $post_id, string $monica_id): void {
        $addresses = array_filter($this->tables['addresses'] ?? [], function($a) use ($monica_id) {
    ...
    private function import_notes(int $post_id, string $monica_id): void {
        $notes = array_filter($this->tables['notes'] ?? [], function($n) use ($monica_id) {
    ...
    private function import_special_dates(int $post_id, string $monica_id, string $first_name, string $last_name, array $contact): void {
        ...
        $dates = array_filter($this->tables['special_dates'] ?? [], function($d) use ($monica_id) {
    ...
    private function import_contact_photo(int $post_id, string $monica_id, string $first_name, string $last_name, array $contact): void {
        ...
        if ($avatar_source === 'photo') {
            $contact_photos = array_filter($this->tables['contact_photo'] ?? [], function($cp) use ($monica_id) {
