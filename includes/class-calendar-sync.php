...
class Sync {
	 ...
	 * Cron action name
	 */
	const CRON_HOOK = 'rondo_calendar_sync';
	 ...
	 */
	const USER_INDEX_TRANSIENT = 'rondo_calendar_sync_last_user_index';
	...
	private function process_event_for_auto_log( $event ) {
		...
		if ( $activities_created > 0 ) {
			...
			error_log(
				sprintf(
					'RONDO_Calendar_Sync: Auto-logged event %d as %d activities',
					$event_id,
					$activities_created
				)
			);
