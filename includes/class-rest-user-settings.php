...
class UserSettings extends Base {
	...
	private function get_available_columns_metadata(): array {
		...
		if ( \Rondo\Volunteer\PeopleShiftProgress::can_view() ) {
			foreach ( [
				'status'    => 'Inschrijftaken',
				'planned'   => 'Ingepland',
				'completed' => 'Afgerond',
				'required'  => 'Vereist',
			] as $key => $label ) {
	...
	public function get_current_user_data( $user_id ) {
		...
		return [
			...
			'dashboard_context'              => \Rondo\Dashboard\RoleDashboard::context( $user_id ),
			'can_access_club_dashboard'      => $is_kader && ( \Rondo\Core\UserRoles::can_access_section( 'teams', $user_id ) || ! \Rondo\Core\AccessControl::has_coordinator_team_scope( $user_id ) ),
			'can_access_teams'               => \Rondo\Core\AccessControl::can_access_teams( $user_id ),
			...
			'can_view_people_shift_progress' => \Rondo\Volunteer\PeopleShiftProgress::can_view(),
