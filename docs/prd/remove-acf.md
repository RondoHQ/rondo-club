...
| Loading | Normal plugin + `class_exists('ACF')` guard (`functions.php:182`). No Composer/npm dependency. Test suite and CI also require ACF Pro (§3.5). |
...
- One less paid plugin dependency in prod, demo, dev, CI, and every test environment — and CI's PHP suite stops being license-gated (§3.5).
...
- Test suite (389 tests) requires ACF Pro in the WP install, and CI downloads ACF Pro per run using a license-key secret — full details and migration steps in §3.5.
...
**Existing field-level test coverage is thin and uneven — the golden masters are the only net for roughly half the fields.** Cross-referencing all 182 static field names against `tests/` (a *generous* proxy: a name appearing in a test file ≠ its behavior being asserted), 99/182 (~54%) appear at all:

| Coverage | Groups (referenced fields / total) |
|---|---|
| Full / near-full | `dienst_shift` 9/9, `todo` 4/4, `taakuitleg` 1/1, `dienst_type` 14/16, `shift_template` 8/9 |
| Partial | `person` 44/80, `volunteer_policy` 6/12, `invoice` 6/14, `feedback` 3/10 |
| Essentially untested | `discipline_case` 1/13, `commissie` 1/12, `team` 2/8, `team_kickoff` 0/2, `relationship_type` 0/1 |
| Zero | dynamic custom-fields subsystem |

Targeted backfill before the corresponding slice (not blanket coverage — golden masters carry the rest):

1. **`addresses` sub-fields** (`postal_code`, `house_number`, …): zero test references, yet they feed the family-discount raw SQL (risk #5, silent-empty failure mode). Add a household/family-discount regression test before the person slice.
2. **`inverse_relationship_type`** (term meta): zero references despite driving the #1-risk inverse-sync engine — pin the inverse-type resolution path in the RelationshipService suite.
3. **Invoice money fields** (`total_amount`, `line_items.amount`, `payment_link`, `sent_date`) and **discipline cases**: nearly test-free; backfill alongside their golden masters before slices 6–7.
4. The untested `team`/`commissie`/`discipline_case`/`invoice` groups line up with the migration order's middle slices — budget the backfill into those slices' estimates (§5 already assumes this).

### 3.2 Step 1: the shim (read- and write-side)

`Rondo\Fields\Fields::get($name, $post_id)` / `::update($name, $value, $post_id)` / `::delete(...)` — post-type-aware (resolves the `contact_info` ambiguity), initially delegating to `get_field()`/`update_field()`. Codemod all 685 sites file-by-file (mechanical PRs, zero behavior change; the 67 test call sites and `RondoTestCase` helpers are part of the same sweep). The ~55 dynamic-name sites get hand-checked. This is the cheap intermediate step: after it lands, "migrating a field group" = flipping the shim's backend for that group.
...
### 3.5 Test suite and CI migration

The 389-test wpunit suite is ACF-coupled in four distinct ways; each has its own step:

1. **Call sites (67, ~20 files):** migrated by the same shim codemod as production code (§3.2), so tests keep passing through every per-group cutover. The `RondoTestCase::createPerson()/createTeam()/…` helpers that take an `$acf` array of field values switch to the shim with them.
2. **Bootstrap:** `RondoTestCase::setUp()` explicitly boots ACF local JSON (`acf_get_local_json_files()`), and the suite loads `advanced-custom-fields-pro/acf.php` in its WP install (docs/testing.md §2). Both go away in the removal PR; the native field registry needs no boot step beyond theme load.
3. **ACF-specific workarounds to delete with the plugin:** the `ignoreIncorrectUsage('rest_handle_multi_type_schema')` silencer (ACF Pro's WP 7.0 REST-schema notice) and the "person writes must round-trip the full `acf` object because ACF marks fields required" testing convention — the native REST provider can accept partial writes, and tests get simpler.
4. **CI (`.github/workflows/ci.yml:74-119`):** the PHP test job downloads ACF Pro at run time using the `ACF_PRO_LICENSE_KEY` repository secret and **skips the entire suite with only a warning when the secret is absent**. During the migration this stays (tests need ACF until the last group flips). In the removal PR: delete the download/gate steps and the secret, making the PHP suite *unconditional* in CI — a reliability win in its own right, since today a lapsed license or missing secret silently produces green builds with zero PHP tests run.

Interim guard: while groups cut over one by one, the golden-master suite (§3.1) and per-group wpunit tests run in an environment that still has ACF installed — that's correct and intended, since production does too. No parallel "ACF-less" CI matrix is needed before the removal PR; the removal PR itself proves the suite passes without the plugin.

Also update docs/testing.md (drop §2 "ACF Pro", the license-secret instructions, and the ACF-related pitfalls) as part of the removal PR.

### 3.6 Final removal

Only after step 9 of §3.4 is verified in production:

- Delete the `class_exists('ACF')` dependency check (`functions.php:178-202`) — ACF is no longer *required*; add a temporary inverse guard if desired (warn if ACF is still active, to avoid double-serving).
- Delete the `acf-json/` load/save points and the `acf-json/` directory (schema now lives in `includes/fields/`).
- Test/CI decoupling per §3.5.
- Docs: docs/testing.md, CLAUDE.md/AGENTS.md (the "Rule 0" storage table, "Adding ACF fields" workflow, and both ACF pitfall sections), developer docs site.
- Deactivate and remove the ACF Pro plugin on prod, demo, and test installs; cancel the license if unused elsewhere.
- Optional `_field`-reference-row sweep (dry-run + DB backup first).
- rondo-sync needs **no change** under D1 — verify its pipelines against demo before and after anyway.
...
| Shim + 685-site codemod (incl. tests) | 1.5–2 |
...
| ACF removal + cleanup + docs + test/CI decoupling | 1 |
...
- Tests: 67 ACF call sites across ~20 wpunit files + the bootstrap/CI coupling detailed in §3.5.

## Appendix B — Provenance

The analysis behind §§1–2 (hook-by-hook handlers, repeater read/write site lists, REST endpoint tables, customfields internals) was produced against commit `7e6d61b6` (2026-07-31). File:line references throughout this doc are from that commit.
