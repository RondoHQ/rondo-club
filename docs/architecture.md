...
add_action('rondo_relationship_created', function($from_person_id, $to_person_id, $relationship_type_id) {
...
```php
add_filter('rondo_inverse_relationship_type', function($inverse_type_id, $relationship_type_id, $from_person_id, $to_person_id) {
...
```php
add_filter('rondo_resolve_gender_dependent_type', function($resolved_type_id, $group, $gender, $related_person_id) {
...
- Relationship creation respects `Rondo\Core\AccessControl` permissions
