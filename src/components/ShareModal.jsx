      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-xl">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <div>
              <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-50">Delen</h2>
          ...
          <div className="px-6 py-4">
            ...
            <div className="relative mb-4">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-50 focus:ring-2 focus:ring-electric-cyan focus:border-electric-cyan"
              />
            </div>

            {/* Permission selector */}
            <div className="flex items-center gap-2 mb-4">
              <span className="text-sm text-gray-600 dark:text-gray-400">Rechten:</span>
              <select
                value={selectedPermission}
                onChange={(e) => setSelectedPermission(e.target.value)}
                className="text-sm border border-gray-300 dark:border-gray-600 rounded-md px-2 py-1 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-50 focus:ring-electric-cyan focus:border-electric-cyan"
              >
                <option value="view">Kan bekijken</option>
                <option value="edit">Kan bewerken</option>
              </select>
            </div>
            ...
            {searchQuery.length >= 2 && (
              <div className="mb-4">
                {searchLoading ? (
                  <div className="py-3 text-center text-sm text-gray-500 dark:text-gray-400">Zoeken...</div>
                ) : availableUsers.length === 0 ? (
                  <div className="py-3 text-center text-sm text-gray-500 dark:text-gray-400">
                    {searchResults.length === 0 ? 'Geen gebruikers gevonden' : 'Alle overeenkomende gebruikers hebben al toegang'}
                  </div>
                ) : (
                    ...
                    {availableUsers.map((user) => (
                      <div key={user.id} className="flex items-center gap-3 p-3 hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <button
                          ...
                          className="p-2 text-electric-cyan dark:text-electric-cyan hover:bg-cyan-50 dark:hover:bg-obsidian/30 rounded-full"
                        >
            ...
            <div>
              <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Gedeeld met ({shares.length})
              </h3>
              {sharesLoading ? (
                <div className="py-3 text-center text-sm text-gray-500 dark:text-gray-400">Laden...</div>
              ) : shares.length === 0 ? (
                <div className="py-6 text-center text-sm text-gray-500 dark:text-gray-400">
                  Nog niet met iemand gedeeld
                </div>
              ) : (
                <div className="border border-gray-200 dark:border-gray-700 rounded-lg divide-y divide-gray-100 dark:divide-gray-700">
                  {shares.map((share) => (
                    <div key={share.user_id} className="flex items-center gap-3 p-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                          {share.permission === 'view' ? 'Kan bekijken' : 'Kan bewerken'}
                        </div>
                      </div>
                      <button
