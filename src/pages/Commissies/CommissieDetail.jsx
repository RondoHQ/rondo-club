...
export default function CommissieDetail() {
  ...
  const investorDetails = useMemo(() => {
    ...
    return investorIds.map(investorId => {
      ...
      return {
        id: post.id,
        type: isPerson ? 'person' : 'commissie',
        name: isPerson
    ...
    try {
      const payload = {
        ...
        acf: {
          ...
          _visibility: data.visibility || commissie.acf?._visibility || 'private',
          _assigned_workspaces: data.assigned_workspaces || commissie.acf?._assigned_workspaces || [],
        },
      };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Invested in (commissies this organization has invested in) */}
        ...
        <div className="card p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {investments.map((commissie) => (
              <Link
                key={commissie.id}
                to={`/commissies/${commissie.id}`}
                className="flex items-center p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 border border-gray-200 dark:border-gray-700"
              >
                {commissie.thumbnail ? (
                  <img
                    src={commissie.thumbnail}
                    alt={commissie.name}
                    loading="lazy"
                ...
                <div className="ml-3">
                  <p className="text-sm font-medium">{commissie.name}</p>
                  {commissie.industry && (
                    <p className="text-xs text-gray-500 dark:text-gray-400">{commissie.industry}</p>
                  )}
                </div>
              </Link>
            ))}
