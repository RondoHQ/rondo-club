...
function EmailEditor({ person }) {
  ...
  return (
    <div className="card max-w-3xl p-5">
      ...
      <div className="mt-3 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-200">
      ...
      {pending ? (
        <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-200">
      ...
      <dl className="mt-4 grid gap-4 sm:grid-cols-2">
        <div>
          <dt className="text-xs text-gray-500">Primair</dt>
          <dd className="mt-1 break-words text-sm text-gray-900 dark:text-gray-100">{fields.email_1 || 'Niet ingesteld'}</dd>
          {!editing ? <button type="button" className="mt-2 text-sm font-medium text-bright-cobalt" onClick={() => open('primary', fields.email_1)}>Wijzigen</button> : null}
        </div>
        <div>
          <dt className="text-xs text-gray-500">Tweede e-mailadres</dt>
          ...
          {!editing ? (
            <div className="mt-2 flex flex-wrap gap-x-4 gap-y-2 text-sm font-medium">
              <button type="button" className="text-bright-cobalt" onClick={() => open('secondary', fields.email_2)}>{fields.email_2 ? 'Wijzigen' : 'Toevoegen'}</button>
              {fields.email_2 ? <button type="button" className="text-bright-cobalt" onClick={promote}>Primair maken</button> : null}
              {fields.email_2 ? <button type="button" className="inline-flex items-center gap-1 text-red-600" onClick={remove}><Trash2 className="h-3.5 w-3.5" /> Verwijderen</button> : null}
            </div>
          ) : null}
        </div>
      </dl>
    ...
    <div className="card max-w-3xl p-5">
      ...
      {editing ? (
        <form className="mt-4" onSubmit={submit}>
          ...
          <p className="mt-3 text-xs text-gray-500">Telefoon 2 blijft alleen in Rondo; Sportlink ondersteunt dit veld niet.</p>
          <div className="mt-4 flex gap-2">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {PHONE_FIELDS.map((field) => <div key={field}><div className="text-xs text-gray-500">{labels[field]}</div><div className="text-sm text-gray-900 dark:text-gray-100">{fields[field] || 'Niet ingesteld'}</div></div>)}
        </div>
      )}
      {error ? <p className="mt-3 text-sm text-red-600 dark:text-red-400">{error}</p> : null}
    </div>
...
export default function MemberProfileEditors({ people, linkedPersonId }) {
  const self = people.find((person) => person.id === linkedPersonId);
  if (!self) return null;
  return (
    <div className="space-y-4">
      <EmailEditor person={self} />
      <PhoneEditor person={self} />
      <AddressEditor people={people} />
    </div>
  );
}
