  ...
  const handleSaveSettings = async (e) => {
    ...
    try {
      await updateSettings.mutateAsync({
        cooldown_seasons: parseInt(cooldown, 10),
        current_season: currentSeason,
      });
    } catch {
      alert('Instellingen konden niet worden opgeslagen.');
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <form onSubmit={handleCreateAssignment} className="card p-6 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input className="input" placeholder="Seizoen" value={assignmentForm.season} onChange={(e) => setAssignmentForm((v) => ({ ...v, season: e.target.value }))} />
      ...
      <form onSubmit={handleSaveSettings} className="card p-6 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <input
            ...
            placeholder="Cooldown (seizoenen)"
          />
          <input className="input" value={currentSeason} onChange={(e) => setCurrentSeason(e.target.value)} placeholder="Huidig seizoen (YYYY-YYYY)" />
          <button type="submit" className="btn-secondary text-sm" disabled={updateSettings.isPending}>Instellingen opslaan</button>
        </div>
      </form>
