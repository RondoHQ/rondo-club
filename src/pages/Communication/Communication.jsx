...
import { CalendarClock, Check, ChevronDown, ChevronUp, Copy, ExternalLink, FileText, ImagePlus, MessageCircle, Pause, Play, Plus, Repeat2, RotateCcw, Search, Send, X } from 'lucide-react';
...
const channels = { whatsapp: 'WhatsApp', newsletter: 'Nieuwsbrief', website: 'Website' };
const recurrenceLabels = { none: 'Geen', monthly: 'Maandelijks', yearly: 'Jaarlijks' };
const openStatuses = ['concept', 'preparing', 'ready'];

const blank = {
  title: '', description: '', channel: '', google_docs_url: '', audience: '', planned_date: '', assignee_id: '',
...
function ItemForm({ item, users, onClose }) {
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 p-3 sm:p-8" role="dialog" aria-modal="true" aria-labelledby="communication-dialog-title">
      <div className="mx-auto max-w-3xl rounded-xl bg-white shadow-xl dark:bg-gray-800">
        ...
        <form onSubmit={save} className="space-y-5 p-5">
          ...
          <fieldset>
            <legend className="mb-2 text-sm font-medium dark:text-gray-200">Kanaal *</legend>
            <div className="flex flex-wrap gap-4">
              {Object.entries(channels).map(([value, label]) => <label key={value} className="flex items-center gap-2 text-sm dark:text-gray-200"><input type="radio" name="channel" value={value} checked={form.channel === value} onChange={(e) => set('channel', e.target.value)} required /> {label}</label>)}
            </div>
          </fieldset>
          <div>
          ...
          {form.status === 'sent' && <div><label className="mb-1 block text-sm font-medium dark:text-gray-200" htmlFor="comm-published">Link naar gepubliceerd bericht</label><input id="comm-published" type="url" className="input w-full" value={form.published_url} onChange={(e) => set('published_url', e.target.value)} /></div>}
          <div className="grid gap-4 sm:grid-cols-2">
          ...
          {editing && form.series_id && openStatuses.includes(form.status) && <label className="flex items-start gap-2 rounded-lg bg-blue-50 p-3 text-sm text-blue-800 dark:bg-blue-900/20 dark:text-blue-200"><input type="checkbox" className="mt-1" checked={form.apply_to_future} onChange={(e) => set('apply_to_future', e.target.checked)} />Pas titel, inhoud, kanaal, doelgroep en verantwoordelijke ook toe op toekomstige openstaande keren.</label>}
          <div className="flex flex-wrap justify-between gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
            <div className="flex flex-wrap gap-2">
              ...
              {editing && ['sent', 'skipped', 'cancelled'].includes(form.status) && <button type="button" className="btn-tertiary" onClick={() => runAction(form.status === 'sent' ? 'reopen' : 'restore')}><RotateCcw className="mr-2 h-4 w-4" />Ongedaan maken</button>}
            </div>
...
export default function Communication() {
  ...
  const filtered = useMemo(() => items.filter((item) => {
    ...
    return tabMatch && (!search || `${item.title} ${item.description}`.toLowerCase().includes(search.toLowerCase())) && (!channel || item.channel === channel) && (!assignee || String(item.assignee_id) === assignee) && (!status || item.status === status) && (!dateFrom || relevantDate >= dateFrom) && (!dateTo || relevantDate <= dateTo) && (!overdueOnly || isOverdue);
  ...
  async function complete(item) {
    const actualDate = window.prompt(item.channel === 'website' ? 'Publicatiedatum (jjjj-mm-dd)' : 'Verzenddatum (jjjj-mm-dd)', today);
    if (!actualDate) return;
    try {
      const publishedUrl = window.prompt('Link naar het gepubliceerde bericht (optioneel)', item.published_url || '');
      if (publishedUrl === null) return;
      await action.mutateAsync({ id: item.id, data: { action: 'complete', actual_date: actualDate, published_url: publishedUrl } });
    } catch (err) {
  ...
  return <div className="space-y-5 p-4 sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-2xl font-bold text-gray-900 dark:text-white">Communicatie</h2><p className="mt-1 text-sm text-gray-500">Plan wat via WhatsApp, nieuwsbrief en website gedeeld moet worden.</p></div><button className="btn-primary" onClick={() => setSelected({})}><Plus className="mr-2 h-4 w-4" />Nieuw item</button></div>
    ...
    <div className="grid gap-3 rounded-lg bg-gray-50 p-3 sm:grid-cols-4 dark:bg-gray-800"><label className="relative sm:col-span-1"><Search className="absolute left-3 top-2.5 h-4 w-4 text-gray-400" /><input className="input w-full pl-9" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Zoeken" aria-label="Zoeken" /></label><select className="input" value={channel} onChange={(e) => setChannel(e.target.value)} aria-label="Kanaal"><option value="">Alle kanalen</option>{Object.entries(channels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><select className="input" value={assignee} onChange={(e) => setAssignee(e.target.value)} aria-label="Verantwoordelijke"><option value="">Iedereen</option>{users.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select>{tab === 'open' ? <select className="input" value={status} onChange={(e) => setStatus(e.target.value)} aria-label="Status"><option value="">Alle statussen</option>{openStatuses.map((value) => <option key={value} value={value}>{statuses[value]}</option>)}</select> : <div />}<label className="text-xs text-gray-500">Vanaf<input type="date" className="input mt-1 w-full" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} /></label><label className="text-xs text-gray-500">Tot en met<input type="date" className="input mt-1 w-full" value={dateTo} onChange={(e) => setDateTo(e.target.value)} /></label></div>
    ...
    <div className="space-y-3">
      {filtered.map((item) => {
        ...
        return <article key={item.id} className={`card flex flex-col gap-3 p-4 sm:flex-row sm:items-center ${overdue ? 'border-l-4 border-l-red-500' : ''}`}>
          <button className="min-w-0 flex-1 text-left" onClick={() => setSelected(item)}><div className="mb-2 flex flex-wrap items-center gap-2"><h3 className="font-semibold text-gray-900 dark:text-white">{item.title}</h3><Badge tone={item.status === 'ready' ? 'green' : item.status === 'preparing' ? 'blue' : item.status === 'sent' ? 'green' : 'gray'}>{statuses[item.status]}</Badge><Badge>{channels[item.channel]}</Badge>{item.series_id && <Badge tone="blue"><Repeat2 className="mr-1 h-3 w-3" />{recurrenceLabels[item.recurrence]}</Badge>}{overdue && <Badge tone="red">Te laat</Badge>}</div><div className="flex flex-wrap gap-x-5 gap-y-1 text-sm text-gray-500"><span className="flex items-center gap-1"><CalendarClock className="h-4 w-4" />{niceDate(tab === 'sent' ? item.actual_date : item.planned_date)}</span><span>{item.assignee_name || 'Niet toegewezen'}</span>{item.audience && <span>{item.audience}</span>}</div></button>
          <div className="flex shrink-0 items-center gap-2">{item.google_docs_url && <a className="btn-tertiary" href={item.google_docs_url} target="_blank" rel="noreferrer" title="Open Google-document"><FileText className="h-4 w-4" /></a>}{openStatuses.includes(item.status) && <button className="btn-secondary" onClick={() => complete(item)} disabled={action.isPending}><Send className="mr-2 h-4 w-4" />{item.channel === 'website' ? 'Publiceren' : 'Versturen'}</button>}{item.status === 'sent' && item.published_url && <a className="btn-tertiary" href={item.published_url} target="_blank" rel="noreferrer"><ExternalLink className="h-4 w-4" /></a>}<button className="btn-tertiary" onClick={() => setSelected(item)}><Check className="h-4 w-4" /><span className="sr-only">Openen</span></button></div>
        </article>;
      })}
      {filtered.length === 0 && <div className="card p-10 text-center text-gray-500">Geen communicatie-items gevonden.</div>}
    </div>
    {selected && <ItemForm item={selected.id ? selected : null} users={users} onClose={(duplicate) => { setSelected(null); if (duplicate?.id) setSelected(duplicate); }} />}
  </div>;
}
