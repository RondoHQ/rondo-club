...
function SettingsForm({ initial, teams }) {
  ...
  return (
    <form onSubmit={save} className="space-y-6 text-gray-900 dark:text-gray-100">
      <div className="flex items-center justify-between gap-4"><h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Trainingsinstellingen</h2><Link className="btn-secondary" to="/trainingsschema">Naar trainingsschema</Link></div>
      <fieldset disabled={mutation.isPending} className="space-y-6 disabled:opacity-60">
        <section className="card p-5 space-y-4">
          <h3 className="font-semibold text-gray-900 dark:text-gray-100">Velden</h3><p className="text-sm text-gray-500 dark:text-gray-400">Elk veld heeft vier delen: A, B, C en D. Je kunt achtste, kwart, halve, driekwart en hele velden plannen. Achtste velden heten A1, A2 tot en met D2.</p>
        ...
        <section className="card p-5 space-y-4">
          <h3 className="font-semibold text-gray-900 dark:text-gray-100">Standaarden per leeftijdslaag</h3><p className="text-sm text-gray-500 dark:text-gray-400">Duur en veldgrootte worden ingevuld bij nieuwe blokken. Kleuren gelden direct voor alle schema’s.</p>
          {settings.age_groups.map((group) => <div key={group.id} className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><label className="text-sm">Leeftijdslaag<input required maxLength={100} className="input mt-1" placeholder="Bijv. O13" value={group.name} onChange={(event) => changeRow('age_groups', group.id, { name: event.target.value })} /></label><label className="text-sm">Duur (minuten)<input required className="input mt-1" type="number" min={15} max={360} step={15} value={group.duration} onChange={(event) => changeRow('age_groups', group.id, { duration: Number(event.target.value) })} /></label><label className="text-sm">Veldgrootte<SizeSelect value={group.size} onChange={(size) => changeRow('age_groups', group.id, { size })} label={`Veldgrootte ${group.name}`} /></label><label className="text-sm">Kleur<input type="color" aria-label={`Kleur ${group.name}`} className="input mt-1 h-10 cursor-pointer p-1" value={group.color || '#cffafe'} onChange={(event) => changeRow('age_groups', group.id, { color: event.target.value })} /></label><button type="button" className="btn-tertiary self-end" onClick={() => change('age_groups', settings.age_groups.filter((item) => item.id !== group.id))}>Leeftijdslaag verwijderen</button></div>)}
          <button type="button" className="btn-secondary" onClick={() => change('age_groups', [...settings.age_groups, { id: crypto.randomUUID(), name: '', duration: 60, size: 1, color: '#cffafe' }])}>Leeftijdslaag toevoegen</button>
        </section>
        <section className="card p-5 space-y-4">
          <h3 className="font-semibold text-gray-900 dark:text-gray-100">Instellingen per team</h3><p className="text-sm text-gray-500 dark:text-gray-400">Koppel de leeftijdslaag en vul alleen afwijkende tijden of veldgroottes in.</p>
