...
must_haves:
  truths:
    - "Logged-in user can list their own feedback via GET /stadion/v1/feedback"
    - "Logged-in user can create feedback via POST /stadion/v1/feedback"
    - "Logged-in user can read their own feedback via GET /stadion/v1/feedback/{id}"
    - "Logged-in user can update their own feedback via PATCH /stadion/v1/feedback/{id}"
    - "Logged-in user can delete their own feedback via DELETE /stadion/v1/feedback/{id}"
...
<objective>
Create REST API endpoints for feedback CRUD operations under stadion/v1/feedback namespace.
...
4. Register all routes under `stadion/v1/feedback`:
...
- Add require_once in stadion_init() alongside other REST classes
...
grep -n "register_rest_route.*stadion/v1.*feedback" includes/class-rest-feedback.php
...
curl -s -o /dev/null -w "%{http_code}" "https://stadion.joost.blog/wp-json/stadion/v1/feedback"
...
1. **Route registration:** All 5 routes registered under stadion/v1/feedback
...
<success_criteria>
- [ ] GET /stadion/v1/feedback returns user's own feedback (admin sees all)
- [ ] POST /stadion/v1/feedback creates feedback with all fields
- [ ] GET /stadion/v1/feedback/{id} returns single feedback (own or admin)
- [ ] PATCH /stadion/v1/feedback/{id} updates allowed fields (status/priority admin-only)
- [ ] DELETE /stadion/v1/feedback/{id} removes feedback (own or admin)
- [ ] Unauthenticated requests return 401
