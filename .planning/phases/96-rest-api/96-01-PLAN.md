...
must_haves:
  truths:
    - "Logged-in user can list their own feedback via GET /prm/v1/feedback"
    - "Logged-in user can create feedback via POST /prm/v1/feedback"
    - "Logged-in user can read their own feedback via GET /prm/v1/feedback/{id}"
    - "Logged-in user can update their own feedback via PATCH /prm/v1/feedback/{id}"
    - "Logged-in user can delete their own feedback via DELETE /prm/v1/feedback/{id}"
  ...
  artifacts:
    - path: "includes/class-rest-feedback.php"
      ...
      exports: ["Caelis\\REST\\Feedback"]
    - path: "functions.php"
      provides: "Feedback REST class instantiation"
      contains: "new Caelis\\REST\\Feedback"
  key_links:
...
<objective>
Create REST API endpoints for feedback CRUD operations under prm/v1/feedback namespace.
...
- Add require_once in prm_init() alongside other REST classes
...
grep -n "register_rest_route.*prm/v1.*feedback" includes/class-rest-feedback.php
...
1. Get post by ID
2. Validate it's caelis_feedback type
...
curl -s -o /dev/null -w "%{http_code}" "https://caelis.joost.blog/wp-json/prm/v1/feedback"
...
2. Validate post exists and is caelis_feedback type
...
1. **Route registration:** All 5 routes registered under prm/v1/feedback
...
<success_criteria>
- [ ] GET /prm/v1/feedback returns user's own feedback (admin sees all)
- [ ] POST /prm/v1/feedback creates feedback with all fields
- [ ] GET /prm/v1/feedback/{id} returns single feedback (own or admin)
- [ ] PATCH /prm/v1/feedback/{id} updates allowed fields (status/priority admin-only)
- [ ] DELETE /prm/v1/feedback/{id} removes feedback (own or admin)
- [ ] Unauthenticated requests return 401
