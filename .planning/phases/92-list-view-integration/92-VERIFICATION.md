...
**Phase Goal:** Custom fields can appear as columns in People and Organizations list views
**Verified:** 2026-01-20T22:15:00Z
**Status:** passed
...
| 2 | Custom field columns appear in Organizations list view when enabled | VERIFIED | CompaniesList.jsx lines 428-442: identical pattern with `company` post type, renders via `listViewFields.map()` at lines 395-399 and 469-475 |
...
| 5 | Admin can configure column order for list view fields | VERIFIED | FieldFormPanel.jsx lines 1274-1291: conditional `list_view_order` number input (min=1, max=999); sorting implemented in list views at lines 161 (People) and 441 (Organizations) |
...
| `src/pages/Companies/CompaniesList.jsx` | Custom field column integration | VERIFIED | Lines 10 (import), 428-442 (metadata query + listViewFields memo), 469-475 (headers), 395-399 (cells via CustomFieldColumn) |

### Key Link Verification

| From | To | Via | Status | Details |
|------|-----|-----|--------|---------|
| PeopleList.jsx | /prm/v1/custom-fields/person/metadata | useQuery with prmApi.getCustomFieldsMetadata('person') | WIRED | Line 152 calls API, response stored in customFields |
| CompaniesList.jsx | /prm/v1/custom-fields/company/metadata | useQuery with prmApi.getCustomFieldsMetadata('company') | WIRED | Line 432 calls API, response stored in customFields |
| CustomFieldColumn.jsx | person.acf/company.acf | Reading field.name from ACF object | WIRED | PeopleList line 117: `person.acf?.[field.name]`, CompaniesList line 397: `company.acf?.[field.name]` |
...
| API client | REST endpoint | getCustomFieldsMetadata method | WIRED | client.js line 273: `getCustomFieldsMetadata: (postType) => api.get('/prm/v1/custom-fields/${postType}/metadata')` |
...
| DISP-06: Custom field columns appear in Organizations list when enabled | SATISFIED | Verified via CompaniesList.jsx implementation |
...
**Test:** Repeat test 1 for Organizations.
**Expected:** Custom field column appears in Organizations list.
...
4. **List View Integration** - Both PeopleList.jsx and CompaniesList.jsx fetch metadata, filter by `show_in_list_view`, sort by `list_view_order`, render headers and cells using CustomFieldColumn
