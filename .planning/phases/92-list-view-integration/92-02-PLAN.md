...
must_haves:
  truths:
    ...
    - "Custom field columns appear in Teams list when enabled"
  ...
  key_links:
    - from: "src/pages/People/PeopleList.jsx"
      to: "/rondo/v1/custom-fields/person/metadata"
...
<objective>
Integrate custom fields as columns in People and Teams list views.
...
<task type="auto">
  <name>Task 3: Integrate custom field columns into PeopleList and TeamsList</name>
...
Apply the same pattern but with 'team' post type:
1. Same imports
2. Query with `['custom-fields-metadata', 'team']` and `prmApi.getCustomFieldsMetadata('team')`
3. Add headers after Workspace column in TeamListView
4. Pass listViewFields to TeamListRow
5. Add cells in TeamListRow after workspace (around line 393)

The pattern is identical, just swap 'person' for 'team' and adjust the component/prop names accordingly.
  </action>
  <verify>
Build succeeds: `npm run build`
...
4. Repeat for Teams
  ...
  <done>Custom field columns appear in both People and Teams list views when enabled by admin.</done>
</task>
...
5. Teams list shows custom field columns for enabled fields
...
- DISP-06: Custom field columns appear in Teams list when enabled
