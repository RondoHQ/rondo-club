...
```bash
curl -s "https://[PRODUCTION_URL]/wp-json/prm/v1/custom-fields/person/metadata" -H "Cookie: [auth]" | grep -E "(show_in_list_view|list_view_order)"
```
  </verify>
...
- Admin can update these properties via PUT /prm/v1/custom-fields/{post_type}/{key}
