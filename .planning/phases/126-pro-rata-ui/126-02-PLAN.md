...
Place the route registration near other data retrieval routes. Place the callback method with other list methods.
  ...
  <verify>
Deploy and test: `curl -s "https://stadion.svawc.nl/wp-json/stadion/v1/fees" -H "Cookie: <auth>"` returns JSON with members array.
  </verify>
  <done>REST endpoint /stadion/v1/fees returns fee list sorted by category and name.</done>
</task>
...
<verification>
1. REST endpoint /stadion/v1/fees returns correct data structure
