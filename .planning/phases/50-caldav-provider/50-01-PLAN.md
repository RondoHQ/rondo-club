...
Output: PRM_CalDAV_Provider class with calendar discovery, event sync, and iCalendar parsing.
</objective>
...
<task type="auto">
  <name>Task 1: Create PRM_CalDAV_Provider class</name>
  <files>includes/class-caldav-provider.php, functions.php</files>
  <action>
Create PRM_CalDAV_Provider class that mirrors the pattern from PRM_Google_Calendar_Provider.
...
Add class to autoloader in functions.php prm_init().
  </action>
  <verify>
  ...
  <done>PRM_CalDAV_Provider class created with test_connection, discover_calendars, and sync methods</done>
</task>
...
<task type="auto">
  ...
  <action>
Update the existing stub methods in PRM_REST_Calendar to call PRM_CalDAV_Provider:

1. **test_caldav()** - Replace 501 stub with working implementation:
   - Get url, username, password from request params
   - Call PRM_CalDAV_Provider::test_connection()
...
2. **trigger_sync()** - Add CalDAV support to existing method:
   ...
   - If so, call PRM_CalDAV_Provider::sync() instead of Google provider
...
- [ ] PRM_CalDAV_Provider class exists and loads without errors
