...
Output: STADION_CalDAV_Provider class with calendar discovery, event sync, and iCalendar parsing.
</objective>
...
<task type="auto">
  <name>Task 1: Create STADION_CalDAV_Provider class</name>
  <files>includes/class-caldav-provider.php, functions.php</files>
  <action>
Create STADION_CalDAV_Provider class that mirrors the pattern from STADION_Google_Calendar_Provider.
...
Add class to autoloader in functions.php stadion_init().
  </action>
  <verify>
  ...
  <done>STADION_CalDAV_Provider class created with test_connection, discover_calendars, and sync methods</done>
</task>
...
<task type="auto">
  ...
  <action>
Update the existing stub methods in STADION_REST_Calendar to call STADION_CalDAV_Provider:

1. **test_caldav()** - Replace 501 stub with working implementation:
   - Get url, username, password from request params
   - Call STADION_CalDAV_Provider::test_connection()
...
2. **trigger_sync()** - Add CalDAV support to existing method:
   ...
   - If so, call STADION_CalDAV_Provider::sync() instead of Google provider
...
- [ ] STADION_CalDAV_Provider class exists and loads without errors
