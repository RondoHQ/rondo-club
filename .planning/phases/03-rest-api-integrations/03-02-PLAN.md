...
<objective>
Extract import/export endpoints from PRM_REST_API into a dedicated PRM_REST_Import_Export class.
...
@.planning/phases/02-rest-api-people-companies/02-02-SUMMARY.md
...
**Tech stack available:** PRM_REST_Base abstract class with shared permission and formatting methods
**Established patterns:** Domain-specific REST classes extend PRM_REST_Base, register routes via rest_api_init hook
**Constraining decisions:**
- Phase 01-01: Base class is abstract, permission methods are public, formatting methods are protected
- Phase 02-02: Domain-specific classes instantiated after prior classes in prm_init() to maintain route registration order
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create PRM_REST_Import_Export class with export and CardDAV endpoints</name>
  <files>includes/class-rest-import-export.php</files>
  <action>
Create new file `includes/class-rest-import-export.php`:
1. Class `PRM_REST_Import_Export extends PRM_REST_Base`
2. Constructor registers routes via `rest_api_init` hook (following established pattern)
3. Move the following routes from PRM_REST_API:
   - `/export/vcard` (export_vcard)
   - `/export/google-csv` (export_google_csv)
   - `/carddav/urls` (get_carddav_urls)
4. Move ALL corresponding callback methods from PRM_REST_API:
   - export_vcard()
   - export_google_csv()
   - get_carddav_urls()
...
7. Ensure class uses `$this->` for any inherited methods from PRM_REST_Base
...
<task type="auto">
  <name>Task 2: Remove export/CardDAV code from PRM_REST_API and update autoloader</name>
...
2. In `functions.php`:
   - Add `require_once get_template_directory() . '/includes/class-rest-import-export.php';` after class-rest-slack.php (or class-rest-companies.php if Plan 01 not yet executed) in the require block
   - Add `new PRM_REST_Import_Export();` after the Slack class (or Companies class if Plan 01 not yet executed) in prm_init() function

Note: If Plan 03-01 has already been executed, add after class-rest-slack.php. If running in parallel, add after class-rest-companies.php.
  </action>
  <verify>grep -c "export_vcard\|export_google_csv\|carddav" includes/class-rest-api.php returns 0, grep -c "PRM_REST_Import_Export" functions.php returns 2</verify>
  <done>Export/CardDAV code removed from PRM_REST_API, autoloader updated with new class</done>
</task>
...
- [ ] `functions.php` requires and instantiates PRM_REST_Import_Export
...
- No export/CardDAV code remains in PRM_REST_API
- PRM_REST_Import_Export properly extends PRM_REST_Base
