...
<objective>
Extract import/export endpoints from STADION_REST_API into a dedicated STADION_REST_Import_Export class.
...
- Phase 02-02: Domain-specific classes instantiated after prior classes in stadion_init() to maintain route registration order
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create STADION_REST_Import_Export class with export and CardDAV endpoints</name>
  <files>includes/class-rest-import-export.php</files>
  <action>
Create new file `includes/class-rest-import-export.php`:
1. Class `STADION_REST_Import_Export extends STADION_REST_Base`
2. Constructor registers routes via `rest_api_init` hook (following established pattern)
3. Move the following routes from STADION_REST_API:
...
7. Ensure class uses `$this->` for any inherited methods from STADION_REST_Base
...
<task type="auto">
  <name>Task 2: Remove export/CardDAV code from STADION_REST_API and update autoloader</name>
...
2. In `functions.php`:
   ...
   - Add `new STADION_REST_Import_Export();` after the Slack class (or Companies class if Plan 01 not yet executed) in stadion_init() function
...
Note: If Plan 03-01 has already been executed, add after class-rest-slack.php. If running in parallel, add after class-rest-companies.php.
  ...
  <verify>grep -c "export_vcard\|export_google_csv\|carddav" includes/class-rest-api.php returns 0, grep -c "STADION_REST_Import_Export" functions.php returns 2</verify>
  <done>Export/CardDAV code removed from STADION_REST_API, autoloader updated with new class</done>
</task>
...
- [ ] `functions.php` requires and instantiates STADION_REST_Import_Export
...
- No export/CardDAV code remains in STADION_REST_API
- STADION_REST_Import_Export properly extends STADION_REST_Base
