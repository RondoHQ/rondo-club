...
<objective>
Extract Slack integration endpoints from RONDO_REST_API into a dedicated RONDO_REST_Slack class.
...
- Phase 02-01: Domain-specific classes instantiated after RONDO_REST_API in rondo_init() to maintain route registration order
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create RONDO_REST_Slack class with all Slack endpoints</name>
  <files>includes/class-rest-slack.php</files>
  <action>
Create new file `includes/class-rest-slack.php`:
1. Class `RONDO_REST_Slack extends RONDO_REST_Base`
2. Constructor registers routes via `rest_api_init` hook (following established pattern)
3. Move ALL Slack-related routes from RONDO_REST_API:
...
6. Ensure class uses `$this->` for any inherited methods from RONDO_REST_Base
...
<task type="auto">
  <name>Task 2: Remove Slack code from RONDO_REST_API and update autoloader</name>
...
2. In `functions.php`:
   - Add `require_once get_template_directory() . '/includes/class-rest-slack.php';` after class-rest-teams.php in the require block
   - Add `new RONDO_REST_Slack();` after `new RONDO_REST_Teams();` in rondo_init() function
  </action>
  <verify>grep -c "slack" includes/class-rest-api.php returns 0, grep -c "RONDO_REST_Slack" functions.php returns 2</verify>
  <done>Slack code removed from RONDO_REST_API, autoloader updated with new class</done>
</task>
...
- [ ] `functions.php` requires and instantiates RONDO_REST_Slack
...
- No Slack code remains in RONDO_REST_API
- RONDO_REST_Slack properly extends RONDO_REST_Base
