...
<objective>
Extract Slack integration endpoints from PRM_REST_API into a dedicated PRM_REST_Slack class.
...
- Phase 02-01: Domain-specific classes instantiated after PRM_REST_API in prm_init() to maintain route registration order
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create PRM_REST_Slack class with all Slack endpoints</name>
  <files>includes/class-rest-slack.php</files>
  <action>
Create new file `includes/class-rest-slack.php`:
1. Class `PRM_REST_Slack extends PRM_REST_Base`
2. Constructor registers routes via `rest_api_init` hook (following established pattern)
3. Move ALL Slack-related routes from PRM_REST_API:
...
6. Ensure class uses `$this->` for any inherited methods from PRM_REST_Base
...
<task type="auto">
  <name>Task 2: Remove Slack code from PRM_REST_API and update autoloader</name>
...
2. In `functions.php`:
   ...
   - Add `new PRM_REST_Slack();` after `new PRM_REST_Companies();` in prm_init() function
  </action>
  <verify>grep -c "slack" includes/class-rest-api.php returns 0, grep -c "PRM_REST_Slack" functions.php returns 2</verify>
  <done>Slack code removed from PRM_REST_API, autoloader updated with new class</done>
</task>
...
- [ ] `functions.php` requires and instantiates PRM_REST_Slack
...
- No Slack code remains in PRM_REST_API
- PRM_REST_Slack properly extends PRM_REST_Base
