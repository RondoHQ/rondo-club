...
<objective>
Extend RONDO_Access_Control to check visibility, workspace membership, and direct shares.
...
@.planning/PROJECT.md
@.planning/ROADMAP.md
@Stadion-Multi-User-Project-Plan.md (section 1.6)
...
public function get_accessible_post_ids($post_type, $user_id) {
    ...
    $workspace_ids = RONDO_Workspace_Members::get_user_workspace_ids($user_id);
    $workspace_posts = [];
...
public function user_can_access_post($post_id, $user_id = null) {
    ...
    if (!user_can($user_id, 'manage_options')) {
        if (!RONDO_User_Roles::is_user_approved($user_id)) {
            return false;
        }
    }
    ...
    // 2. Check visibility
    $visibility = RONDO_Visibility::get_visibility($post_id);

    // Private = only author (already checked above)
    if ($visibility === RONDO_Visibility::PRIVATE) {
        return false;
    }

    // 3. Workspace visibility check
    if ($visibility === RONDO_Visibility::WORKSPACE) {
        // Get user's workspace IDs
        $user_workspace_ids = RONDO_Workspace_Members::get_user_workspace_ids($user_id);
    ...
    if (RONDO_Visibility::user_has_share($post_id, $user_id)) {
        return true;
    }
...
public function get_user_permission($post_id, $user_id = null) {
    ...
    if ($visibility === RONDO_Visibility::WORKSPACE) {
        ...
        if (!is_wp_error($post_terms)) {
            foreach ($post_terms as $slug) {
                $workspace_id = (int) str_replace('workspace-', '', $slug);
                if (in_array($workspace_id, $user_workspace_ids)) {
                    $role = RONDO_Workspace_Members::get_user_role($workspace_id, $user_id);
                    if ($role) {
    ...
    $share_permission = RONDO_Visibility::get_share_permission($post_id, $user_id);
    if ($share_permission) {
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    2. wp eval "var_dump(RONDO_Visibility::set_visibility(123, 'workspace'));" (use real post ID)
    3. wp eval "var_dump(RONDO_Workspace_Members::add(WORKSPACE_ID, USER_ID, 'member'));"
    4. wp eval "var_dump((new RONDO_Access_Control())->user_can_access_post(123, USER_ID));"
  </how-to-verify>
