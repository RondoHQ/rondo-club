...
<objective>
Extend STADION_Access_Control to check visibility, workspace membership, and direct shares.
...
public function user_can_access_post($post_id, $user_id = null) {
    ...
    if ($visibility === STADION_Visibility::PRIVATE) {
        return false;
    }

    // 3. Workspace visibility check
    if ($visibility === STADION_Visibility::WORKSPACE) {
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    2. wp eval "var_dump(STADION_Visibility::set_visibility(123, 'workspace'));" (use real post ID)
    3. wp eval "var_dump(STADION_Workspace_Members::add(WORKSPACE_ID, USER_ID, 'member'));"
    4. wp eval "var_dump((new STADION_Access_Control())->user_can_access_post(123, USER_ID));"
  </how-to-verify>
