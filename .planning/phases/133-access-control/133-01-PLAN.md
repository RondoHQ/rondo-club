...
must_haves:
  truths:
    ...
    - "Users without fairplay capability do not see Tuchtzaken tab on person detail"
  artifacts:
    ...
    - path: "src/pages/People/PersonDetail.jsx"
      provides: "Conditional Tuchtzaken tab"
      contains: "can_access_fairplay"
  key_links:
...
Purpose: Restrict sensitive disciplinary information to authorized users only. Users without fairplay capability cannot access discipline cases via the UI (both route and navigation hidden, person detail tab hidden). Follows existing pattern for admin-only features.

Output: Backend capability registration, REST API capability exposure, React route protection, conditional navigation rendering, and conditional person detail tab rendering.
</objective>
...
<task type="auto">
  <name>Task 3: Add conditional Tuchtzaken tab on person detail page</name>
  <files>
    src/pages/People/PersonDetail.jsx
  </files>
  <action>
    **In src/pages/People/PersonDetail.jsx:**

    1. Add import for prmApi if not already imported (it is already imported on line 26):
       ```jsx
       import { wpApi, prmApi } from '@/api/client';
       ```

    2. Add import for Gavel icon from lucide-react (add to existing imports around line 3-7):
       ```jsx
       import { ..., Gavel } from 'lucide-react';
       ```

    3. Add a query for current user inside the PersonDetail component (after the existing useQuery hooks, around line 60-67):
       ```jsx
       // Fetch current user for capability check
       const { data: currentUser } = useQuery({
         queryKey: ['current-user'],
    ...
    4. Add a Tuchtzaken TabButton in the tab navigation section (around line 1264-1270), conditionally rendered based on canAccessFairplay:
       ```jsx
       {/* Tab Navigation */}
    ...
    5. Add tab content for Tuchtzaken (placeholder for Phase 134) after the Work Tab content section (around line 1775):
       ```jsx
  ...
  <verify>
    ...
    2. After deployment, test:
       - Admin user: should see Tuchtzaken tab on person detail page
       - Non-fairplay user: should NOT see Tuchtzaken tab on person detail page
       - Clicking Tuchtzaken tab shows placeholder content
  </verify>
  <done>
    - currentUser query added to PersonDetail component
    - canAccessFairplay boolean derived from user capability
    - Tuchtzaken TabButton conditionally rendered based on capability
    - Placeholder tab content added for Phase 134
    - Users without fairplay capability do not see Tuchtzaken tab
  </done>
</task>
...
2. **REST API:**
   ...
   - `/rondo/v1/user/me` returns `can_access_fairplay: false` for users without capability
...
5. **Person Detail Tab:**
   - Tuchtzaken tab visible on person detail for users with fairplay
   - Tuchtzaken tab hidden on person detail for users without fairplay
...
- ACCESS-04: Person Tuchtzaken tab restricted to fairplay users (navigation and person detail tab)
</success_criteria>
