...
1. Add PersonListView function component above PeopleList:
   ```jsx
   function PersonListView({ people, teamMap, workspaces }) {
     return (
...
2. Add PersonListRow component:
   ```jsx
   function PersonListRow({ person, teamName, workspaces }) {
...
Avoid: Duplicating team lookup logic - reuse existing personTeamMap. Do not modify filtering/sorting logic.
  </action>
  ...
  <done>List view renders as table. Name column shows avatar+name+favorite star. Org column shows team name. Workspace column shows assigned workspaces.</done>
</task>
...
- [ ] List view shows Name, Team, Workspace columns
