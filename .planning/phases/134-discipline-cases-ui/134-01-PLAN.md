...
public function get_current_season() {
    // Use the helper from RONDO_Taxonomies
    $current_season = RONDO_Taxonomies::get_current_season();
...
Note: The get_current_season() static method was added in Phase 132 to RONDO_Taxonomies class.
  </action>
  <verify>Test endpoint: `curl -s "https://[site]/wp-json/rondo/v1/current-season" -H "Cookie: [auth]"` returns current season or null</verify>
...
4. Backend /rondo/v1/current-season endpoint returns season data
</verification>
