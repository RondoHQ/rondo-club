...
2. Add new public static method `get_contacts_client(bool $include_granted_scopes = true, bool $readonly = true): ?\Google\Client`:
   ...
   - Set redirect URI to `rest_url('rondo/v1/google-contacts/callback')`
...
2. Routes registered: `wp rest list --path=/rondo/v1/google-contacts` shows status, auth, callback
3. Status endpoint works: `curl -s "https://cael.is/wp-json/rondo/v1/google-contacts/status" -H "X-WP-Nonce: ..."` returns JSON
  </verify>
  <done>
...
2. **Integration verification:**
   ...
   - REST routes are registered (`wp rest list --path=/rondo/v1/google-contacts`)
