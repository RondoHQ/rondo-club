...
<task type="auto">
  ...
  <action>
Extend the existing GoogleOAuth class in `Caelis\Calendar` namespace to support contacts scope:
...
2. Add new public static method `get_contacts_client(bool $include_granted_scopes = true, bool $readonly = true): ?\Google\Client`:
   ...
   - Set redirect URI to `rest_url('prm/v1/google-contacts/callback')`
...
2. Verify class loads: `wp eval "var_dump(class_exists('Caelis\Calendar\GoogleOAuth'));"`
3. Verify constants exist: `wp eval "var_dump(Caelis\Calendar\GoogleOAuth::CONTACTS_SCOPE_READONLY);"`
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
Create new class `GoogleContactsConnection` in `Caelis\Contacts` namespace for managing Google Contacts connection state in user meta. Follow the pattern from `Caelis\Calendar\Connections` but adapted for single-connection-per-user:
...
<task type="auto">
  ...
  <action>
Create new REST controller class `Caelis\REST\GoogleContacts` extending Base (follow pattern from class-rest-calendar.php):
...
- Add the use statement at top: `use Caelis\REST\GoogleContacts as RESTGoogleContacts;`
...
2. Routes registered: `wp rest list --path=/prm/v1/google-contacts` shows status, auth, callback
3. Status endpoint works: `curl -s "https://cael.is/wp-json/prm/v1/google-contacts/status" -H "X-WP-Nonce: ..."` returns JSON
  </verify>
  <done>
...
2. **Integration verification:**
   ...
   - REST routes are registered (`wp rest list --path=/prm/v1/google-contacts`)
