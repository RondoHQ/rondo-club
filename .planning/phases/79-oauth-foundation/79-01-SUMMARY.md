...
key-decisions:
  - "Use separate callback endpoint /prm/v1/google-contacts/callback to distinguish from calendar OAuth"
...
## Decisions Made
- **Separate callback endpoint:** Using `/prm/v1/google-contacts/callback` instead of shared callback to allow different post-auth behavior (contacts redirects to subtab=contacts, sets pending_import flag)
...
- **Pending import flag:** OAuth callback sets `_prm_google_contacts_pending_import` user meta for Phase 80 to detect and auto-start import
...
- REST endpoints deployed and verified on production:
  - `GET /prm/v1/google-contacts/status`
  - `GET /prm/v1/google-contacts/auth`
  - `GET /prm/v1/google-contacts/callback`
  - `DELETE /prm/v1/google-contacts`
