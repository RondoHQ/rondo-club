...
1. In class-google-calendar-provider.php, add a static method `list_calendars(array $connection): array` that:
   - Gets access token via `\PRM_Google_OAuth::get_access_token($connection)`
...
2. In class-rest-calendar.php, add endpoint `GET /prm/v1/calendar/connections/{id}/calendars`:
   ...
   - Routes to appropriate provider:
     - For Google: `\PRM_Google_Calendar_Provider::list_calendars($connection)`
     - For CalDAV: `\PRM_CalDAV_Provider::discover_calendars($url, $username, $password)` (decrypt credentials first)
...
- Verify the new method signature matches what the endpoint expects
  ...
  <done>
- GET /prm/v1/calendar/connections/{id}/calendars returns list of available calendars
...
3. Add API method in src/api/client.js (prmApi object):
   - `getConnectionCalendars: (connectionId) => apiClient.get(\`/prm/v1/calendar/connections/\${connectionId}/calendars\`)`
