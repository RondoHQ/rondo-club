...
Purpose: Verify /rondo/v1/ custom endpoints return correct data and enforce access control.
...
<task type="auto">
  ...
  <action>
Create SearchDashboardTest extending StadionTestCase. Test /rondo/v1/search endpoint:

1. Test basic search:
   - Create approved user with several persons ("John Doe", "Jane Smith")
   - GET /rondo/v1/search?q=John
...
1. Test dashboard summary (/rondo/v1/dashboard):
...
3. Test reminders endpoint (/rondo/v1/reminders):
   - Create important_date with date in next 7 days
   - GET /rondo/v1/reminders?days_ahead=30
   - Assert upcoming date included
   - Create date 60 days out
   - Assert NOT included with days_ahead=30

4. Test reminders validation:
   - GET /rondo/v1/reminders?days_ahead=0 (invalid)
   - Assert 400 error
   - GET /rondo/v1/reminders?days_ahead=500 (too large)
   - Assert 400 error

5. Test todos endpoint (/rondo/v1/todos):
