...
Purpose: Verify /prm/v1/ custom endpoints return correct data and enforce access control.
...
<task type="auto">
  ...
  <action>
Create SearchDashboardTest extending CaelisTestCase. Test /prm/v1/search endpoint:

1. Test basic search:
   - Create approved user with several persons ("John Doe", "Jane Smith")
   - GET /prm/v1/search?q=John
...
1. Test dashboard summary (/prm/v1/dashboard):
   - Create user with 3 persons, 2 companies, 5 important dates
   - GET /prm/v1/dashboard
   - Assert response includes counts (people_count, companies_count, dates_count)
...
3. Test reminders endpoint (/prm/v1/reminders):
   - Create important_date with date in next 7 days
   - GET /prm/v1/reminders?days_ahead=30
   - Assert upcoming date included
   - Create date 60 days out
   - Assert NOT included with days_ahead=30

4. Test reminders validation:
   - GET /prm/v1/reminders?days_ahead=0 (invalid)
   - Assert 400 error
   - GET /prm/v1/reminders?days_ahead=500 (too large)
   - Assert 400 error

5. Test todos endpoint (/prm/v1/todos):
