...
Purpose: Verify person-company relationships, sharing functionality, and bulk operations work correctly via REST API.
...
<task type="auto">
  ...
  <action>
Create RelationshipsSharesTest extending CaelisTestCase. Test relationship endpoints:

1. Test person-company relationship:
   - Create company "Acme Corp"
   - Create person with organization_id = company ID (via ACF)
   - GET /prm/v1/companies/{id}/people
   - Assert person appears in company's people list

2. Test person-dates relationship:
   - Create person
   - Create important_date linked to person (via ACF person field)
   - GET /prm/v1/people/{id}/dates
   - Assert date appears in person's dates list

3. Test relationship data expansion:
   - Create person with work_history (array of company references)
   - GET /wp/v2/people/{id}
   - Assert work_history expanded with company details (not just IDs)
...
1. Test sharing endpoints for people (/prm/v1/people/{id}/shares):
...
3. Test sharing for companies:
   - Same tests as people at /prm/v1/companies/{id}/shares

4. Test bulk update for people (/prm/v1/people/bulk-update):
   - Create 3 persons
...
6. Test bulk update for companies:
   - Similar tests at /prm/v1/companies/bulk-update
  </action>
...
- [ ] Person-company relationship endpoint works
- [ ] Person-dates relationship endpoint works
- [ ] Sharing add/remove works for people and companies
