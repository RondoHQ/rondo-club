...
Display the person's current position (job title and organization) prominently in the header section of PersonDetail, providing immediate context about who the person is professionally.
...
- `company` (post object ID)
- `job_title` (text)
- `is_current` (boolean)
- `start_date`, `end_date` (dates)

Company data is already fetched via `companyQueries` and available in `companyMap` (lines 1053-1065).
...
- Company name as a clickable link to the organization page
  ...
  <p className="text-base text-gray-600">
    {currentPositions.map((job, idx) => (
      <span key={idx}>
        {idx > 0 && ', '}
        {job.job_title}
        {job.company && companyMap[job.company] && (
          <>
        ...
        {job.job_title && !job.company && job.company_name_text && (
          // Fallback for jobs with company name but no linked company
          <>
            <span className="text-gray-400"> at </span>
            <span>{job.company_name_text}</span>
          </>
        )}
      </span>
    ))}
...
1. **Job title only** (no company linked): Show just the title
2. **Company only** (no job title): Show "Works at [Company]"
...
3. Manual test cases:
   - Person with one current job (title + company)
   - Person with multiple current jobs
   - Person with job title only (no company linked)
...
- Uses existing `companyMap` populated by `companyQueries` — no additional API calls needed
