...
1. Register GET endpoint: `/stadion/v1/user/theme-preferences`
   - Returns: { color_scheme: 'light'|'dark'|'system', accent_color: 'orange'|... }
   - Default values: color_scheme = 'system', accent_color = 'orange'
   - Read from user meta: 'stadion_color_scheme', 'stadion_accent_color'

2. Register PATCH endpoint: `/stadion/v1/user/theme-preferences`
...
- getThemePreferences: () => apiClient.get('/stadion/v1/user/theme-preferences')
- updateThemePreferences: (prefs) => apiClient.patch('/stadion/v1/user/theme-preferences', prefs)
  </action>
  <verify>Test endpoints with curl or in browser console:
- GET /wp-json/stadion/v1/user/theme-preferences returns default values
