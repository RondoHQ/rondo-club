...
<objective>
Extract people-related REST API endpoints and methods into a dedicated STADION_REST_People class.
...
<task type="auto">
  <name>Task 1: Create STADION_REST_People class with routes and methods</name>
  <files>includes/class-rest-people.php</files>
  <action>
Create new file `includes/class-rest-people.php` with class STADION_REST_People extending STADION_REST_Base.
...
Permission methods are public in base class and can be used as permission_callback via $this.
  ...
  <done>class-rest-people.php exists with STADION_REST_People class containing all people-specific endpoints and filters</done>
</task>

<task type="auto">
  <name>Task 2: Remove people methods from STADION_REST_API</name>
...
1. Add autoloader mapping for STADION_REST_People:
   'STADION_REST_People' => 'includes/class-rest-people.php'
   (Add near the STADION_REST_Base and STADION_REST_API mappings)

2. Instantiate the new class in stadion_init() function:
   new STADION_REST_People();
   (Add after the STADION_REST_API instantiation)
...
The order matters - classes should be instantiated so routes don't conflict.
  ...
  <done>Autoloader maps STADION_REST_People, class is instantiated in stadion_init()</done>
</task>
...
- STADION_REST_People class properly extends STADION_REST_Base
