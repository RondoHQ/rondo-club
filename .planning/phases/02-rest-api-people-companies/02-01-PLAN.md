...
<objective>
Extract people-related REST API endpoints and methods into a dedicated RONDO_REST_People class.
...
<task type="auto">
  <name>Task 1: Create RONDO_REST_People class with routes and methods</name>
  <files>includes/class-rest-people.php</files>
  <action>
Create new file `includes/class-rest-people.php` with class RONDO_REST_People extending RONDO_REST_Base.
...
Permission methods are public in base class and can be used as permission_callback via $this.
  ...
  <done>class-rest-people.php exists with RONDO_REST_People class containing all people-specific endpoints and filters</done>
</task>

<task type="auto">
  <name>Task 2: Remove people methods from RONDO_REST_API</name>
...
Do NOT remove methods that are used by other domains (get_people_by_team stays - it's team-related despite the name).
  </action>
...
1. Add autoloader mapping for RONDO_REST_People:
   'RONDO_REST_People' => 'includes/class-rest-people.php'
   (Add near the RONDO_REST_Base and RONDO_REST_API mappings)

2. Instantiate the new class in rondo_init() function:
   new RONDO_REST_People();
   (Add after the RONDO_REST_API instantiation)
...
The order matters - classes should be instantiated so routes don't conflict.
  ...
  <done>Autoloader maps RONDO_REST_People, class is instantiated in rondo_init()</done>
</task>
...
- RONDO_REST_People class properly extends RONDO_REST_Base
- People endpoints extracted to dedicated class
- No duplicate code between files
</success_criteria>

<output>
After completion, create `.planning/phases/02-rest-api-people-teams/02-01-SUMMARY.md`
</output>
