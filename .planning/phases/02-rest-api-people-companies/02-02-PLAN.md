...
<objective>
Extract team-related REST API endpoints and methods into a dedicated RONDO_REST_Teams class.
...
<task type="auto">
  <name>Task 1: Create RONDO_REST_Teams class with routes and methods</name>
  <files>includes/class-rest-teams.php</files>
  <action>
Create new file `includes/class-rest-teams.php` with class RONDO_REST_Teams extending RONDO_REST_Base.
...
- get_people_by_team (lines ~481-568) - note: this checks access internally via RONDO_Access_Control
...
Permission methods are public in base class and can be used as permission_callback via $this.
  ...
  <done>class-rest-teams.php exists with RONDO_REST_Teams class containing all team-specific endpoints</done>
</task>

<task type="auto">
  <name>Task 2: Remove team methods from RONDO_REST_API and update autoloader</name>
...
1. Add autoloader mapping for RONDO_REST_Teams:
   'RONDO_REST_Teams' => 'includes/class-rest-teams.php'
   (Add near the RONDO_REST_People mapping)

2. Instantiate the new class in rondo_init() function:
   new RONDO_REST_Teams();
   (Add after the RONDO_REST_People instantiation)
  </action>
...
- RONDO_REST_Teams class properly extends RONDO_REST_Base
