...
type: execute
depends_on: ["09-01"]
files_modified: [src/components/ShareModal.jsx, src/hooks/useSharing.js, src/api/client.js, src/pages/People/PersonDetail.jsx, src/pages/Teams/TeamDetail.jsx, includes/class-rest-people.php, includes/class-rest-teams.php]
---

<objective>
Create ShareModal component for sharing contacts/teams with specific users.

Purpose: Enable users to share individual contacts or teams directly with other users (separate from workspace sharing).
Output: Working ShareModal with user search, share/unshare functionality, integrated into PersonDetail and TeamDetail pages.
</objective>
...
<task type="auto">
  ...
  <files>includes/class-rest-people.php, includes/class-rest-teams.php</files>
  <action>
...
```php
// Sharing endpoints
register_rest_route('stadion/v1', '/people/(?P<id>\d+)/shares', [
    [
        'methods' => 'GET',
...
register_rest_route('stadion/v1', '/people/(?P<id>\d+)/shares/(?P<user_id>\d+)', [
    'methods' => 'DELETE',
...
Add the same endpoints to `class-rest-teams.php` with the route prefix changed to `/teams/`.
...
register_rest_route('stadion/v1', '/users/search', [
    'methods' => 'GET',
...
```
  ...
  <verify>Test endpoints via curl or browser - GET /stadion/v1/people/{id}/shares should return empty array for unshared post</verify>
  <done>Share REST endpoints added to People and Teams classes, user search endpoint added</done>
</task>
...
```javascript
// In prmApi object:
getPostShares: (postType, postId) => api.get(`/stadion/v1/${postType}/${postId}/shares`),
addShare: (postType, postId, userId, permission) => api.post(`/stadion/v1/${postType}/${postId}/shares`, { user_id: userId, permission }),
removeShare: (postType, postId, userId) => api.delete(`/stadion/v1/${postType}/${postId}/shares/${userId}`),
...
<task type="auto">
  <name>Task 4: Add Share button to PersonDetail and TeamDetail pages</name>
...
Apply the same pattern to TeamDetail.jsx:
- Import ShareModal and Share2 icon
- Add showShareModal state
- Add Share button in header
- Add ShareModal with postType="teams"
  </action>
  <verify>Navigate to PersonDetail and TeamDetail pages, verify Share button appears</verify>
  <done>Share button and ShareModal added to PersonDetail and TeamDetail pages</done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>ShareModal component with user search, share/unshare functionality, integrated into PersonDetail and TeamDetail pages</what-built>
  <how-to-verify>
    1. Run: `npm run dev`
...
- Share button visible on PersonDetail and TeamDetail
