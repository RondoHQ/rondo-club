...
type: execute
depends_on: ["09-01"]
files_modified: [src/pages/People/PeopleList.jsx, src/pages/Teams/TeamsList.jsx]
---

<objective>
Add visibility and workspace filtering to People and Teams list views.
...
<task type="auto">
  <name>Task 2: Add visibility/ownership filter to TeamsList</name>
  <files>src/pages/Teams/TeamsList.jsx</files>
  <action>
Apply the same filtering pattern to TeamsList.jsx:
...
5. Update the filtering logic in useMemo for filteredAndSortedTeams:
```javascript
// Apply ownership filter
if (ownershipFilter === 'mine') {
...
Note: TeamsList may have a different existing filter structure than PeopleList. Adapt the implementation to match the existing patterns in that file while adding the new ownership/workspace filters.
  </action>
  <verify>Open TeamsList, verify ownership and workspace filters work</verify>
  <done>TeamsList includes ownership and workspace filtering</done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Ownership and workspace filtering added to both PeopleList and TeamsList pages</what-built>
  <how-to-verify>
    1. Run: `npm run dev`
    2. Go to People page
    ...
    11. Repeat steps 2-10 for Teams page
...
- [ ] TeamsList has ownership filter
- [ ] TeamsList has workspace filter
...
- Ownership filter working on TeamsList
