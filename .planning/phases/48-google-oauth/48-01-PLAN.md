  ...
  - service: google-cloud
    ...
    dashboard_config:
      - task: "Create OAuth 2.0 Client ID"
        ...
        details: "Application type: Web application, Authorized redirect URI: https://[your-domain]/wp-json/rondo/v1/calendar/auth/google/callback"
...
<task type="auto">
  <name>Task 1: Install google/apiclient and create RONDO_Google_OAuth class</name>
...
- Token storage: Use existing RONDO_Credential_Encryption::encrypt() for tokens
...
- Redirect URI: Build dynamically from rest_url('rondo/v1/calendar/auth/google/callback')
  </action>
  <verify>
...
- RONDO_Google_OAuth class created with all methods
...
**google_auth_init:**
1. Check RONDO_Google_OAuth::is_configured() - return WP_Error if env vars missing
2. Get current user ID
3. Call RONDO_Google_OAuth::get_auth_url($user_id)
...
4. Exchange code for tokens via RONDO_Google_OAuth::handle_callback($code, $user_id)
5. On success: Create calendar connection via RONDO_Calendar_Connections::add_connection()
   - provider: 'google'
...
- With env vars NOT set: GET /rondo/v1/calendar/auth/google returns 400 with "not configured" message
- With env vars set: GET /rondo/v1/calendar/auth/google returns {auth_url: "https://accounts.google.com/..."}
  </verify>
  <done>
...
- Tokens stored encrypted via RONDO_Credential_Encryption
...
<task type="auto">
  ...
  <action>
Implement automatic token refresh in RONDO_Google_OAuth::get_access_token():

1. Decrypt credentials from connection using RONDO_Credential_Encryption::decrypt()
...
Add convenience method to RONDO_Calendar_Connections:
public static function update_credentials(int $user_id, string $connection_id, array $credentials): bool
- Encrypts credentials using RONDO_Credential_Encryption::encrypt()
...
- update_credentials helper added to RONDO_Calendar_Connections
  </done>
</task>
...
#    - Authorized redirect URI: https://your-domain.com/wp-json/rondo/v1/calendar/auth/google/callback
...
- [ ] RONDO_Google_OAuth class loads without fatal errors
- [ ] REST endpoint /rondo/v1/calendar/auth/google returns proper response
...
- RONDO_Google_OAuth class handles complete OAuth2 flow
- Token storage uses existing RONDO_Credential_Encryption
