  ...
  - service: google-cloud
    ...
    dashboard_config:
      - task: "Create OAuth 2.0 Client ID"
        ...
        details: "Application type: Web application, Authorized redirect URI: https://[your-domain]/wp-json/stadion/v1/calendar/auth/google/callback"
...
Purpose: Enable users to securely authorize Stadion to read their Google Calendar data using industry-standard OAuth2.
...
<task type="auto">
  <name>Task 1: Install google/apiclient and create STADION_Google_OAuth class</name>
...
- Token storage: Use existing STADION_Credential_Encryption::encrypt() for tokens
...
- Redirect URI: Build dynamically from rest_url('stadion/v1/calendar/auth/google/callback')
  </action>
  <verify>
...
- STADION_Google_OAuth class created with all methods
...
**google_auth_init:**
1. Check STADION_Google_OAuth::is_configured() - return WP_Error if env vars missing
2. Get current user ID
3. Call STADION_Google_OAuth::get_auth_url($user_id)
...
4. Exchange code for tokens via STADION_Google_OAuth::handle_callback($code, $user_id)
5. On success: Create calendar connection via STADION_Calendar_Connections::add_connection()
   - provider: 'google'
...
- With env vars NOT set: GET /stadion/v1/calendar/auth/google returns 400 with "not configured" message
- With env vars set: GET /stadion/v1/calendar/auth/google returns {auth_url: "https://accounts.google.com/..."}
  </verify>
  <done>
...
- Tokens stored encrypted via STADION_Credential_Encryption
...
<task type="auto">
  ...
  <action>
Implement automatic token refresh in STADION_Google_OAuth::get_access_token():

1. Decrypt credentials from connection using STADION_Credential_Encryption::decrypt()
...
Add convenience method to STADION_Calendar_Connections:
public static function update_credentials(int $user_id, string $connection_id, array $credentials): bool
- Encrypts credentials using STADION_Credential_Encryption::encrypt()
...
- update_credentials helper added to STADION_Calendar_Connections
  </done>
</task>
...
#    - Authorized redirect URI: https://your-domain.com/wp-json/stadion/v1/calendar/auth/google/callback
...
- [ ] STADION_Google_OAuth class loads without fatal errors
- [ ] REST endpoint /stadion/v1/calendar/auth/google returns proper response
...
- STADION_Google_OAuth class handles complete OAuth2 flow
- Token storage uses existing STADION_Credential_Encryption
