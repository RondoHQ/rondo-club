...
<task type="auto">
  ...
  <action>
Add `awaiting_response` query parameter to the `/rondo/v1/todos` endpoint in `get_all_todos()`:

1. In `register_routes()`, add `awaiting_response` arg to the `/rondo/v1/todos` endpoint args array:
...
Note: Keep existing sorting logic intact. Filter applies before sorting.
  ...
  <verify>
Test with curl:
- `curl -s "localhost/wp-json/rondo/v1/todos" | jq length` returns all open todos
- `curl -s "localhost/wp-json/rondo/v1/todos?awaiting_response=true" | jq length` returns only awaiting todos
- `curl -s "localhost/wp-json/rondo/v1/todos?awaiting_response=true&completed=true" | jq length` returns awaiting todos including completed
  </verify>
...
1. `test_todos_with_awaiting_response_filter_returns_only_awaiting()`:
   ...
   - Call GET /rondo/v1/todos?awaiting_response=true
...
2. `test_todos_awaiting_response_filter_with_completed()`:
   ...
   - Call GET /rondo/v1/todos?awaiting_response=true (no completed param)
   - Assert only open + awaiting is returned
   - Call GET /rondo/v1/todos?awaiting_response=true&completed=true
...
3. `test_todos_awaiting_response_filter_false_returns_non_awaiting()`:
   ...
   - Call GET /rondo/v1/todos?awaiting_response=false
