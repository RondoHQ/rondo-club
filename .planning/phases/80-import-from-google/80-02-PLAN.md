...
must_haves:
  ...
  artifacts:
    - path: "includes/class-rest-google-contacts.php"
      provides: "POST /stadion/v1/google-contacts/import endpoint"
...
Output: REST endpoint at `POST /stadion/v1/google-contacts/import` and updated API client
</objective>
...
4. **Update get_status() to include more import-relevant info:**
  ...
  <done>REST endpoint registered at POST /stadion/v1/google-contacts/import with proper error handling and stats return</done>
</task>
...
4. Quick endpoint test (requires connected Google account):
   ...
   curl -s "https://cael.is/wp-json/stadion/v1/google-contacts/status" | jq .
   ```
...
<success_criteria>
- POST /stadion/v1/google-contacts/import endpoint exists and returns import stats
