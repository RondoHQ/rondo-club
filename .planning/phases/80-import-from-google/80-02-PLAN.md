...
must_haves:
  ...
  artifacts:
    - path: "includes/class-rest-google-contacts.php"
      provides: "POST /prm/v1/google-contacts/import endpoint"
...
Output: REST endpoint at `POST /prm/v1/google-contacts/import` and updated API client
</objective>
   ...
   public function trigger_import( $request ) {
       ...
       try {
           ...
           return rest_ensure_response( [
               ...
               'stats'   => $stats,
               'message' => sprintf(
                   __( 'Import complete: %d imported, %d updated, %d skipped', 'caelis' ),
...
4. **Update get_status() to include more import-relevant info:**
  ...
  <done>REST endpoint registered at POST /prm/v1/google-contacts/import with proper error handling and stats return</done>
</task>
...
4. Quick endpoint test (requires connected Google account):
   ...
   curl -s "https://cael.is/wp-json/prm/v1/google-contacts/status" | jq .
   ```
...
<success_criteria>
- POST /prm/v1/google-contacts/import endpoint exists and returns import stats
