...
Output: calendar_event CPT registered, RONDO_Calendar_Connections helper class, RONDO_Credential_Encryption class.
</objective>
...
<task type="auto">
  ...
  <action>
Add `register_calendar_event_post_type()` method to RONDO_Post_Types class following existing pattern.
...
<task type="auto">
  <name>Task 2: Create RONDO_Calendar_Connections helper class</name>
  <files>includes/class-calendar-connections.php, functions.php</files>
  <action>
Create new class `RONDO_Calendar_Connections` with static methods for managing calendar connections stored in user meta.

User meta key: `_stadion_calendar_connections`
...
'RONDO_Calendar_Connections' => 'class-calendar-connections.php'
  </action>
  <verify>
...
- Verify delete_connection removes associated calendar_event posts
  ...
  <done>RONDO_Calendar_Connections class with CRUD methods for user meta connections</done>
</task>

<task type="auto">
  <name>Task 3: Create RONDO_Credential_Encryption class</name>
  <files>includes/class-credential-encryption.php, functions.php</files>
  <action>
Create new class `RONDO_Credential_Encryption` for securely storing OAuth tokens and CalDAV credentials.
...
```php
class RONDO_Credential_Encryption {
...
Add to autoloader in functions.php:
'RONDO_Credential_Encryption' => 'class-credential-encryption.php'
  </action>
  <verify>
- PHPUnit test: Encrypt test data, decrypt it, verify match
- Test that decrypt returns null for invalid/corrupted data
  </verify>
  <done>RONDO_Credential_Encryption class with encrypt/decrypt methods using sodium</done>
</task>
...
- [ ] RONDO_Calendar_Connections::add_connection() creates connection in user meta
- [ ] RONDO_Calendar_Connections::delete_connection() removes connection and events
- [ ] RONDO_Credential_Encryption::encrypt() and decrypt() round-trip correctly
...
- RONDO_Calendar_Connections helper class with full CRUD
- RONDO_Credential_Encryption with sodium-based encrypt/decrypt
