...
Output: calendar_event CPT registered, STADION_Calendar_Connections helper class, STADION_Credential_Encryption class.
</objective>
...
<task type="auto">
  ...
  <action>
Add `register_calendar_event_post_type()` method to STADION_Post_Types class following existing pattern.
...
<task type="auto">
  <name>Task 2: Create STADION_Calendar_Connections helper class</name>
  <files>includes/class-calendar-connections.php, functions.php</files>
  <action>
Create new class `STADION_Calendar_Connections` with static methods for managing calendar connections stored in user meta.
...
'STADION_Calendar_Connections' => 'class-calendar-connections.php'
  </action>
  <verify>
...
- Verify delete_connection removes associated calendar_event posts
  ...
  <done>STADION_Calendar_Connections class with CRUD methods for user meta connections</done>
</task>

<task type="auto">
  <name>Task 3: Create STADION_Credential_Encryption class</name>
  <files>includes/class-credential-encryption.php, functions.php</files>
  <action>
Create new class `STADION_Credential_Encryption` for securely storing OAuth tokens and CalDAV credentials.
...
Add to autoloader in functions.php:
'STADION_Credential_Encryption' => 'class-credential-encryption.php'
  </action>
  <verify>
- PHPUnit test: Encrypt test data, decrypt it, verify match
- Test that decrypt returns null for invalid/corrupted data
  </verify>
  <done>STADION_Credential_Encryption class with encrypt/decrypt methods using sodium</done>
</task>
...
- [ ] STADION_Calendar_Connections::add_connection() creates connection in user meta
- [ ] STADION_Calendar_Connections::delete_connection() removes connection and events
- [ ] STADION_Credential_Encryption::encrypt() and decrypt() round-trip correctly
...
- STADION_Calendar_Connections helper class with full CRUD
- STADION_Credential_Encryption with sodium-based encrypt/decrypt
