...
Output: PRM_REST_Calendar class with registered routes and stub handlers.
</objective>
...
@functions.php (REST initialization in prm_init)
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create PRM_REST_Calendar class with connection endpoints</name>
  <files>includes/class-rest-calendar.php</files>
  <action>
Create new class `PRM_REST_Calendar` extending `PRM_REST_Base`.

Namespace: `/prm/v1/calendar`
...
// GET /prm/v1/calendar/connections/(?P<id>[a-z0-9_]+) - Get single
...
// PUT /prm/v1/calendar/connections/(?P<id>[a-z0-9_]+) - Update
register_rest_route('prm/v1', '/calendar/connections/(?P<id>[a-z0-9_]+)', [
    'methods' => 'PUT',
...
// DELETE /prm/v1/calendar/connections/(?P<id>[a-z0-9_]+) - Delete
register_rest_route('prm/v1', '/calendar/connections/(?P<id>[a-z0-9_]+)', [
    'methods' => 'DELETE',
...
```
  ...
  <verify>
- `curl -s /wp-json/prm/v1/calendar/connections` returns 401 when logged out
- Routes appear in `curl -s /wp-json/prm/v1` route listing
  </verify>
...
<task type="auto">
  ...
  <files>includes/class-rest-calendar.php</files>
  <action>
Implement the connection CRUD methods using PRM_Calendar_Connections helper.
...
public function create_connection($request) {
    ...
    if (!empty($data['credentials'])) {
        $data['credentials'] = PRM_Credential_Encryption::encrypt($data['credentials']);
    }
...
Add initialization in prm_init() with other REST classes (around line 165):
```php
// Calendar REST endpoints
if (prm_is_rest_request()) {
    new PRM_REST_Calendar();
}
```
  </action>
  <verify>
- `curl -s /wp-json/prm/v1/people/1/meetings` returns empty structure (not error)
- `curl -s /wp-json/prm/v1/calendar/auth/google` returns 501 with message
  </verify>
...
- [ ] All routes appear in `/wp-json/prm/v1` discovery document
...
<success_criteria>
- PRM_REST_Calendar class with all routes registered
