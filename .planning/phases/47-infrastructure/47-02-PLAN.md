...
Output: STADION_REST_Calendar class with registered routes and stub handlers.
</objective>
...
@functions.php (REST initialization in stadion_init)
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create STADION_REST_Calendar class with connection endpoints</name>
  <files>includes/class-rest-calendar.php</files>
  <action>
Create new class `STADION_REST_Calendar` extending `STADION_REST_Base`.

Namespace: `/stadion/v1/calendar`
...
register_rest_route('stadion/v1', '/calendar/connections', [
    'methods' => 'GET',
...
// GET /stadion/v1/calendar/connections/(?P<id>[a-z0-9_]+) - Get single
register_rest_route('stadion/v1', '/calendar/connections/(?P<id>[a-z0-9_]+)', [
    'methods' => 'GET',
    'callback' => [$this, 'get_connection'],
    'permission_callback' => [$this, 'check_user_approved'],
]);

// PUT /stadion/v1/calendar/connections/(?P<id>[a-z0-9_]+) - Update
register_rest_route('stadion/v1', '/calendar/connections/(?P<id>[a-z0-9_]+)', [
    'methods' => 'PUT',
...
// DELETE /stadion/v1/calendar/connections/(?P<id>[a-z0-9_]+) - Delete
register_rest_route('stadion/v1', '/calendar/connections/(?P<id>[a-z0-9_]+)', [
    'methods' => 'DELETE',
...
register_rest_route('stadion/v1', '/calendar/connections/(?P<id>[a-z0-9_]+)/sync', [
    'methods' => 'POST',
...
register_rest_route('stadion/v1', '/calendar/auth/google', [
    'methods' => 'GET',
...
register_rest_route('stadion/v1', '/calendar/auth/google/callback', [
    'methods' => 'GET',
...
register_rest_route('stadion/v1', '/calendar/auth/caldav/test', [
    'methods' => 'POST',
...
register_rest_route('stadion/v1', '/calendar/events', [
    'methods' => 'GET',
...
register_rest_route('stadion/v1', '/people/(?P<person_id>\d+)/meetings', [
    'methods' => 'GET',
...
register_rest_route('stadion/v1', '/calendar/events/(?P<id>\d+)/log', [
    'methods' => 'POST',
...
```
  ...
  <verify>
- `curl -s /wp-json/stadion/v1/calendar/connections` returns 401 when logged out
- Routes appear in `curl -s /wp-json/stadion/v1` route listing
  </verify>
...
<task type="auto">
  ...
  <files>includes/class-rest-calendar.php</files>
  <action>
Implement the connection CRUD methods using STADION_Calendar_Connections helper.
...
public function create_connection($request) {
    ...
    if (!empty($data['credentials'])) {
        $data['credentials'] = STADION_Credential_Encryption::encrypt($data['credentials']);
    }
...
public function update_connection($request) {
    ...
    if (!empty($data['credentials'])) {
        $updates['credentials'] = STADION_Credential_Encryption::encrypt($data['credentials']);
    }
...
```php
'STADION_REST_Calendar' => 'class-rest-calendar.php',
```

Add initialization in stadion_init() with other REST classes (around line 165):
```php
// Calendar REST endpoints
if (stadion_is_rest_request()) {
    new STADION_REST_Calendar();
}
```
  </action>
  <verify>
- `curl -s /wp-json/stadion/v1/people/1/meetings` returns empty structure (not error)
- `curl -s /wp-json/stadion/v1/calendar/auth/google` returns 501 with message
  </verify>
...
- [ ] All routes appear in `/wp-json/stadion/v1` discovery document
...
<success_criteria>
- STADION_REST_Calendar class with all routes registered
