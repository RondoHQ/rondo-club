...
must_haves:
  ...
  key_links:
    - from: "includes/class-rest-api.php"
      ...
      pattern: "RONDO_VOG_Email|VOGEmail"
...
Output:
- `RONDO_VOG_Email` class for email sending and template management
- REST API endpoints for VOG settings (GET/POST /rondo/v1/vog/settings)
...
<task type="auto">
  ...
  <action>
Create a new PHP class `RONDO_VOG_Email` in the `Stadion\VOG` namespace with these methods:
...
1. **GET /rondo/v1/vog/settings**
   - Permission: `check_admin_permission` (admin only)
   - Returns: `{ from_email, template_new, template_renewal }` via `RONDO_VOG_Email::get_all_settings()`

2. **POST /rondo/v1/vog/settings**
   - Permission: `check_admin_permission` (admin only)
...
Add callback methods `get_vog_settings()` and `update_vog_settings()` that instantiate `RONDO_VOG_Email` and call appropriate methods.
  </action>
  <verify>
...
`grep -n "get_vog_settings\|update_vog_settings" includes/class-rest-api.php` shows callbacks
  ...
  <done>
GET and POST /rondo/v1/vog/settings endpoints exist. Admin-only access. Returns/updates from_email, template_new, template_renewal.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
1. Add the VOG email class to the `rondo_init()` function where other classes are instantiated.

Find the section that loads classes (around line 95-120 in functions.php) and add:
```php
// VOG Email Service
require_once RONDO_THEME_DIR . '/includes/class-vog-email.php';
```
...
```php
class_alias( 'Stadion\VOG\VOGEmail', 'RONDO_VOG_Email' );
```
...
`grep "RONDO_VOG_Email" functions.php` shows the class alias
  </verify>
  <done>
...
2. REST endpoints exist: Deploy and test via curl:
   - `curl -X GET /wp-json/rondo/v1/vog/settings` (requires admin auth)
...
<success_criteria>
- RONDO_VOG_Email class exists with email sending and settings methods
- GET/POST /rondo/v1/vog/settings endpoints work (admin only)
