...
Check if `class-cli-commands.php` exists. If not, create it with `RONDO_CLI_Commands` class.
...
if (class_exists('WP_CLI')) {
    WP_CLI::add_command('prm migrate-todos', ['RONDO_CLI_Commands', 'migrate_todos']);
}
```
...
<task type="auto">
  ...
  <action>
Remove all todo-related code from `RONDO_Comment_Types`:
...
<task type="auto">
  ...
  <action>
Remove legacy todo code from `RONDO_REST_API`:

1. Remove `/rondo/v1/todos` route registration from `register_routes()`
   (Now handled by RONDO_REST_Todos)

2. Remove `get_all_todos()` method entirely
   (Now handled by RONDO_REST_Todos::get_todos())
...
- [ ] GET /rondo/v1/todos returns todos from CPT (via new class)
