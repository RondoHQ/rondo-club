...
Check if `class-cli-commands.php` exists. If not, create it with `STADION_CLI_Commands` class.

**Command:** `wp prm migrate-todos [--dry-run]`

**Implementation:**
1. Query all comments with `comment_type = 'stadion_todo'`
2. For each comment:
   - Create `stadion_todo` post with:
...
if (class_exists('WP_CLI')) {
    WP_CLI::add_command('prm migrate-todos', ['STADION_CLI_Commands', 'migrate_todos']);
}
```
...
<task type="auto">
  ...
  <action>
Remove all todo-related code from `STADION_Comment_Types`:
...
<task type="auto">
  ...
  <action>
Remove legacy todo code from `STADION_REST_API`:

1. Remove `/stadion/v1/todos` route registration from `register_routes()`
   (Now handled by STADION_REST_Todos)

2. Remove `get_all_todos()` method entirely
   (Now handled by STADION_REST_Todos::get_todos())
...
- [ ] GET /stadion/v1/todos returns todos from CPT (via new class)
