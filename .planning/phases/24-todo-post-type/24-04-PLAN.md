...
<task type="auto">
  ...
  <action>
In `STADION_REST_API`, add new method to count open todos from CPT and update dashboard:
...
private function count_open_todos_cpt() {
    ...
    // Query stadion_todo posts where:
...
1. **CPT Registration:**
   - `test_stadion_todo_post_type_registered()` - verify post type exists
...
3. **REST API - CRUD:**
   - `test_get_todos_returns_user_todos()` - GET /stadion/v1/todos returns user's todos
   - `test_get_person_todos_filters_by_person()` - GET /stadion/v1/people/{id}/todos filters
   - `test_create_todo_creates_stadion_todo_post()` - POST creates CPT post
...
**Helpers needed:**
- `createApprovedStadionUser()` - from existing tests
...
- [ ] `GET /stadion/v1/dashboard` returns accurate `open_todos_count`
