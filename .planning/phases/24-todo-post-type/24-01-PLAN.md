...
<objective>
Register `prm_todo` custom post type with ACF fields and add access control filtering.
...
Output: `prm_todo` post type registered, ACF fields defined, access control integrated.
</objective>
...
<task type="auto">
  <name>Task 1: Register prm_todo custom post type</name>
...
Call `$this->register_todo_post_type()` from `register_post_types()`.
  ...
  <verify>
WordPress admin shows "Todos" in left menu after visiting /wp-admin/. Use `wp post-type list | grep prm_todo` to verify registration.
  </verify>
  <done>
`prm_todo` post type registered, visible in admin menu, accessible via wp/v2/todos REST endpoint.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
Create ACF field group for `prm_todo` post type with these fields:
...
Location rule: post_type == prm_todo
...
<task type="auto">
  <name>Task 3: Add access control for prm_todo</name>
  <files>includes/class-access-control.php</files>
  <action>
Update `PRM_Access_Control` to handle the new `prm_todo` post type:

1. Add `'prm_todo'` to `$controlled_post_types` array (line ~17)
...
This ensures todos follow same visibility/workspace rules as person, company, important_date.
  ...
  <verify>
Create a todo via admin as user A, verify user B cannot see it via REST. Check `$controlled_post_types` includes `prm_todo`.
  </verify>
  <done>
prm_todo has full access control: query filtering, REST API filtering, workspace support.
  </done>
</task>
...
- [ ] `wp post-type list | grep prm_todo` shows post type registered
- [ ] `/wp-admin/edit.php?post_type=prm_todo` loads without error
- [ ] ACF fields visible when creating/editing a todo in admin
- [ ] REST endpoint `/wp/v2/todos` accessible (may be empty)
- [ ] `PRM_Access_Control::$controlled_post_types` includes `prm_todo`
</verification>

<success_criteria>

- prm_todo CPT registered with correct configuration
