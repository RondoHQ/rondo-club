...
<objective>
Create `PRM_REST_Todos` class with full CRUD endpoints for the todo CPT.
...
<task type="auto">
  <name>Task 1: Create PRM_REST_Todos class</name>
  <files>includes/class-rest-todos.php</files>
  <action>
Create new file `includes/class-rest-todos.php` extending `PRM_REST_Base`. Implement these endpoints:

**Person-scoped endpoints:**
1. `GET /prm/v1/people/{person_id}/todos` - Get todos for a specific person
2. `POST /prm/v1/people/{person_id}/todos` - Create todo linked to person

**Global endpoints:**
3. `GET /prm/v1/todos` - Get all todos for current user (with optional `completed` param)
4. `GET /prm/v1/todos/{id}` - Get single todo
5. `PUT /prm/v1/todos/{id}` - Update todo
6. `DELETE /prm/v1/todos/{id}` - Delete todo
...
- Query `prm_todo` post type with ACF field `related_person` for person filtering
...
For input: `sanitize_textarea_field()` for content, `sanitize_text_field()` for due_date.
  ...
  <verify>
File exists at `includes/class-rest-todos.php`. Class extends `PRM_REST_Base`. Has all 6 endpoint methods registered.
  </verify>
  <done>
`PRM_REST_Todos` class created with all CRUD endpoints and proper permission callbacks.
  </done>
</task>

<task type="auto">
  <name>Task 2: Register PRM_REST_Todos in functions.php</name>
...
2. In `prm_init()` function, instantiate the class (after other REST classes):
   `new PRM_REST_Todos();`

Follow the existing pattern for other REST classes like `PRM_REST_People`, `PRM_REST_Companies`.
  </action>
  <verify>
`grep -n "class-rest-todos" functions.php` shows require_once line.
`grep -n "PRM_REST_Todos" functions.php` shows instantiation in prm_init().
  </verify>
  <done>
PRM_REST_Todos class loaded and instantiated on theme initialization.
  </done>
</task>
...
1. Create a test todo via admin:
   - Go to /wp-admin/post-new.php?post_type=prm_todo
   - Add title (todo content), select a related person, save

2. Test GET all todos:
   `curl -X GET "https://cael.is/wp-json/prm/v1/todos" -H "Cookie: <auth_cookie>"`

3. Test GET person todos:
   `curl -X GET "https://cael.is/wp-json/prm/v1/people/{person_id}/todos" -H "Cookie: <auth_cookie>"`
...
- [ ] `GET /prm/v1/todos` returns array (empty is OK)
- [ ] `GET /prm/v1/people/{id}/todos` returns array filtered by person
...
- PRM_REST_Todos class complete with all 6 endpoints
