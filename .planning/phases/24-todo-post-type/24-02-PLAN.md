...
<objective>
Create `STADION_REST_Todos` class with full CRUD endpoints for the todo CPT.
...
<task type="auto">
  <name>Task 1: Create STADION_REST_Todos class</name>
  <files>includes/class-rest-todos.php</files>
  <action>
Create new file `includes/class-rest-todos.php` extending `STADION_REST_Base`. Implement these endpoints:

**Person-scoped endpoints:**
1. `GET /stadion/v1/people/{person_id}/todos` - Get todos for a specific person
2. `POST /stadion/v1/people/{person_id}/todos` - Create todo linked to person

**Global endpoints:**
3. `GET /stadion/v1/todos` - Get all todos for current user (with optional `completed` param)
4. `GET /stadion/v1/todos/{id}` - Get single todo
5. `PUT /stadion/v1/todos/{id}` - Update todo
6. `DELETE /stadion/v1/todos/{id}` - Delete todo
...
For input: `sanitize_textarea_field()` for content, `sanitize_text_field()` for due_date.
  ...
  <verify>
File exists at `includes/class-rest-todos.php`. Class extends `STADION_REST_Base`. Has all 6 endpoint methods registered.
  </verify>
  <done>
`STADION_REST_Todos` class created with all CRUD endpoints and proper permission callbacks.
  </done>
</task>

<task type="auto">
  <name>Task 2: Register STADION_REST_Todos in functions.php</name>
...
2. In `stadion_init()` function, instantiate the class (after other REST classes):
   `new STADION_REST_Todos();`

Follow the existing pattern for other REST classes like `STADION_REST_People`, `STADION_REST_Companies`.
  </action>
  <verify>
`grep -n "class-rest-todos" functions.php` shows require_once line.
`grep -n "STADION_REST_Todos" functions.php` shows instantiation in stadion_init().
  </verify>
  <done>
STADION_REST_Todos class loaded and instantiated on theme initialization.
  </done>
</task>
...
2. Test GET all todos:
   `curl -X GET "https://cael.is/wp-json/stadion/v1/todos" -H "Cookie: <auth_cookie>"`

3. Test GET person todos:
   `curl -X GET "https://cael.is/wp-json/stadion/v1/people/{person_id}/todos" -H "Cookie: <auth_cookie>"`
...
- [ ] `GET /stadion/v1/todos` returns array (empty is OK)
- [ ] `GET /stadion/v1/people/{id}/todos` returns array filtered by person
...
- STADION_REST_Todos class complete with all 6 endpoints
