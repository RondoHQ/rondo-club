...
requires:
  - phase: 24-01
    provides: prm_todo CPT registration
  - phase: 24-02
    provides: PRM_REST_Todos class for CRUD operations
provides:
...
patterns-established:
  - "PRM_Todos_CLI_Command follows existing CLI command patterns"
...
- Created `PRM_Todos_CLI_Command` class with `migrate` subcommand for todo migration
- Successfully migrated 13 comment-based todos to prm_todo CPT posts on production
- Removed all todo-related code from `PRM_Comment_Types` (152 lines)
- Removed legacy `get_all_todos()` and `/prm/v1/todos` route from `PRM_REST_API`
...
- `includes/class-wp-cli.php` - Added `PRM_Todos_CLI_Command` class with migrate subcommand
...
2. **Dashboard count**: Uses `WP_Query` with automatic access control filtering via `PRM_Access_Control` hooks
...
- Todos accessible via `/prm/v1/todos` from new `PRM_REST_Todos` class
