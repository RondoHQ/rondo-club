...
Output: RONDO_Calendar_Matcher class with email-first matching, fuzzy name fallback, and transient-based email lookup cache.
</objective>
...
<task type="auto">
  <name>Task 1: Create RONDO_Calendar_Matcher class</name>
...
```php
'RONDO_Calendar_Matcher' => 'class-calendar-matcher.php',
```
...
add_action('acf/save_post', function($post_id) {
    if (get_post_type($post_id) === 'person') {
        ...
        RONDO_Calendar_Matcher::invalidate_cache($user_id);
    }
}, 20);
```
  </action>
  ...
  <done>
    - RONDO_Calendar_Matcher class with match_attendees(), get_email_lookup(), fuzzy_name_match() methods
...
<task type="auto">
  ...
  <action>
Update both calendar providers to call RONDO_Calendar_Matcher after upserting events.

**In RONDO_Google_Calendar_Provider::upsert_event():**
...
**In RONDO_CalDAV_Provider::upsert_event():**
...
```
  ...
  <verify>
    - RONDO_Google_Calendar_Provider::upsert_event() calls RONDO_Calendar_Matcher::match_attendees()
    - RONDO_CalDAV_Provider::upsert_event() calls RONDO_Calendar_Matcher::match_attendees()
...
**Endpoint:** `GET /rondo/v1/people/{id}/meetings`
...
```php
register_rest_route('rondo/v1', '/people/(?P<id>\d+)/meetings', [
...
public function check_person_access($request) {
    ...
    return RONDO_Access_Control::can_access($person_id);
}
```
  </action>
  <verify>
    - REST endpoint registered: /rondo/v1/people/{id}/meetings
  ...
  <done>
    - GET /rondo/v1/people/{id}/meetings endpoint returns person's calendar events
...
- [ ] RONDO_Calendar_Matcher class has match_attendees(), get_email_lookup(), fuzzy_name_match()
- [ ] Google provider calls matching on sync
- [ ] CalDAV provider calls matching on sync
- [ ] REST endpoint `/rondo/v1/people/{id}/meetings` returns correct response format
- [ ] `npm run build` succeeds (no frontend changes, but verify nothing broken)
</verification>

<success_criteria>
- RONDO_Calendar_Matcher class implemented with email-first, fuzzy-name-fallback algorithm
