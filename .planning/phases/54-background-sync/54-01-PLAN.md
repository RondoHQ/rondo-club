...
<task type="auto">
  <name>Task 1: Create RONDO_Calendar_Sync class with WP-Cron scheduling</name>
  <files>includes/class-calendar-sync.php, functions.php</files>
  <action>
Create new RONDO_Calendar_Sync class that:
...
3. Implements run_background_sync() callback that:
   ...
   - Calls appropriate provider sync() method (RONDO_Google_Calendar_Provider or RONDO_CalDAV_Provider)
...
Add class to functions.php autoloader and instantiate in rondo_init().
  </action>
  <verify>
...
- Verify class loads without fatal errors
  ...
  <done>
- RONDO_Calendar_Sync class created with WP-Cron scheduling
...
<task type="auto">
  ...
  <action>
Add auto_log_past_meetings() method to RONDO_Calendar_Sync class that:
...
2. For each qualifying event:
   ...
   - Actually reuse the logic from RONDO_REST_Calendar::log_event_as_activity()
   - Extract method from REST class to RONDO_Calendar_Sync::create_activity_from_event()
...
<task type="auto">
  ...
  <files>includes/class-calendar-sync.php</files>
  <action>
Add utility methods to RONDO_Calendar_Sync:
