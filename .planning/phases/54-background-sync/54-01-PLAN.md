...
<task type="auto">
  <name>Task 1: Create STADION_Calendar_Sync class with WP-Cron scheduling</name>
  <files>includes/class-calendar-sync.php, functions.php</files>
  <action>
Create new STADION_Calendar_Sync class that:

1. Registers custom cron schedule 'every_15_minutes' via cron_schedules filter (interval: 900)

2. Schedules 'stadion_calendar_sync' cron event on theme activation if not already scheduled:
   - Check wp_next_scheduled('stadion_calendar_sync')
   - If not scheduled, wp_schedule_event(time(), 'every_15_minutes', 'stadion_calendar_sync')

3. Implements run_background_sync() callback that:
   - Gets all users with _stadion_calendar_connections meta (meta_compare EXISTS)
   ...
   - Calls appropriate provider sync() method (STADION_Google_Calendar_Provider or STADION_CalDAV_Provider)
   - Updates last_sync and last_error on connection after sync
   - Logs errors but doesn't fail entire run
   - Uses set_transient('stadion_calendar_sync_last_user_index', $index, HOUR_IN_SECONDS) to rotate users
...
Add class to functions.php autoloader and instantiate in stadion_init().
  </action>
  <verify>
- Check cron schedule exists: wp cron event list | grep stadion_calendar_sync
- Verify class loads without fatal errors
  </verify>
  <done>
- STADION_Calendar_Sync class created with WP-Cron scheduling
...
<task type="auto">
  ...
  <action>
Add auto_log_past_meetings() method to STADION_Calendar_Sync class that:
...
2. For each qualifying event:
   ...
   - Actually reuse the logic from STADION_REST_Calendar::log_event_as_activity()
   - Extract method from REST class to STADION_Calendar_Sync::create_activity_from_event()
...
<task type="auto">
  ...
  <files>includes/class-calendar-sync.php</files>
  <action>
Add utility methods to STADION_Calendar_Sync:

1. get_sync_status() static method returning:
   - next_scheduled: wp_next_scheduled('stadion_calendar_sync')
...
- [ ] WP-Cron 'stadion_calendar_sync' event is scheduled (wp cron event list)
