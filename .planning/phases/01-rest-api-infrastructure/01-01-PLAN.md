...
Output: Abstract `STADION_REST_Base` class with shared methods, `STADION_REST_API` updated to extend it.
</objective>
...
- `check_team_edit_permission()` (line 1638) - Edit permission check
- `format_person_summary()` (line 1474) - Response formatting helper
- `format_team_summary()` (line 1489) - Response formatting helper
- `format_date()` (line 1502) - Response formatting helper

**Decisions:**
- Keep all routes in STADION_REST_API for now (phases 2-3 will extract them)
...
<task type="auto">
  <name>Task 1: Create STADION_REST_Base abstract class</name>
  <files>includes/class-rest-base.php</files>
  <action>
Create new file `includes/class-rest-base.php` with abstract class `STADION_REST_Base`.
...
5. `check_team_edit_permission($request)` - verify team edit permission

Also extract these protected helper methods:
6. `format_person_summary($post)` - format person for API response
7. `format_team_summary($post)` - format team for API response
...
- `STADION_` prefix for class name
...
<task type="auto">
  <name>Task 2: Update STADION_REST_API to extend base class</name>
  <files>includes/class-rest-api.php</files>
  <action>
Modify `class-rest-api.php`:

1. Change class declaration from `class STADION_REST_API` to `class STADION_REST_API extends STADION_REST_Base`
...
2. Remove these methods (now inherited from base):
   ...
   - `check_team_edit_permission()` (~23 lines, around line 1638)
   - `format_person_summary()` (~15 lines, around line 1474)
   - `format_team_summary()` (~13 lines, around line 1489)
...
Verify by checking that method calls like `$this->check_user_approved()` will resolve to the parent class method.
  ...
  <verify>php -l includes/class-rest-api.php returns no syntax errors AND grep -c "extends STADION_REST_Base" includes/class-rest-api.php returns 1</verify>
  <done>STADION_REST_API extends STADION_REST_Base, 8 methods removed, no duplicate method definitions</done>
</task>
...
<task type="auto">
  ...
  <action>
Add `STADION_REST_Base` to the autoloader class map in functions.php.

Find the `$class_map` array (around line 55) and add:
```php
'STADION_REST_Base' => 'class-rest-base.php',
```

Add it in alphabetical order by class name (after STADION_Reminders, before STADION_REST_API).

Also update the comment block above the array if one exists to reflect the new class.
  </action>
  <verify>grep "STADION_REST_Base" functions.php returns the new mapping line</verify>
  <done>Autoloader includes STADION_REST_Base mapping</done>
</task>
...
- [ ] `grep "extends STADION_REST_Base" includes/class-rest-api.php` returns match
- [ ] `grep "STADION_REST_Base" functions.php` returns autoloader mapping
</verification>

<success_criteria>
- Abstract base class created with shared permission and formatting methods
- STADION_REST_API extends base class
- Autoloader updated
