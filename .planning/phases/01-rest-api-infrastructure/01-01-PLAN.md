...
Output: Abstract `RONDO_REST_Base` class with shared methods, `RONDO_REST_API` updated to extend it.
</objective>
...
**Decisions:**
- Keep all routes in RONDO_REST_API for now (phases 2-3 will extract them)
...
<task type="auto">
  <name>Task 1: Create RONDO_REST_Base abstract class</name>
  <files>includes/class-rest-base.php</files>
  <action>
Create new file `includes/class-rest-base.php` with abstract class `RONDO_REST_Base`.
...
<task type="auto">
  <name>Task 2: Update RONDO_REST_API to extend base class</name>
  <files>includes/class-rest-api.php</files>
  <action>
Modify `class-rest-api.php`:

1. Change class declaration from `class RONDO_REST_API` to `class RONDO_REST_API extends RONDO_REST_Base`
...
Verify by checking that method calls like `$this->check_user_approved()` will resolve to the parent class method.
  ...
  <verify>php -l includes/class-rest-api.php returns no syntax errors AND grep -c "extends RONDO_REST_Base" includes/class-rest-api.php returns 1</verify>
  <done>RONDO_REST_API extends RONDO_REST_Base, 8 methods removed, no duplicate method definitions</done>
</task>
...
<task type="auto">
  ...
  <action>
Add `RONDO_REST_Base` to the autoloader class map in functions.php.

Find the `$class_map` array (around line 55) and add:
```php
'RONDO_REST_Base' => 'class-rest-base.php',
```

Add it in alphabetical order by class name (after RONDO_Reminders, before RONDO_REST_API).

Also update the comment block above the array if one exists to reflect the new class.
  </action>
  <verify>grep "RONDO_REST_Base" functions.php returns the new mapping line</verify>
  <done>Autoloader includes RONDO_REST_Base mapping</done>
</task>
...
- [ ] `grep "extends RONDO_REST_Base" includes/class-rest-api.php` returns match
- [ ] `grep "RONDO_REST_Base" functions.php` returns autoloader mapping
</verification>

<success_criteria>
- Abstract base class created with shared permission and formatting methods
- RONDO_REST_API extends base class
- Autoloader updated
