...
All routes in prm/v1 namespace must respond correctly:
...
- Company endpoints: /companies/{id}/people, /companies/{id}/logo
...
wp eval "
\$api = new PRM_REST_API();
echo 'PRM_REST_API instantiated successfully\n';
...
```
  ...
  <verify>wp eval or php -r command shows PRM_REST_Base as parent class</verify>
  <done>Class inheritance verified working, parent class is PRM_REST_Base</done>
</task>
...
curl -s -o /dev/null -w "%{http_code}" "$SITE_URL/wp-json/prm/v1/version"
# Expected: 200

# Test authenticated endpoint (will return 401 without auth, which is correct behavior)
curl -s -o /dev/null -w "%{http_code}" "$SITE_URL/wp-json/prm/v1/dashboard"
...
1. Count of routes still in PRM_REST_API:
```bash
...
- [ ] PHP class inheritance verified (PRM_REST_API extends PRM_REST_Base)
