...
All routes in rondo/v1 namespace must respond correctly:
...
wp eval "
\$api = new RONDO_REST_API();
echo 'RONDO_REST_API instantiated successfully\n';
...
```
  ...
  <verify>wp eval or php -r command shows RONDO_REST_Base as parent class</verify>
  <done>Class inheritance verified working, parent class is RONDO_REST_Base</done>
</task>
...
curl -s -o /dev/null -w "%{http_code}" "$SITE_URL/wp-json/rondo/v1/version"
# Expected: 200

# Test authenticated endpoint (will return 401 without auth, which is correct behavior)
curl -s -o /dev/null -w "%{http_code}" "$SITE_URL/wp-json/rondo/v1/dashboard"
...
1. Count of routes still in RONDO_REST_API:
```bash
...
- [ ] PHP class inheritance verified (RONDO_REST_API extends RONDO_REST_Base)
