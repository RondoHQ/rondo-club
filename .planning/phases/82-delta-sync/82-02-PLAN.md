...
must_haves:
  truths:
    ...
    - "Deleted contacts in Google are detected and unlinked in Caelis"
  artifacts:
...
4. **Add helper method** `unlink_contact(string $resource_name): void`:
   ...
   - Preserves all Caelis data, just removes Google link
...
5. **Update run_background_sync()** to capture and log sync results:
   ...
   - Log success with stats: `error_log(sprintf('PRM_Contacts_Sync: User %d - pulled %d, pushed %d', ...))`
...
<success_criteria>
- Delta sync uses syncToken for efficient Google -> Caelis sync
- Expired syncToken gracefully falls back to full sync
- Deleted contacts in Google are detected and unlinked in Caelis
- Modified Caelis contacts are pushed to Google
