...
must_haves:
  ...
  key_links:
    - from: "functions.php"
      ...
      via: "class instantiation in prm_init()"
...
<task type="auto">
  ...
  <action>
Create a new `GoogleContactsSync` class in the `Caelis\Contacts` namespace following the `Caelis\Calendar\Sync` pattern exactly.
...
6. **Get users with connections** `get_users_with_connections()`:
   - Query users with `_prm_google_contacts_connection` meta key
...
2. **Instantiate in prm_init()** function (where other classes are instantiated):
   ```php
   new \Caelis\Contacts\GoogleContactsSync();
   ```
...
4. Verify cron is registered:
  ...
  <verify>
The cron event `prm_google_contacts_sync` appears in the WP-CLI cron event list output.
