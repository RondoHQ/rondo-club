...
must_haves:
  ...
  key_links:
    - from: "src/pages/Settings.jsx"
      to: "/rondo/v1/google-contacts/sync"
...
<task type="auto">
  ...
  <action>
Add two new REST endpoints in RONDO_REST_API class.
           ...
           'args'                => [
               'frequency' => [
                   ...
                   'validate_callback' => function($value) {
                       $valid = array_keys(\Stadion\Contacts\GoogleContactsConnection::get_frequency_options());
...
2. **Add callback method** `trigger_contacts_sync()`:
   ...
   public function trigger_contacts_sync(\WP_REST_Request $request) {
       ...
       $connection = \Stadion\Contacts\GoogleContactsConnection::get_connection($user_id);
       ...
       try {
           // Run delta sync
           $sync = new \Stadion\Contacts\GoogleContactsSync();
...
3. **Add callback method** `update_contacts_sync_frequency()`:
   ...
   public function update_contacts_sync_frequency(\WP_REST_Request $request) {
       ...
       // Update frequency
       \Stadion\Contacts\GoogleContactsConnection::update_connection($user_id, [
           'sync_frequency' => $frequency,
       ]);

       $options = \Stadion\Contacts\GoogleContactsConnection::get_frequency_options();
...
4. **Add `sync_user_manual()` method to GoogleContactsSync** in `includes/class-google-contacts-sync.php`:
   ...
   public function sync_user_manual(int $user_id): array {
       ...
       $importer = new \Stadion\Import\GoogleContactsAPI($user_id);
...
1. **Add API functions to client.js**:
   ...
   export const triggerContactsSync = () =>
     api.post('/rondo/v1/google-contacts/sync');

   export const updateContactsSyncFrequency = (frequency) =>
     api.post('/rondo/v1/google-contacts/sync-frequency', { frequency });
   ```
...
3. **Update contacts status endpoint** to return sync_frequency and last_sync (if not already returned):
   Check if `/rondo/v1/google-contacts/status` endpoint returns these fields. If not, update it in class-rest-api.php to include:
   ```php
...
1. Run `npm run build` from /Users/joostdevalk/Code/rondo/rondo-club/

2. Deploy using rsync commands:
   ```bash
   rsync -avz --delete -e "ssh -p 18765" /Users/joostdevalk/Code/rondo/rondo-club/dist/ u25-eninwxjgiulh@c1130624.sgvps.net:~/www/cael.is/public_html/wp-content/themes/rondo-club/dist/
   rsync -avz --exclude='.git' --exclude='node_modules' --exclude='dist' -e "ssh -p 18765" /Users/joostdevalk/Code/rondo/rondo-club/ u25-eninwxjgiulh@c1130624.sgvps.net:~/www/cael.is/public_html/wp-content/themes/rondo-club/
   ```
