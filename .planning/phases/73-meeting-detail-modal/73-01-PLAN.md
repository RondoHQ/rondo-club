...
```bash
curl -s "https://cael.is/wp-json/stadion/v1/calendar/today-meetings" -H "Cookie: [auth]" | jq '.meetings[0].attendees'
```
...
```php
// GET /stadion/v1/calendar/events/(?P<id>\d+)/notes - Get meeting notes
register_rest_route(
    'stadion/v1',
...
// PUT /stadion/v1/calendar/events/(?P<id>\d+)/notes - Update meeting notes
register_rest_route(
    'stadion/v1',
    '/calendar/events/(?P<id>\d+)/notes',
    [
...
Test GET:
```bash
curl -s "https://cael.is/wp-json/stadion/v1/calendar/events/123/notes" -H "Cookie: [auth]"
```
Should return { "notes": "" } for new event.

Test PUT:
```bash
curl -X PUT "https://cael.is/wp-json/stadion/v1/calendar/events/123/notes" \
...
Should return { "success": true, "message": "Notes saved." }
  ...
  <done>GET and PUT /stadion/v1/calendar/events/{id}/notes endpoints working with proper auth</done>
</task>
...
- GET /stadion/v1/calendar/events/{id}/notes returns stored notes
- PUT /stadion/v1/calendar/events/{id}/notes saves notes to _meeting_notes meta
