...
```
  ...
  <verify>Test by checking API response includes timezone offset (curl /prm/v1/calendar/today-meetings), verify frontend shows correct local time</verify>
...
**Note:** The PHP-rendered favicon from `prm_theme_add_favicon()` will be replaced by the JS-generated one once React loads. This is fine since the React app handles the SPA experience.
  </action>
