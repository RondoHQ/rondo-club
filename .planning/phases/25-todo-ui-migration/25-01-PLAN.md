...
<task type="auto">
  ...
  <action>
Modify `get_timeline()` method to include todos from the `prm_todo` CPT alongside notes and activities from comments.

**Implementation:**
1. After fetching comments (notes + activities), also query `prm_todo` posts for this person:
   ```php
   $todos = get_posts([
       'post_type'      => 'prm_todo',
       'post_status'    => 'publish',
...
**Note:** The access control for todos is automatic via `PRM_Access_Control` hooks on `WP_Query`.
  </action>
  <verify>
Test on production:
1. `curl -X GET "https://cael.is/wp-json/prm/v1/people/{person_id}/timeline" -H "Cookie: ..." | jq '.[] | select(.type == "todo")'`
