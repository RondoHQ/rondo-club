...
**Note:** The access control for todos is automatic via `STADION_Access_Control` hooks on `WP_Query`.
  </action>
  <verify>
Test on production:
1. `curl -X GET "https://cael.is/wp-json/stadion/v1/people/{person_id}/timeline" -H "Cookie: ..." | jq '.[] | select(.type == "todo")'`
