...
Output: Modified `/rondo/v1/fees` endpoint that accepts `forecast=true` parameter and returns next season projections.
</objective>
...
<task type="auto">
  ...
  <action>
Modify the `/rondo/v1/fees` endpoint to support forecast mode. Two changes required:
...
2. Test API endpoint:
   - `curl "https://stadion.svawc.nl/wp-json/rondo/v1/fees"` - returns current season data with nikki fields
   - `curl "https://stadion.svawc.nl/wp-json/rondo/v1/fees?forecast=true"` - returns next season (2026-2027) with 100% pro-rata, no nikki fields
  </verify>
  <done>
- `/rondo/v1/fees?forecast=true` returns next season key (2026-2027)
...
3. **API test (current):** `curl -s "URL/wp-json/rondo/v1/fees" | jq '.season, .forecast, (.members[0] | keys)'` shows current season, forecast=false, includes nikki fields
4. **API test (forecast):** `curl -s "URL/wp-json/rondo/v1/fees?forecast=true" | jq '.season, .forecast, (.members[0] | keys)'` shows next season, forecast=true, no nikki fields
5. **Pro-rata check:** `curl -s "URL/wp-json/rondo/v1/fees?forecast=true" | jq '.members | map(.prorata_percentage) | unique'` shows only `[1]`
