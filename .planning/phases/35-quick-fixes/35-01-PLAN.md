...
<objective>
Implement four quick UI fixes: clickable website links, remove labels column from teams, simplify Slack display, and switch to build numbers for refresh detection.

Purpose: Address pending UI polish items from backlog to improve UX.
Output: Cleaner Teams list, improved contact display, and reliable refresh detection.
</objective>
...
<task type="auto">
  <name>Task 1: Make team website clickable and remove labels column</name>
  <files>src/pages/Teams/TeamsList.jsx</files>
  <action>
In TeamListRow component (around line 388):
...
In TeamListView component:
3. Remove labels header:
   - Remove the SortableHeader for labels (around line 466)

In TeamsList component:
4. Clean up labels sorting:
   - Remove 'labels' option from sort dropdown (around line 729)
   - Remove labels sorting logic from sortedTeams useMemo (around lines 663-667)
   - Remove getTeamLabelNames helper function (lines 610-618)

5. Remove BulkLabelsModal and related state/handlers since labels aren't used for teams:
...
- Teams list shows website as clickable blue link opening in new tab
- Labels column is removed from teams list
...
- Labels column, sorting, and bulk action completely removed from Teams list
  </done>
</task>
...
2. In functions.php, update stadion_get_js_config() to include build_time:
...
4. In src/hooks/useVersionCheck.js, update to compare buildTime instead of version:
   ...
   - Read from `window.stadionConfig?.buildTime` instead of `version`
...
- stadionConfig.buildTime is populated in browser console
- /stadion/v1/version endpoint returns buildTime field
...
- [ ] `npm run lint` passes
- [ ] Teams list: website clickable, no labels column
- [ ] Person profile: Slack shows label only as link
- [ ] stadionConfig includes buildTime in browser
- [ ] /stadion/v1/version API returns buildTime
</verification>
