...
<task type="auto">
  ...
  <action>
Create UserIsolationTest extending StadionTestCase. Test user_can_access_post() method:

1. Create two Stadion users (Alice, Bob) using createStadionUser()
2. Approve both users via update_user_meta($user_id, 'stadion_user_approved', '1')
3. Create a person post authored by Alice using createPerson(['post_author' => $alice_id])
4. Get STADION_Access_Control instance (it's instantiated in functions.php, use global or new instance)
5. Test assertions:
   - Alice can access her own post (author check passes)
   - Bob cannot access Alice's post (non-author denied)
   - Test with important_date and team post types as well

Use wp_set_current_user() to switch user context where needed. Ensure tests cover all three controlled post types (person, team, important_date).
  </action>
...
1. Create two approved Stadion users
...
- [ ] Tests cover person, team, and important_date post types
