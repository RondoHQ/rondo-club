...
<task type="auto">
  ...
  <action>
Create VisibilityRulesTest extending CaelisTestCase. Test private visibility:

1. Create two approved Caelis users (Alice, Bob)
2. Create a person post authored by Alice
3. Set visibility to private: PRM_Visibility::set_visibility($post_id, 'private')
4. Test assertions:
...
Use PRM_Access_Control::user_can_access_post() for access checks.
  </action>
...
3. Add Alice as admin, Bob as member to workspace via PRM_Workspace_Members::add()
...
3. Share with Bob via PRM_Visibility::add_share($post_id, $bob_id, 'view')
4. Test assertions:
   - Alice can access (author)
   - Bob can access (direct share, despite private visibility)
   - Verify share permission via PRM_Visibility::get_share_permission()
   - Test 'edit' permission level

Also test:
- Remove share via PRM_Visibility::remove_share(), Bob loses access
