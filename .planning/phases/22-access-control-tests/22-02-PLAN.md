...
3. Set visibility to private: RONDO_Visibility::set_visibility($post_id, 'private')
4. Test assertions:
...
Use RONDO_Access_Control::user_can_access_post() for access checks.
  </action>
...
3. Add Alice as admin, Bob as member to workspace via RONDO_Workspace_Members::add()
...
3. Share with Bob via RONDO_Visibility::add_share($post_id, $bob_id, 'view')
4. Test assertions:
   - Alice can access (author)
   - Bob can access (direct share, despite private visibility)
   - Verify share permission via RONDO_Visibility::get_share_permission()
   - Test 'edit' permission level

Also test:
- Remove share via RONDO_Visibility::remove_share(), Bob loses access
