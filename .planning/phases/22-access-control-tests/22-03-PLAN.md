...
<task type="auto">
  ...
  <action>
Create WorkspacePermissionsTest extending StadionTestCase. Test STADION_Workspace_Members:

1. Test membership add/remove:
   - Create workspace and users
   - Add user as member: STADION_Workspace_Members::add($workspace_id, $user_id, 'member')
   - Verify is_member() returns true
   - Remove user: STADION_Workspace_Members::remove()
...
4. Test get_user_permission() on STADION_Access_Control:
   - Author gets 'owner'
...
<task type="auto">
  ...
  <files>tests/Wpunit/WorkspacePermissionsTest.php</files>
  <action>
Add tests for user approval blocking via STADION_User_Roles:

1. Test approval status:
   - Create Stadion user (default unapproved)
   - Verify STADION_User_Roles::is_user_approved() returns false
