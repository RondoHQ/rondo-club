...
<task type="auto">
  ...
  <action>
Create WorkspacePermissionsTest extending CaelisTestCase. Test PRM_Workspace_Members:

1. Test membership add/remove:
   - Create workspace and users
   - Add user as member: PRM_Workspace_Members::add($workspace_id, $user_id, 'member')
   - Verify is_member() returns true
   - Remove user: PRM_Workspace_Members::remove()
...
4. Test get_user_permission() on PRM_Access_Control:
   - Author gets 'owner'
...
<task type="auto">
  ...
  <files>tests/Wpunit/WorkspacePermissionsTest.php</files>
  <action>
Add tests for user approval blocking via PRM_User_Roles:

1. Test approval status:
   - Create Caelis user (default unapproved)
   - Verify PRM_User_Roles::is_user_approved() returns false
   - Approve via update_user_meta($user_id, 'caelis_user_approved', '1')
