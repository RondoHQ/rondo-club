...
<objective>
Set up PHPUnit testing infrastructure using wp-browser (Codeception) for the Caelis WordPress theme.
...
- Dedicated test database (caelis_test)
...
- Separate test database (caelis_test) - isolation from dev/prod
...
2. Create the test database via MySQL CLI:
   ```bash
   mysql -u root -e "CREATE DATABASE IF NOT EXISTS caelis_test CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
   ```
...
- Don't point to existing caelis database - it WILL be wiped
  </action>
  <verify>
- `composer show lucatume/wp-browser` shows version 4.5.x installed
- `mysql -u root -e "SHOW DATABASES;" | grep caelis_test` returns caelis_test
...
<task type="auto">
  <name>Task 2: Configure Codeception with WPLoader for Caelis</name>
...
2. Create tests/.env.testing template (to be copied to .env):
   ...
   TEST_DB_URL=mysql://root@localhost/caelis_test
...
5. Create tests/Support/Helper/Wpunit.php:
   ...
   class Wpunit extends \Codeception\Module {
       // Custom helper methods for Caelis tests
   }
   ```
...
<task type="auto">
  ...
  <files>tests/Support/CaelisTestCase.php, tests/Wpunit/SmokeTest.php</files>
  <action>
1. Create tests/Support/CaelisTestCase.php - base class for all Caelis tests:
   ```php
   <?php
...
2. Create tests/Wpunit/SmokeTest.php - verifies the setup works:
   ...
   class SmokeTest extends CaelisTestCase {
       ...
       public function test_can_create_caelis_user(): void {
           ...
           $this->assertTrue(in_array('caelis_user', $user->roles));
       }
...
- [ ] CaelisTestCase.php provides createPerson, createOrganization, createCaelisUser helpers
</verification>
...
- Test database caelis_test exists
- Codeception configured with WPLoader
- ACF Pro and Caelis theme activated in test environment
