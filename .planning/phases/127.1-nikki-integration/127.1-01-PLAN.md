...
must_haves:
  ...
  artifacts:
    - path: "includes/class-wp-cli.php"
      provides: "STADION_Nikki_CLI_Command class with import subcommand"
      contains: "class STADION_Nikki_CLI_Command"
  key_links:
    - from: "STADION_Nikki_CLI_Command::import()"
      to: "PDO SQLite"
...
Output: STADION_Nikki_CLI_Command class with `import` subcommand that reads SQLite, matches records by KNVB ID, and stores in person meta
</objective>
...
<task type="auto">
  <name>Task 1: Create STADION_Nikki_CLI_Command class</name>
  <files>includes/class-wp-cli.php</files>
  <action>
Add STADION_Nikki_CLI_Command class BEFORE the final closing brace of the `if ( defined( 'WP_CLI' ) && WP_CLI )` block (around line 2810).
...
Check class exists: `grep -n "class STADION_Nikki_CLI_Command" includes/class-wp-cli.php`
Check registration: `grep -n "stadion nikki" includes/class-wp-cli.php`
  </verify>
  <done>
STADION_Nikki_CLI_Command class exists with import method, build_knvb_lookup method, and command registered
  </done>
</task>
...
- [ ] `grep "class STADION_Nikki_CLI_Command" includes/class-wp-cli.php` finds class
