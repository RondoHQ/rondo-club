...
The codebase has well-established patterns for all components: WP-CLI commands follow the STADION_*_CLI_Command pattern with PHPDoc-style options, post meta storage follows the `_prefix_year_field` pattern, and the ContributieList.jsx component already displays fee columns that can be extended. The `knvb-id` field exists on person posts and is used for Sportlink integration.
...
**Primary recommendation:** Create STADION_Nikki_CLI_Command class with an `import` subcommand that reads SQLite via PDO, matches records by KNVB ID, stores data in `_nikki_{year}_total` and `_nikki_{year}_saldo` meta keys, then extend the fees REST endpoint and ContributieList to display the new columns.
...
```
includes/
  class-wp-cli.php              # Add STADION_Nikki_CLI_Command class
...
**Example:**
```php
// Source: includes/class-wp-cli.php (existing pattern from STADION_Google_Contacts_CLI_Command)
class STADION_Nikki_CLI_Command {
...
WP_CLI::add_command( 'stadion nikki', 'STADION_Nikki_CLI_Command' );
```
...
- `/Users/joostdevalk/Code/stadion/includes/class-wp-cli.php` - WP-CLI command patterns, registration
- `/Users/joostdevalk/Code/stadion/includes/class-rest-api.php` - get_fee_list() endpoint at line 2570
- `/Users/joostdevalk/Code/stadion/includes/class-membership-fees.php` - Fee cache meta key pattern
- `/Users/joostdevalk/Code/stadion/src/pages/Contributie/ContributieList.jsx` - Full component structure
- `/Users/joostdevalk/Code/stadion/src/pages/People/PersonDetail.jsx` - knvb-id field usage at line 1106
...
2. **WP-CLI pattern:** STADION_*_CLI_Command class with PHPDoc options, register with `WP_CLI::add_command()`
