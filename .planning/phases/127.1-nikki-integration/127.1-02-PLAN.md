...
must_haves:
  truths:
    ...
    - "REST API /stadion/v1/fees returns nikki_total and nikki_saldo per member"
  artifacts:
...
3. Test REST API returns new fields:
   ```bash
   source .env && curl -s "${DEPLOY_PRODUCTION_URL}/wp-json/stadion/v1/fees" -H "Cookie: ..." | jq '.members[0] | {nikki_total, nikki_saldo}'
   ```
