...
must_haves:
  truths:
    - "User can filter people by birth year range via filtered endpoint"
    - "Birth year filter uses YEAR() function on _birthdate meta"
  ...
  artifacts:
    - path: "includes/class-rest-people.php"
      provides: "Birth year filter parameters and SQL implementation"
      contains: "birth_year"
  key_links:
    - from: "birth_year_from/birth_year_to parameters"
      to: "YEAR(bd.meta_value) BETWEEN"
      via: "get_filtered_people method"
      pattern: "YEAR.*_birthdate"
---
...
2. **Test birth year filter via curl:**

   Test single year filter (birth_year_from only):
   ```bash
   # Replace with actual values from production
...
4. **Verify filter combines with other filters (AND logic):**
   ```bash
...
5. **Test validation rejects invalid years:**
   ```bash
