...
Output:
- `/rondo/v1/people/filtered` endpoint accepts `birth_year_from` and `birth_year_to` parameters
...
3. **Test birth year filter via curl:**
   ...
   curl -s "https://stadion.svawc.nl/wp-json/rondo/v1/people/filtered?birth_year_from=1985" \
...
4. **Test year range filter:**
   ```bash
   curl -s "https://stadion.svawc.nl/wp-json/rondo/v1/people/filtered?birth_year_from=1980&birth_year_to=1990" \
     -H "Cookie: <auth_cookie>" \
...
5. **Verify filter combines with other filters (AND logic):**
   ```bash
   curl -s "https://stadion.svawc.nl/wp-json/rondo/v1/people/filtered?birth_year_from=1985&ownership=mine" \
     -H "Cookie: <auth_cookie>" \
     -H "X-WP-Nonce: <nonce>" | jq '.total'
   ```
...
6. **Test validation rejects invalid years:**
   ```bash
   curl -s "https://stadion.svawc.nl/wp-json/rondo/v1/people/filtered?birth_year_from=1800" \
     -H "Cookie: <auth_cookie>" \
