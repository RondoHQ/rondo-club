...
must_haves:
  truths:
    - "Admin can trigger sync for a user via wp caelis google-contacts sync --user-id=ID"
    - "Admin can force full resync via --full flag"
    - "Admin can check sync status via wp caelis google-contacts status --user-id=ID"
    - "Admin can list unresolved conflicts via wp caelis google-contacts conflicts --user-id=ID"
    - "Admin can unlink all contacts to reset sync state via wp caelis google-contacts unlink-all --user-id=ID"
  artifacts:
    - path: "includes/class-wp-cli.php"
      provides: "PRM_Google_Contacts_CLI_Command class with 5 commands"
      contains: "class PRM_Google_Contacts_CLI_Command"
  key_links:
...
Output: `PRM_Google_Contacts_CLI_Command` class with 5 subcommands registered under `wp caelis google-contacts`.
</objective>
...
<task type="auto">
  <name>Task 1: Create PRM_Google_Contacts_CLI_Command class with all 5 commands</name>
  <files>includes/class-wp-cli.php</files>
  <action>
Add a new `PRM_Google_Contacts_CLI_Command` class inside the `if ( defined( 'WP_CLI' ) && WP_CLI )` block in class-wp-cli.php. Follow the existing pattern used by `PRM_Calendar_CLI_Command`.
...
- Options: `--user-id=<user_id>` (required)
- Query comments with type 'prm_activity' and activity_type meta = 'sync_conflict'
...
Note: Use `caelis` namespace (not `prm`) to match the requirement spec which says `wp caelis google-contacts`.
  </action>
  <verify>
...
wp caelis google-contacts --help

# Each subcommand should show help
wp caelis google-contacts sync --help
wp caelis google-contacts status --help
wp caelis google-contacts conflicts --help
wp caelis google-contacts unlink-all --help
```
  </verify>
  <done>
All 5 WP-CLI commands are implemented:
- wp caelis google-contacts sync --user-id=ID triggers delta sync
- wp caelis google-contacts sync --user-id=ID --full triggers full resync
- wp caelis google-contacts status --user-id=ID shows connection status
- wp caelis google-contacts conflicts --user-id=ID lists unresolved conflicts
- wp caelis google-contacts unlink-all --user-id=ID removes all Google links
  </done>
</task>
