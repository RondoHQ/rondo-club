...
3. Update the log message in the sync loop to include frequency info (around line 193):
   ...
   error_log(
       sprintf(
           'STADION_Calendar_Sync: Synced connection %s for user %d - %d events (%d created, %d updated) [freq: %d min]',
           $connection_id,
           $user_id,
           $result['total'] ?? 0,
           $result['created'] ?? 0,
...
4. Add a skip log for debugging (optional but helpful), in the frequency check block:
   ...
   // error_log( sprintf( 'STADION_Calendar_Sync: Skipping connection %s - not due yet', $connection_id ) );
   ```
  </action>
  <verify>
...
This makes the connection objects include a `next_sync` field that the UI can display.
  ...
  <verify>
    - Run: `composer lint` to check PHP syntax
    - Test: Call GET `/stadion/v1/calendar/sync/status` and verify `next_sync` field in response
  </verify>
  <done>
