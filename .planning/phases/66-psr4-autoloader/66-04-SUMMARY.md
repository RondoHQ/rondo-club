...
- Manual autoloader removed: `grep -c "function stadion_autoloader" functions.php` returns 0
...
4. **Import aliases**: Used for classes that appear in multiple namespaces:
   ...
   - `Stadion\Export\VCard` as `VCardExport`
   - `Stadion\REST\Calendar` as `RESTCalendar`
   - `Stadion\CardDAV\Server` as `CardDAVServer`
