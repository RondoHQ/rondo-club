...
- STADION_Post_Types → Stadion\Core\PostTypes
- STADION_Taxonomies → Stadion\Core\Taxonomies
- STADION_Access_Control → Stadion\Core\AccessControl
- STADION_Visibility → Stadion\Core\Visibility
- STADION_User_Roles → Stadion\Core\UserRoles
- STADION_Auto_Title → Stadion\Core\AutoTitle
- STADION_REST_Base → Stadion\REST\Base
- STADION_REST_API → Stadion\REST\Api
- STADION_REST_People → Stadion\REST\People
- STADION_REST_Companies → Stadion\REST\Companies
- STADION_REST_Todos → Stadion\REST\Todos
- STADION_REST_Workspaces → Stadion\REST\Workspaces
- STADION_REST_Slack → Stadion\REST\Slack
- STADION_REST_Import_Export → Stadion\REST\ImportExport
- STADION_REST_Calendar → Stadion\REST\Calendar
...
2. Rename the class to remove STADION_ prefix:
   - `class STADION_Post_Types` → `class PostTypes`
   - `class STADION_Taxonomies` → `class Taxonomies`
   - `class STADION_Access_Control` → `class AccessControl`
   - `class STADION_Visibility` → `class Visibility`
   - `class STADION_User_Roles` → `class UserRoles`
   - `class STADION_Auto_Title` → `class AutoTitle`
...
```
  ...
  <verify>Each file has `namespace Stadion\Core;` and class name without STADION_ prefix: `grep -l "namespace Stadion\\\\Core" includes/class-post-types.php includes/class-taxonomies.php includes/class-access-control.php includes/class-visibility.php includes/class-user-roles.php includes/class-auto-title.php`</verify>
...
2. Rename the class to remove STADION_REST_ prefix:
   - `class STADION_REST_Base` → `class Base`
   - `class STADION_REST_API` → `class Api`
   - `class STADION_REST_People` → `class People`
   - `class STADION_REST_Companies` → `class Companies`
   - `class STADION_REST_Todos` → `class Todos`
   - `class STADION_REST_Workspaces` → `class Workspaces`
   - `class STADION_REST_Slack` → `class Slack`
   - `class STADION_REST_Import_Export` → `class ImportExport`
   - `class STADION_REST_Calendar` → `class Calendar`

3. For classes that extend STADION_REST_Base, update the extends clause:
   - `extends STADION_REST_Base` → `extends Base`
