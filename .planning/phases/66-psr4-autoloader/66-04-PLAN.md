...
2. Update all class instantiations in rondo_init() and other functions:
   - `new RONDO_Post_Types()` → `new PostTypes()`
   - `new RONDO_Taxonomies()` → `new Taxonomies()`
   - `new RONDO_Access_Control()` → `new AccessControl()`
   - etc. for all classes
...
4. Note: VCard appears in both Import and Export namespaces, so use aliases:
  ...
  <verify>`php -l functions.php` passes, and grep for "new RONDO_" returns no results in functions.php</verify>
...
2. Add class aliases AFTER the use statements for backward compatibility. This ensures any code using old RONDO_* names still works:

```php
// Backward compatibility class aliases
// These allow code using old RONDO_* names to continue working
...
if ( ! class_exists( 'RONDO_Visibility' ) ) {
    class_alias( Visibility::class, 'RONDO_Visibility' );
}
...
if ( ! class_exists( 'RONDO_REST_Base' ) ) {
    class_alias( Base::class, 'RONDO_REST_Base' );
}
...
if ( ! class_exists( 'RONDO_REST_Workspaces' ) ) {
    class_alias( Workspaces::class, 'RONDO_REST_Workspaces' );
}
if ( ! class_exists( 'RONDO_REST_Slack' ) ) {
    class_alias( Slack::class, 'RONDO_REST_Slack' );
}
...
if ( ! class_exists( 'RONDO_REST_Calendar' ) ) {
    class_alias( Calendar::class, 'RONDO_REST_Calendar' );
}
if ( ! class_exists( 'RONDO_Reminders' ) ) {
    class_alias( Reminders::class, 'RONDO_Reminders' );
}
if ( ! class_exists( 'RONDO_Monica_Import' ) ) {
    class_alias( Monica::class, 'RONDO_Monica_Import' );
}
...
if ( ! class_exists( 'RONDO_Workspace_Members' ) ) {
    class_alias( WorkspaceMembers::class, 'RONDO_Workspace_Members' );
}
if ( ! class_exists( 'RONDO_Mentions' ) ) {
    class_alias( Mentions::class, 'RONDO_Mentions' );
}
...
if ( ! class_exists( 'RONDO_Credential_Encryption' ) ) {
    class_alias( CredentialEncryption::class, 'RONDO_Credential_Encryption' );
}
...
if ( ! class_exists( 'RONDO_Notification_Channel' ) ) {
    class_alias( Channel::class, 'RONDO_Notification_Channel' );
}
if ( ! class_exists( 'RONDO_Email_Channel' ) ) {
    class_alias( EmailChannel::class, 'RONDO_Email_Channel' );
}
if ( ! class_exists( 'RONDO_Slack_Channel' ) ) {
    class_alias( SlackChannel::class, 'RONDO_Slack_Channel' );
}
```
...
<task type="auto">
  ...
  <action>
1. Update class-wp-cli.php to add use statements for any RONDO_* classes it references:
   - Search for `new RONDO_` and `RONDO_*::` patterns
