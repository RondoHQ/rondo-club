...
**Established patterns:** CPT registration in PRM_Post_Types, REST endpoints in domain classes
**Constraining decisions:**
- Phase 7: Workspace membership via PRM_Workspace_Members
- Phase 8-01: PRM_REST_Workspaces class exists
</context>
...
<task type="auto">
  ...
  <action>
Add register_workspace_invite_post_type() method to PRM_Post_Types:
...
<task type="auto">
  <name>Task 2: Add invite REST endpoints to PRM_REST_Workspaces</name>
  <files>includes/class-rest-workspaces.php</files>
  <action>
Add invite management endpoints to PRM_REST_Workspaces:

1. POST /prm/v1/workspaces/(?P<id>\d+)/invites - Create & send invite
...
2. GET /prm/v1/workspaces/(?P<id>\d+)/invites - List pending invites
   - Permission: check_workspace_admin
   - Returns array of pending invites for workspace

3. DELETE /prm/v1/workspaces/(?P<id>\d+)/invites/(?P<invite_id>\d+) - Revoke invite
   - Permission: check_workspace_admin
   - Sets _invite_status = 'revoked'
   - Returns success

4. GET /prm/v1/invites/(?P<token>[a-zA-Z0-9]+) - Validate invite (PUBLIC)
...
5. POST /prm/v1/invites/(?P<token>[a-zA-Z0-9]+)/accept - Accept invite
...
- Sends via wp_mail() with HTML template:
  Subject: "You've been invited to [Workspace Name] on Caelis"
