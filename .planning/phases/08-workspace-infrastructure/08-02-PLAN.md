...
**Established patterns:** CPT registration in STADION_Post_Types, REST endpoints in domain classes
**Constraining decisions:**
- Phase 7: Workspace membership via STADION_Workspace_Members
- Phase 8-01: STADION_REST_Workspaces class exists
</context>
...
<task type="auto">
  ...
  <action>
Add register_workspace_invite_post_type() method to STADION_Post_Types:
...
<task type="auto">
  <name>Task 2: Add invite REST endpoints to STADION_REST_Workspaces</name>
  <files>includes/class-rest-workspaces.php</files>
  <action>
Add invite management endpoints to STADION_REST_Workspaces:

1. POST /stadion/v1/workspaces/(?P<id>\d+)/invites - Create & send invite
...
2. GET /stadion/v1/workspaces/(?P<id>\d+)/invites - List pending invites
   - Permission: check_workspace_admin
   - Returns array of pending invites for workspace

3. DELETE /stadion/v1/workspaces/(?P<id>\d+)/invites/(?P<invite_id>\d+) - Revoke invite
   - Permission: check_workspace_admin
   - Sets _invite_status = 'revoked'
   - Returns success

4. GET /stadion/v1/invites/(?P<token>[a-zA-Z0-9]+) - Validate invite (PUBLIC)
...
5. POST /stadion/v1/invites/(?P<token>[a-zA-Z0-9]+)/accept - Accept invite
