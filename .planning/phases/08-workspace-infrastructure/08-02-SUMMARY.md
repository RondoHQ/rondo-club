...
1. **POST /prm/v1/workspaces/{id}/invites** - Create & send invite
...
2. **GET /prm/v1/workspaces/{id}/invites** - List pending invites
   - Permission: workspace admin
   - Returns array with id, email, role, status, expires_at, invited_by, invited_at

3. **DELETE /prm/v1/workspaces/{id}/invites/{invite_id}** - Revoke invite
   - Permission: workspace admin
   - Sets status to 'revoked'
   - Only pending invites can be revoked

4. **GET /prm/v1/invites/{token}** - Validate invite (PUBLIC)
...
2. `feat(08-02): add invite REST endpoints to PRM_REST_Workspaces` (6f6c531)
...
- Email uses gradient header consistent with Caelis branding
