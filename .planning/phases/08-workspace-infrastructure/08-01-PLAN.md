...
Output: New STADION_REST_Workspaces class with CRUD endpoints for workspaces and members.
</objective>
...
**Tech stack available:** WordPress REST API, STADION_REST_Base pattern
**Established patterns:** Domain-specific REST classes extend STADION_REST_Base
**Constraining decisions:**
- Phase 7: Workspace membership stored in user meta via STADION_Workspace_Members
...
<task type="auto">
  <name>Task 1: Create STADION_REST_Workspaces class with member endpoints</name>
  <files>includes/class-rest-workspaces.php, functions.php</files>
  <action>
Create new STADION_REST_Workspaces class extending STADION_REST_Base:
...
1. In constructor, register routes via `add_action('rest_api_init', [$this, 'register_routes'])`:
   ...
   - GET /stadion/v1/workspaces - List user's workspaces
     ...
     - Uses STADION_Workspace_Members::get_user_workspaces() + enriches with post data
   ...
   - GET /stadion/v1/workspaces/(?P<id>\d+) - Get workspace details
     ...
     - Uses STADION_Workspace_Members::get_members()
   ...
   - POST /stadion/v1/workspaces/(?P<id>\d+)/members - Add member
     ...
     - Validates user exists, uses STADION_Workspace_Members::add()
   ...
   - DELETE /stadion/v1/workspaces/(?P<id>\d+)/members/(?P<user_id>\d+) - Remove member
     ...
     - Uses STADION_Workspace_Members::remove()
     - Prevent removing workspace owner (handled by class)

   - PUT /stadion/v1/workspaces/(?P<id>\d+)/members/(?P<user_id>\d+) - Update role
     - Permission: check_workspace_admin
     - Args: role (required, one of admin|member|viewer)
     - Uses STADION_Workspace_Members::update_role()
...
3. Update functions.php autoloader to map 'STADION_REST_Workspaces' => 'class-rest-workspaces.php'

4. Update stadion_init() to instantiate STADION_REST_Workspaces after other REST classes.

Follow existing pattern from STADION_REST_People: extend STADION_REST_Base, register routes in constructor.
  </action>
  <verify>Check route registration via: curl -X OPTIONS https://cael.is/wp-json/stadion/v1/workspaces (should show GET method)</verify>
  <done>
    - STADION_REST_Workspaces class created with all 5 endpoints
    - Permission callbacks check workspace membership/admin status
    - Class registered in autoloader and instantiated in stadion_init()
...
1. POST /stadion/v1/workspaces - Create workspace
   - Permission: check_user_approved
   - Args: title (required), description (optional)
   - Creates workspace CPT post with current user as author
   - STADION_Workspace_Members hook auto-adds author as admin (from 07-03)
   - Returns workspace data

2. PUT /stadion/v1/workspaces/(?P<id>\d+) - Update workspace
   - Permission: check_workspace_admin
   - Args: title (optional), description (optional)
   - Updates workspace post
   - Returns updated workspace data

3. DELETE /stadion/v1/workspaces/(?P<id>\d+) - Delete workspace
...
Note: Use wp_insert_post(), wp_update_post(), wp_trash_post() - standard WordPress functions.
  ...
  <verify>Test create: curl -X POST https://cael.is/wp-json/stadion/v1/workspaces -H "X-WP-Nonce: ..." -d "title=Test"</verify>
  <done>
...
- [ ] All 8 REST endpoints registered (OPTIONS /stadion/v1/workspaces shows methods)
- [ ] GET /stadion/v1/workspaces returns user's workspaces
