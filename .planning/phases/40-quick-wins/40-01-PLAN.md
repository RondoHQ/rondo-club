...
Note: The existing `handleToggleTodo` function already handles awaiting→completed transition (lines 264-279), so just pass it through.
...
<task type="auto">
  ...
  <action>
Add a new method to RONDO_Auto_Title class to lowercase email addresses in the contact_info repeater field.

1. Add new hook in constructor:
   ```php
   add_filter('acf/update_value/key=field_contact_value', [$this, 'maybe_lowercase_email'], 10, 4);
   add_filter('acf/update_value/key=field_team_contact_value', [$this, 'maybe_lowercase_email'], 10, 4);
   ```
...
Alternative simpler approach if the above is complex: Hook into acf/save_post and iterate through contact_info repeater to lowercase any email values.
  ...
  <done>Email addresses in contact_info are automatically lowercased on save for both person and team post types</done>
</task>
...
- [ ] Team email addresses are also lowercased on save
</verification>
