...
- REST API pattern follows `/stadion/v1/vog/settings` example (GET + POST on same route)
...
3. Verify REST API endpoints respond correctly using browser dev tools or curl:
   - GET `/wp-json/stadion/v1/membership-fees/settings` should return default values
...
- [ ] REST endpoint GET `/stadion/v1/membership-fees/settings` returns fee defaults
- [ ] REST endpoint POST `/stadion/v1/membership-fees/settings` updates and persists fees
...
2. REST API endpoints registered under `/stadion/v1/membership-fees/settings`
