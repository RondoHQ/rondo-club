...
Output: List view with First Name, Last Name, Labels, Team, Workspace columns and sorting by all fields.
</objective>
...
- personTeamMap lookup for team names
...
Keep the existing avatar rendering (thumbnail or initial fallback), deceased marker (†), and favorite star (yellow Star icon) with the first name column.
  </action>
  <verify>npm run build succeeds, list view shows separate First Name and Last Name columns</verify>
  <done>Table has 6 columns: checkbox, First Name (with avatar/icons), Last Name, Team, Workspace, Labels (to be added)</done>
</task>
...
<task type="auto">
  <name>Task 3: Add Team and Workspace sorting options</name>
  <files>src/pages/People/PeopleList.jsx</files>
  <action>
Extend the sorting logic to support Team and Workspace:
...
2. In filteredAndSortedPeople useMemo sort logic, add cases for new sort fields:
   ...
   For "team" sorting:
   - Get team name from personTeamMap[person.id] or empty string
   - Compare alphabetically (localeCompare)
   - Empty/no team sorts last
...
3. Ensure sortOrder (asc/desc) is respected for all new sort fields.
  ...
  <done>Sort dropdown includes Team, Workspace, Labels options; sorting produces correct order for each</done>
</task>
...
- [ ] List view shows 6 columns: Checkbox, First Name, Last Name, Team, Workspace, Labels
- [ ] Sort dropdown includes: First name, Last name, Last modified, Team, Workspace, Labels
