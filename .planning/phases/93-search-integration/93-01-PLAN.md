...
must_haves:
  truths:
    ...
    - "Searching for text that exists in a URL custom field returns matching Organizations"
...
<objective>
Integrate custom field values into global search for People and Organizations.

Purpose: Users can find records by searching text in custom fields (linkedin URL, company email, notes, etc.)
...
<task type="auto">
  ...
  <action>
Add two private helper methods to PRM_REST_API class:

1. `get_searchable_custom_fields( string $post_type ): array`
   - Use `\Caelis\CustomFields\Manager` to get active fields via `get_fields($post_type, false)`
...
Place these methods after the existing helper methods in the class (around line 380, after format_company_summary).
  </action>
  <verify>
PHP syntax check: `php -l includes/class-rest-api.php` passes without errors
  </verify>
  <done>
Two helper methods exist in PRM_REST_API class that can retrieve searchable field names and build meta queries
  </done>
</task>
...
**For Companies (replace simple 's' search with scored approach):**
1. Refactor company search to use same scoring pattern as people:
...
2. Searching text in Organization custom fields returns matching organizations
...
5. Repeat for Organizations if custom fields are defined there
</verification>
