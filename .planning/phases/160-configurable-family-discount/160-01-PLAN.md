...
must_haves:
  truths:
    ...
    - "get_family_discount_config() copies previous season config forward when new season has no config (copy-forward pattern)"
  ...
  key_links:
    ...
    - from: "includes/class-membership-fees.php::get_family_discount_config()"
      to: "get_previous_season_key()"
      via: "copy-forward: new seasons inherit previous season's discount config"
      pattern: "get_previous_season_key"
...
This method follows the **copy-forward pattern** from `get_categories_for_season()` (lines 603-615): when a season has no config, it checks the previous season and copies that config forward before falling back to defaults. This ensures that when a new season is created, discount percentages carry over automatically.
...
**Important:** The WP_Error code changes from `'invalid_categories'` to `'invalid_settings'` since validation now covers both categories and family discount. Update this in the existing error return.
  ...
  <verify>
Deploy and test via WP-CLI on production:
```bash
# Test GET endpoint returns family_discount for both seasons
source .env && ssh -p "$DEPLOY_SSH_PORT" "$DEPLOY_SSH_USER@$DEPLOY_SSH_HOST" "cd $DEPLOY_REMOTE_THEME_PATH && wp eval '
\$fees = new \Rondo\Fees\MembershipFees();
\$current = \$fees->get_season_key();
\$next = \$fees->get_next_season_key();
echo \"Current season (\" . \$current . \") discount: \" . json_encode(\$fees->get_family_discount_config(\$current)) . \"\n\";
echo \"Next season (\" . \$next . \") discount: \" . json_encode(\$fees->get_family_discount_config(\$next)) . \"\n\";
// Test save + read round-trip
\$fees->save_family_discount_config([\"second_child_percent\" => 30, \"third_child_percent\" => 60], \$current);
echo \"After save: \" . json_encode(\$fees->get_family_discount_config(\$current)) . \"\n\";
// Restore defaults
\$fees->save_family_discount_config([\"second_child_percent\" => 25, \"third_child_percent\" => 50], \$current);
echo \"After restore: \" . json_encode(\$fees->get_family_discount_config(\$current)) . \"\n\";
'"
```
Expected: both seasons return {second_child_percent: 25, third_child_percent: 50} by default. After save, current season returns {second_child_percent: 30, third_child_percent: 60}. After restore, back to defaults.
