...
<objective>
Add bulk team and bulk label modals to the People list view.

Purpose: Enable users to assign teams and manage labels for multiple people at once from the list view.
Output: Two new modals (BulkTeamModal, BulkLabelsModal) integrated into the Actions dropdown.
</objective>
...
Building2 for teams, Tag or Tags for labels
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create BulkTeamModal component</name>
  <files>src/pages/People/PeopleList.jsx</files>
  <action>
Add a new inline modal component following the BulkWorkspaceModal pattern:

```jsx
// Bulk Team Modal Component
function BulkTeamModal({ isOpen, onClose, selectedCount, teams, onSubmit, isLoading }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between p-4 border-b">
          <h2 className="text-lg font-semibold">Set Team</h2>
        ...
        <div className="p-4 space-y-4">
          <p className="text-sm text-gray-600">
            Set current team for {selectedCount} {selectedCount === 1 ? 'person' : 'people'}:
          </p>
          ...
          >
            <X className={`w-5 h-5 ${selectedTeamId === 'clear' ? 'text-primary-600' : 'text-gray-400'}`} />
            <div className="flex-1">
              <div className="text-sm font-medium text-gray-900">Clear team</div>
              <div className="text-xs text-gray-500">Remove current team from selected people</div>
            </div>
            {selectedTeamId === 'clear' && <Check className="w-5 h-5 text-primary-600" />}
          </button>
...
2. Add state: `const [showBulkTeamModal, setShowBulkTeamModal] = useState(false);`
3. Fetch teams for the modal (can reuse teamsData or add separate query)
4. Add the modal to the JSX near BulkWorkspaceModal
5. Add menu item in Actions dropdown (after "Assign to workspace...")
6. Wire up the submit handler to call bulkUpdateMutation with `{ team_id: selectedTeamId }`

Avoid: Creating a separate component file - keep inline per established pattern.
  </action>
  <verify>Modal appears when clicking "Set team..." in Actions dropdown, lists available teams with search</verify>
  <done>BulkTeamModal functional with search, clear option, and team selection</done>
</task>
...
4. Add menu item in Actions dropdown (after "Set team...")
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Bulk team and label management modals in list view</what-built>
  <how-to-verify>
    ...
    5. Verify new menu items: "Set team..." and "Manage labels..."
    6. Test Set Team:
       - Click "Set team..."
       - Search for an team
       - Select one and click Apply
       - Verify Team column updates for selected people
    7. Test Clear Team:
       - Select people with teams
       - Click "Set team...", choose "Clear team"
       - Verify Team column shows "-" after apply
    8. Test Add Labels:
...
- [ ] Team assignment updates list view
