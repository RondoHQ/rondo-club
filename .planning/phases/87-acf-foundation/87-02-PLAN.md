...
must_haves:
  ...
  key_links:
    ...
    - from: "functions.php"
      ...
      via: "initialization in stadion_init for REST requests"
...
Output: REST API endpoints at `/stadion/v1/custom-fields/{post_type}` with full CRUD operations.
</objective>
...
class CustomFields extends WP_REST_Controller {
    protected string $namespace = 'stadion/v1';
...
1. `GET /stadion/v1/custom-fields/{post_type}` - List all custom fields for post type
...
2. `POST /stadion/v1/custom-fields/{post_type}` - Create new custom field
...
3. `GET /stadion/v1/custom-fields/{post_type}/{field_key}` - Get single field
   - Permission: `manage_options`
   - Response: Field object or 404

4. `PUT /stadion/v1/custom-fields/{post_type}/{field_key}` - Update field
   - Permission: `manage_options`
...
5. `DELETE /stadion/v1/custom-fields/{post_type}/{field_key}` - Deactivate field (soft delete)
   - Permission: `manage_options`
...
```bash
curl -s "https://[site]/wp-json/stadion/v1/" | grep custom-fields
```
  </verify>
  <done>
...
3. In `stadion_init()`, add instantiation in the REST API section (where other REST classes are instantiated):
   ```php
...
```bash
wp eval "
\$request = new WP_REST_Request('GET', '/stadion/v1/custom-fields/person');
...
```
  ...
  <done>
functions.php has use statement, backward compatibility alias, and instantiates RESTCustomFields in the REST section of stadion_init().
  </done>
</task>
...
\$request = new WP_REST_Request('POST', '/stadion/v1/custom-fields/person');
...
if (\$field_key) {
    \$request = new WP_REST_Request('PUT', '/stadion/v1/custom-fields/person/' . \$field_key);
...
\$field_key = \$fields[0]['key'] ?? '';
if (\$field_key) {
    \$request = new WP_REST_Request('DELETE', '/stadion/v1/custom-fields/person/' . \$field_key);
    \$response = rest_do_request(\$request);
    echo 'Status: ' . \$response->get_status() . PHP_EOL;
...
\$request2 = new WP_REST_Request('GET', '/stadion/v1/custom-fields/person');
...
2. **Endpoints are registered:**
   ...
   echo isset(\$routes['/stadion/v1/custom-fields/(?P<post_type>person|company)']) ? 'Collection route OK' : 'MISSING';
   echo PHP_EOL;
   echo isset(\$routes['/stadion/v1/custom-fields/(?P<post_type>person|company)/(?P<field_key>[a-z0-9_]+)']) ? 'Item route OK' : 'MISSING';
   "
   ```
