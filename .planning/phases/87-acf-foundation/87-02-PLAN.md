...
2. **Endpoints are registered:**
   ...
   echo isset(\$routes['/stadion/v1/custom-fields/(?P<post_type>person|team)']) ? 'Collection route OK' : 'MISSING';
   echo PHP_EOL;
   echo isset(\$routes['/stadion/v1/custom-fields/(?P<post_type>person|team)/(?P<field_key>[a-z0-9_]+)']) ? 'Item route OK' : 'MISSING';
   "
   ```
