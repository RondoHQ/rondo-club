...
must_haves:
  truths:
    ...
    - "API can list custom field definitions for Organizations"
  ...
  artifacts:
    - path: "includes/class-rest-custom-fields.php"
      ...
      exports: ["Caelis\\REST\\CustomFields"]
      min_lines: 200
  key_links:
    - from: "includes/class-rest-custom-fields.php"
      to: "Caelis\\CustomFields\\Manager"
      via: "instantiation and method calls"
      pattern: "new Manager\\(\\)|\\$this->manager->"
    - from: "functions.php"
      to: "Caelis\\REST\\CustomFields"
      via: "initialization in prm_init for REST requests"
...
Output: REST API endpoints at `/prm/v1/custom-fields/{post_type}` with full CRUD operations.
</objective>
...
<task type="auto">
  ...
  <action>
Create a new REST controller class `Caelis\REST\CustomFields` that exposes the Manager class via HTTP.
...
class CustomFields extends WP_REST_Controller {
    protected string $namespace = 'prm/v1';
...
1. `GET /prm/v1/custom-fields/{post_type}` - List all custom fields for post type
   - Permission: `manage_options` (admin only)
   - Args: `post_type` (string, required, enum: person|company)
   - Optional query param: `include_inactive` (boolean, default false)
   - Response: Array of field objects

2. `POST /prm/v1/custom-fields/{post_type}` - Create new custom field
...
3. `GET /prm/v1/custom-fields/{post_type}/{field_key}` - Get single field
   - Permission: `manage_options`
   - Response: Field object or 404

4. `PUT /prm/v1/custom-fields/{post_type}/{field_key}` - Update field
   - Permission: `manage_options`
...
5. `DELETE /prm/v1/custom-fields/{post_type}/{field_key}` - Deactivate field (soft delete)
   - Permission: `manage_options`
...
public function register_routes() {
    ...
    register_rest_route($this->namespace, '/' . $this->rest_base . '/(?P<post_type>person|company)', [
        [
    ...
    register_rest_route($this->namespace, '/' . $this->rest_base . '/(?P<post_type>person|company)/(?P<field_key>[a-z0-9_]+)', [
        [
            'methods'             => WP_REST_Server::READABLE,
...
```bash
curl -s "https://[site]/wp-json/prm/v1/" | grep custom-fields
```
  </verify>
  <done>
...
3. In `prm_init()`, add instantiation in the REST API section (where other REST classes are instantiated):
   ```php
...
```bash
wp eval "
\$request = new WP_REST_Request('GET', '/prm/v1/custom-fields/person');
...
```
  ...
  <done>
functions.php has use statement, backward compatibility alias, and instantiates RESTCustomFields in the REST section of prm_init().
  </done>
</task>
...
\$request = new WP_REST_Request('POST', '/prm/v1/custom-fields/person');
...
\$fields = (new Caelis\CustomFields\Manager())->get_fields('person');
\$field_key = \$fields[0]['key'] ?? '';
if (\$field_key) {
    \$request = new WP_REST_Request('PUT', '/prm/v1/custom-fields/person/' . \$field_key);
...
\$field_key = \$fields[0]['key'] ?? '';
if (\$field_key) {
    \$request = new WP_REST_Request('DELETE', '/prm/v1/custom-fields/person/' . \$field_key);
    \$response = rest_do_request(\$request);
    echo 'Status: ' . \$response->get_status() . PHP_EOL;
...
\$request2 = new WP_REST_Request('GET', '/prm/v1/custom-fields/person');
...
1. **REST controller class exists and loads:**
   ```bash
   wp eval "echo class_exists('Caelis\\REST\\CustomFields') ? 'OK' : 'FAIL';"
   ```
...
2. **Endpoints are registered:**
   ...
   echo isset(\$routes['/prm/v1/custom-fields/(?P<post_type>person|company)']) ? 'Collection route OK' : 'MISSING';
   echo PHP_EOL;
   echo isset(\$routes['/prm/v1/custom-fields/(?P<post_type>person|company)/(?P<field_key>[a-z0-9_]+)']) ? 'Item route OK' : 'MISSING';
   "
   ```
