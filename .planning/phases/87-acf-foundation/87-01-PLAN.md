...
must_haves:
  truths:
    ...
    - "PHP can create a custom field definition for Organizations that persists in database"
  ...
  artifacts:
    - path: "includes/class-custom-fields-manager.php"
      ...
      exports: ["Caelis\\CustomFields\\Manager"]
      min_lines: 200
  key_links:
    ...
    - from: "functions.php"
      ...
      pattern: "use Caelis\\\\CustomFields\\\\Manager"
---

<objective>
Create the PHP CustomFields Manager class that provides programmatic CRUD operations for ACF custom field definitions.

Purpose: This is the foundation for the entire Custom Fields feature. It provides the backend infrastructure that the Settings UI and Detail View will use to manage user-defined custom fields for People and Organizations.
...
<task type="auto">
  ...
  <action>
Create a new PHP class `Caelis\CustomFields\Manager` in `includes/class-custom-fields-manager.php` that provides:
...
class Manager {
    ...
    const SUPPORTED_POST_TYPES = ['person', 'company'];
...
- Use error codes like: 'invalid_post_type', 'missing_required', 'field_not_found'
  ...
  <verify>
Run `wp eval "require_once get_template_directory() . '/vendor/autoload.php'; \$m = new Caelis\CustomFields\Manager(); var_dump(\$m->ensure_field_group('person'));"` to verify class loads and can create field group.
  </verify>
  <done>
...
1. Add use statement at top with other Caelis imports:
   ```php
...
Note: The Manager class is stateless and doesn't need to be instantiated in prm_init(). It will be instantiated by the REST API class when needed (Plan 02). The alias ensures backward compatibility if any code references the old class name pattern.
  </action>
  <verify>
Run `npm run lint` in caelis directory (for PHP linting via composer).
Run `wp eval "class_exists('Caelis\CustomFields\Manager') ? print('Class exists') : print('Class missing');"` to verify autoloading works.
  </verify>
  <done>
...
class ManagerTest extends \Codeception\TestCase\WPTestCase {
    ...
    private function cleanup_test_fields(): void {
        ...
        foreach (['person', 'company'] as $post_type) {
...
2. `test_ensure_field_group_creates_for_company()` - Creates group for company
...
1. **Manager class loads correctly:**
   ```bash
   wp eval "require_once get_template_directory() . '/vendor/autoload.php'; echo class_exists('Caelis\CustomFields\Manager') ? 'OK' : 'FAIL';"
   ```

2. **Can create field group:**
   ```bash
   wp eval "
   \$m = new Caelis\CustomFields\Manager();
...
<success_criteria>
1. Manager class exists at includes/class-custom-fields-manager.php with namespace Caelis\CustomFields\Manager
