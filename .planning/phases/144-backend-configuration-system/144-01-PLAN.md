...
must_haves:
  truths:
    - "GET /rondo/v1/config returns club_name, accent_color, and freescout_url with sensible defaults"
    - "POST /rondo/v1/config allows admin to update any subset of settings"
    - "POST /rondo/v1/config returns 403 for non-admin authenticated users"
...
<objective>
Implement the backend configuration system for club-wide settings (name, accent color, FreeScout URL) using WordPress Options API, exposed via REST API at `/rondo/v1/config`.
...
@functions.php (stadion_get_js_config at line 542, stadion_theme_document_title_parts at line 455, rondo_init at line 310, use statements at line 28)
</context>
...
Note: The ClubConfig class does NOT need to be instantiated in `rondo_init()` because it has no constructor hooks. It is used on-demand by the REST API callbacks and stadion_get_js_config(). This follows the same pattern as MembershipFees (not instantiated in init, created where needed).
  </action>
  <verify>
...
5. Reset test data:
  ...
  <done>
REST endpoint at `/rondo/v1/config` responds to GET (returns defaults) and POST (admin-only, partial updates). Page title reads from club config with "Stadion" fallback. stadionConfig includes clubName, accentColor, freescoutUrl.
  </done>
</task>
...
<verification>
1. **CFG-01**: `GET /rondo/v1/config` returns `club_name` field; `POST` allows setting it
2. **CFG-02**: `GET /rondo/v1/config` returns `accent_color` field; `POST` allows setting hex value
3. **CFG-03**: `GET /rondo/v1/config` returns `freescout_url` field; `POST` allows setting it
...
- REST endpoint /rondo/v1/config handles GET (all users) and POST (admin only)
