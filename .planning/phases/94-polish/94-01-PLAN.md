...
```
  ...
  <verify>
curl -X PUT localhost/wp-json/prm/v1/custom-fields/person/order with admin auth returns 200;
...
2. In functions.php, add to prm_init() function after the CustomFields REST API initialization (around line where other classes are instantiated):

```php
// Initialize custom field validation.
new \Caelis\CustomFields\Validation();
```
  </action>
...
2. Reorder endpoint: PUT /prm/v1/custom-fields/person/order with array of keys returns success
