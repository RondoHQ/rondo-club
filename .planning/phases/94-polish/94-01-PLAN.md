...
must_haves:
  ...
  artifacts:
    ...
    - path: "includes/class-rest-custom-fields.php"
      provides: "PUT /stadion/v1/custom-fields/{post_type}/order endpoint"
      contains: "order"
  ...
  key_links:
    - from: "src/api/client.js"
      to: "/stadion/v1/custom-fields/{post_type}/order"
      via: "PUT request"
...
```
  ...
  <verify>
curl -X PUT localhost/wp-json/stadion/v1/custom-fields/person/order with admin auth returns 200;
...
2. In functions.php, add to stadion_init() function after the CustomFields REST API initialization (around line where other classes are instantiated):
...
2. Reorder endpoint: PUT /stadion/v1/custom-fields/person/order with array of keys returns success
