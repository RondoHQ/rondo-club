...
- PRM_Visibility helper for visibility updates
...
<task type="auto">
  ...
  <action>
Add bulk update endpoint to PRM_REST_Companies class, following the pattern from class-rest-people.php:

1. In register_routes(), add route registration for POST `/prm/v1/companies/bulk-update`:
...
3. Add bulk_update_companies() callback method:
   ...
   - If visibility provided: use PRM_Visibility::set_visibility()
...
Note: Companies do NOT have organization_id update (unlike People) - they ARE organizations. Only visibility, workspaces, and labels.
  ...
  <verify>
curl -X POST "http://localhost/wp-json/prm/v1/companies/bulk-update" \
  -H "X-WP-Nonce: {nonce}" \
