...
<task type="auto">
  ...
  <action>
Add bulk update endpoint to STADION_REST_Teams class, following the pattern from class-rest-people.php:

1. In register_routes(), add route registration for POST `/stadion/v1/teams/bulk-update`:
...
Note: Teams do NOT have team_id update (unlike People) - they ARE teams. Only visibility, workspaces, and labels.
  ...
  <verify>
curl -X POST "http://localhost/wp-json/stadion/v1/teams/bulk-update" \
  -H "X-WP-Nonce: {nonce}" \
