...
must_haves:
  truths:
    - "When same field is modified in both Caelis and Google, conflict is detected"
    - "Caelis value is kept, Google value is discarded"
...
Purpose: When a contact is modified in both Caelis and Google between syncs, detect which specific fields conflict, automatically resolve by keeping Caelis values (Caelis is source of truth), and log the resolution as an activity entry for audit.
...
1. Add new private method `get_field_snapshot(int $post_id): array` that extracts current Caelis field values:
   ...
   - organization: current job company name from work_history (first entry with is_current=true or first entry)
...
3. Add new private method `detect_field_conflicts(int $post_id, object $google_person): array`:
   ...
   - Get current Caelis values via `get_field_snapshot()`
   - Extract Google values from person object (reuse existing extraction logic)
   - For each field, check: `$google_value !== $snapshot_value && $caelis_value !== $snapshot_value`
     - If true: both systems changed this field = conflict
     - Add to conflicts array: `['field' => $field_name, 'google_value' => $gv, 'caelis_value' => $cv, 'kept_value' => $cv]`
...
4. Add new private method `log_conflict_resolution(int $post_id, array $conflicts): void`:
   ...
   - Format content as bullet list:
     ```
     Sync conflict resolved (Caelis wins):
...
5. Modify `process_contact()` method:
   ...
   - Note: Do NOT skip field imports - Caelis wins means we keep Caelis values, but fill-gaps-only import logic already preserves existing Caelis values
...
Add use statement at top: `use Caelis\Collaboration\CommentTypes;`

IMPORTANT: The fill-gaps-only import logic already preserves Caelis values (it only fills empty fields). Conflict detection is about LOGGING when Google had a different value, not about changing resolution behavior.
  </action>
  <verify>
...
- `detect_field_conflicts()` method comparing Google vs Caelis vs snapshot
...
1. Add new public static method `update_field_snapshot(int $post_id): void`:
   ...
   - Get field values (same structure as import class):
     ...
     - organization: current job company name
...
3. Verify PHP syntax on production:
   ```bash
   ssh u25-eninwxjgiulh@c1130624.sgvps.net -p 18765 "php -l ~/www/cael.is/public_html/wp-content/themes/caelis/includes/class-google-contacts-api-import.php && php -l ~/www/cael.is/public_html/wp-content/themes/caelis/includes/class-google-contacts-sync.php"
   ```
  </action>
  <verify>
...
1. Code review:
   ...
   - `detect_field_conflicts()` compares all three values (Google, Caelis, snapshot)
...
<success_criteria>
- [ ] `detect_field_conflicts()` method exists and compares Google vs Caelis vs snapshot
