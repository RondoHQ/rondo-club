...
must_haves:
  truths:
    ...
    - "Google API errors do not block Stadion deletion"
...
<objective>
Add Stadion-to-Google deletion propagation via WordPress hook.

Purpose: When a user permanently deletes a contact in Stadion, automatically delete the corresponding contact from Google Contacts. This maintains sync integrity with Stadion as the source of truth.
...
3. Add new private method `delete_google_contact(string $resource_name, int $user_id): bool`:
   ...
   private function delete_google_contact( string $resource_name, int $user_id ): bool {
       try {
           ...
           $client = \Stadion\Calendar\GoogleOAuth::get_contacts_client( false, false );
           if ( ! $client ) {
               return false;
           }
...
3. Verify PHP syntax on production:
   ```bash
   ssh u25-eninwxjgiulh@c1130624.sgvps.net -p 18765 "php -l ~/www/cael.is/public_html/wp-content/themes/rondo-club/includes/class-google-contacts-export.php"
   ```
  </action>
  <verify>
PHP file passes syntax check on production. Cache is cleared. Production site loads without errors.
  ...
  <done>
Deletion propagation code deployed to production. Permanently deleting a linked contact in Stadion will now delete it from Google Contacts.
  </done>
</task>
...
2. Behavior verification:
   ...
   - Google API errors are logged but don't prevent Stadion deletion
</verification>
