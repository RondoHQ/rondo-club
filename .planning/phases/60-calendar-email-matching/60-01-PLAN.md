...
# - RONDO_Calendar_Matcher::get_email_lookup() already builds lookup from ALL emails in contact_info repeater
# - RONDO_Calendar_Matcher::invalidate_cache() exists but is never called
...
# - RONDO_Auto_Title already hooks acf/save_post for person post type
...
<task type="auto">
  <name>Task 1: Add re-matching method to RONDO_Calendar_Matcher</name>
...
4. Calls `rematch_events_for_user($user_id)` to update all event matches
  ...
  <done>RONDO_Calendar_Matcher has rematch_events_for_user() and on_person_saved() methods</done>
</task>
...
<task type="auto">
  ...
  <action>
Add a hook in RONDO_Auto_Title constructor to trigger calendar re-matching when a person is saved:
...
2. Add new method `trigger_calendar_rematch($post_id)`:
   ...
   public function trigger_calendar_rematch($post_id) {
       ...
       RONDO_Calendar_Matcher::on_person_saved($post_id);
   }
   ```
...
- User won't notice the extra processing time
  ...
  <done>Person saves trigger RONDO_Calendar_Matcher::on_person_saved() via acf/save_post hook</done>
</task>
...
if (defined('WP_CLI') && WP_CLI) {
    WP_CLI::add_command('prm calendar rematch', function($args, $assoc_args) {
        ...
        $count = RONDO_Calendar_Matcher::rematch_events_for_user($user_id);
...
- [ ] `rematch_events_for_user()` method exists in RONDO_Calendar_Matcher
- [ ] `on_person_saved()` method exists in RONDO_Calendar_Matcher
- [ ] `trigger_calendar_rematch()` method exists in RONDO_Auto_Title
- [ ] acf/save_post hook registered at priority 25 in RONDO_Auto_Title constructor
