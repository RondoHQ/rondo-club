...
| # | Task | Status | Commit |
|---|------|--------|--------|
| 1 | Add re-matching method to PRM_Calendar_Matcher | Done | a4ee051 |
...
- When a person is saved, triggers `PRM_Calendar_Matcher::on_person_saved()` to re-match events

### includes/class-wp-cli.php
- Added `rematch` subcommand to `PRM_Calendar_CLI_Command` class
...
- [x] `rematch_events_for_user()` method exists in PRM_Calendar_Matcher
- [x] `on_person_saved()` method exists in PRM_Calendar_Matcher
- [x] `trigger_calendar_rematch()` method exists in PRM_Auto_Title
- [x] acf/save_post hook registered at priority 25 in PRM_Auto_Title constructor
