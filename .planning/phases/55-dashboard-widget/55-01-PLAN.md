...
<task type="auto">
  ...
  <action>
Add new REST endpoint `GET /stadion/v1/calendar/today-meetings` that returns today's calendar events with matched attendees.

In `register_routes()` add:
```php
register_rest_route('stadion/v1', '/calendar/today-meetings', [
...
The `has_connections` flag helps frontend show appropriate message when user has no calendar connections set up.
  ...
  <verify>
Test endpoint: `curl -X GET 'http://localhost/wp-json/stadion/v1/calendar/today-meetings' -H 'X-WP-Nonce: [nonce]'`
