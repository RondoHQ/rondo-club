...
- Key: Generate once and store in wp-config.php (STADION_ENCRYPTION_KEY)
...
<task type="auto">
  ...
  <action>
Add two protected methods to STADION_REST_Slack class:

1. `encrypt_token($token)`:
   - Check STADION_ENCRYPTION_KEY is defined (return base64 fallback if not)
   - Generate 24-byte random nonce: `random_bytes(SODIUM_CRYPTO_SECRETBOX_NONCEBYTES)`
   - Encrypt: `sodium_crypto_secretbox($token, $nonce, STADION_ENCRYPTION_KEY)`
   - Return: base64_encode($nonce . $ciphertext)

2. `decrypt_token($encrypted)`:
   - Check STADION_ENCRYPTION_KEY is defined (return base64_decode fallback if not)
   ...
   - Decrypt: `sodium_crypto_secretbox_open($ciphertext, $nonce, STADION_ENCRYPTION_KEY)`
...
- [ ] Methods handle missing STADION_ENCRYPTION_KEY gracefully (fallback to base64)
</verification>
...
- Graceful degradation if STADION_ENCRYPTION_KEY not set
- PHP syntax valid
</success_criteria>
