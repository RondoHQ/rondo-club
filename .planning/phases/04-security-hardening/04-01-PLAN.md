...
- Key: Generate once and store in wp-config.php (RONDO_ENCRYPTION_KEY)
...
<task type="auto">
  ...
  <action>
Add two protected methods to RONDO_REST_Slack class:

1. `encrypt_token($token)`:
   - Check RONDO_ENCRYPTION_KEY is defined (return base64 fallback if not)
   - Generate 24-byte random nonce: `random_bytes(SODIUM_CRYPTO_SECRETBOX_NONCEBYTES)`
   - Encrypt: `sodium_crypto_secretbox($token, $nonce, RONDO_ENCRYPTION_KEY)`
   - Return: base64_encode($nonce . $ciphertext)

2. `decrypt_token($encrypted)`:
   - Check RONDO_ENCRYPTION_KEY is defined (return base64_decode fallback if not)
   ...
   - Decrypt: `sodium_crypto_secretbox_open($ciphertext, $nonce, RONDO_ENCRYPTION_KEY)`
...
- [ ] Methods handle missing RONDO_ENCRYPTION_KEY gracefully (fallback to base64)
</verification>
...
- Graceful degradation if RONDO_ENCRYPTION_KEY not set
- PHP syntax valid
</success_criteria>
