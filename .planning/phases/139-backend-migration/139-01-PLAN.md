...
1. Create a new command class `STADION_Tasks_CLI_Command` near the end of the file (before the final WP_CLI::add_command calls):
...
- `grep -n "STADION_Tasks_CLI_Command" includes/class-wp-cli.php` should show the class
...
- Deploy to production and run: `wp stadion tasks verify-ownership --verify` to test
  ...
  <done>
- STADION_Tasks_CLI_Command class exists with verify_ownership method
...
4. **API verification (test with different users):**
   - User A creates a task, User B should NOT see it in GET /stadion/v1/todos
