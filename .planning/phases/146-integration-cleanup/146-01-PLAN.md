...
<task type="auto">
  <name>Task 3: Final verification scan and build</name>
  <files>None (verification only)</files>
  <action>
Run comprehensive verification scan to confirm zero AWC/svawc references remain in source code.

1. Run the grep scan with proper exclusions:
```bash
...
3. Run `npm run lint` to verify no code quality issues
...
- If in .planning/: ignore (context documents)
  ...
  <verify>
1. grep scan returns zero matches
2. `npm run build` completes successfully
3. `npm run lint` passes with no new errors
  </verify>
  <done>Theme verified installable by any club - zero AWC/svawc references in source</done>
</task>
...
4. grep scan returns zero awc/svawc matches in source files
...
3. Zero awc/svawc/AWC references in source code (verified by grep scan)
4. AGENTS.md uses `$DEPLOY_REMOTE_THEME_PATH` instead of hardcoded path
5. PERFORMANCE-FINDINGS.md has generic title
6. Build and lint pass with no errors
</success_criteria>
