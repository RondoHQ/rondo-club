...
Find where other classes are initialized (likely in stadion_init() or a later hook like 'init') and add:
```php
...
Test endpoint with curl (after deploying): `curl -X POST https://cael.is/wp-json/stadion/v1/google-contacts/export/123 -H "X-WP-Nonce: ..."` (will need auth)
  </verify>
  <done>
...
- REST endpoint `/stadion/v1/google-contacts/export/{id}` triggers single contact export
