...
must_haves:
  ...
  key_links:
    ...
    - from: "includes/class-google-contacts-export.php"
      ...
      pattern: "wp_schedule_single_event.*caelis_google_contact_export"
---
...
Purpose: Contacts should automatically sync to Google when saved in Caelis, without blocking the save operation.
...
Find where other classes are initialized (likely in prm_init() or a later hook like 'init') and add:
```php
\Caelis\Export\GoogleContactsExport::init();
```
...
Test endpoint with curl (after deploying): `curl -X POST https://cael.is/wp-json/prm/v1/google-contacts/export/123 -H "X-WP-Nonce: ..."` (will need auth)
  </verify>
  <done>
...
2. Save a contact and verify cron job is scheduled: `wp cron event list` should show `caelis_google_contact_export`
...
- REST endpoint `/prm/v1/google-contacts/export/{id}` triggers single contact export
