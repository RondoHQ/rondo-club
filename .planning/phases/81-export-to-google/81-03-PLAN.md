...
must_haves:
  ...
  key_links:
    - from: "src/pages/SettingsPage.jsx"
      to: "/prm/v1/google-contacts/bulk-export"
...
<objective>
Add bulk export functionality that allows users to export all their existing unlinked Caelis contacts to Google Contacts in a single operation.

Purpose: Users may have existing contacts in Caelis that were created before connecting Google Contacts. This feature lets them push those contacts to Google without editing each one individually.
...
public function bulk_export_unlinked( ?callable $progress_callback = null ): array {
    ...
    foreach ( $post_ids as $index => $post_id ) {
        ...
        try {
            ...
            $stats['failed']++;
            $stats['errors'][] = sprintf(
                __( 'Error exporting contact ID %d: %s', 'caelis' ),
                $post_id,
                $e->getMessage()
            );
        }
...
public function bulk_export( $request ) {
    ...
    try {
        $exporter = new \Caelis\Export\GoogleContactsExport( $user_id );
        ...
        return rest_ensure_response( [
            ...
            'stats'   => $stats,
            'message' => sprintf(
                __( 'Bulk export complete: %d exported, %d skipped, %d failed out of %d total.', 'caelis' ),
                $stats['exported'],
                $stats['skipped'],
                $stats['failed'],
                $stats['total']
...
useEffect(() => {
  if (contactsConnection?.access_mode === 'readwrite') {
    apiClient.get('/prm/v1/google-contacts/unlinked-count')
      .then(response => {
...
const handleBulkExport = async () => {
  ...
  try {
    const response = await apiClient.post('/prm/v1/google-contacts/bulk-export');
...
3. REST endpoints work:
   - GET `/prm/v1/google-contacts/unlinked-count` returns count
   - POST `/prm/v1/google-contacts/bulk-export` exports all unlinked contacts
