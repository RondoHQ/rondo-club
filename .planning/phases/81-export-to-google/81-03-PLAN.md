...
must_haves:
  ...
  key_links:
    - from: "src/pages/SettingsPage.jsx"
      to: "/stadion/v1/google-contacts/bulk-export"
...
useEffect(() => {
  if (contactsConnection?.access_mode === 'readwrite') {
    apiClient.get('/stadion/v1/google-contacts/unlinked-count')
      .then(response => {
...
const handleBulkExport = async () => {
  ...
  try {
    const response = await apiClient.post('/stadion/v1/google-contacts/bulk-export');
...
3. REST endpoints work:
   - GET `/stadion/v1/google-contacts/unlinked-count` returns count
   - POST `/stadion/v1/google-contacts/bulk-export` exports all unlinked contacts
