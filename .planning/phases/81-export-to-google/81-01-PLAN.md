...
must_haves:
  truths:
    - "Caelis person fields map correctly to Google Person object format"
...
Purpose: This is the reverse of the import class - maps Caelis ACF fields to Google Person objects and handles CRUD operations against the People API.
...
<task type="auto">
  ...
  <action>
Create a new class `Caelis\Export\GoogleContactsExport` that mirrors the import class structure:

**Class structure:**
- Namespace: `Caelis\Export`
- Uses: `Caelis\Calendar\GoogleOAuth`, `Caelis\Contacts\GoogleContactsConnection`, `Google\Service\PeopleService`
...
13. `build_organizations(int $post_id): array` - Map work_history:
    - Get `work_history` repeater
    - For each entry, get company title and job_title
    - Create `Organization` objects with name, title, current, startDate, endDate
...
<task type="auto">
  ...
  <action>
Add the new export class to the list of included files in `prm_init()` function.
...
Verify the require_once line is in the correct location within prm_init().
  </verify>
  <done>
