...
<task type="auto">
  <name>Task 1: Create STADION_Google_Calendar_Provider class with sync logic</name>
...
**sync() method:**
1. Get valid access token via STADION_Google_OAuth::get_access_token($connection)
...
- Class loads without errors in functions.php
  ...
  <done>
- STADION_Google_Calendar_Provider class created with all methods
...
<task type="auto">
  ...
  <action>
Replace the stub implementations in STADION_REST_Calendar:

**trigger_sync() method:**
1. Get connection_id from request params
2. Get user_id from get_current_user_id()
3. Get connection via STADION_Calendar_Connections::get_connection()
...
7. Try sync:
   ...
   try {
       ...
       STADION_Calendar_Connections::update_connection($user_id, $connection_id, [
...
- With valid Google connection: POST /stadion/v1/calendar/connections/{id}/sync returns 200 with sync results
- GET /stadion/v1/calendar/events returns array of events (empty if no sync yet)
  </verify>
  <done>
- trigger_sync endpoint calls STADION_Google_Calendar_Provider::sync()
...
- [ ] STADION_Google_Calendar_Provider class loads without errors
...
- STADION_Google_Calendar_Provider class syncs events from Google Calendar
