...
<task type="auto">
  <name>Task 1: Create PRM_Google_Calendar_Provider class with sync logic</name>
...
**sync() method:**
1. Get valid access token via PRM_Google_OAuth::get_access_token($connection)
...
- Class loads without errors in functions.php
  ...
  <done>
- PRM_Google_Calendar_Provider class created with all methods
...
<task type="auto">
  ...
  <action>
Replace the stub implementations in PRM_REST_Calendar:

**trigger_sync() method:**
1. Get connection_id from request params
2. Get user_id from get_current_user_id()
3. Get connection via PRM_Calendar_Connections::get_connection()
...
- With valid Google connection: POST /prm/v1/calendar/connections/{id}/sync returns 200 with sync results
- GET /prm/v1/calendar/events returns array of events (empty if no sync yet)
  </verify>
  <done>
- trigger_sync endpoint calls PRM_Google_Calendar_Provider::sync()
...
- [ ] PRM_Google_Calendar_Provider class loads without errors
