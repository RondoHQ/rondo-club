...
@.planning/phases/01-rest-api-infrastructure/01-01-SUMMARY.md (established STADION_REST_Base with format methods)

**Source files:**
@includes/class-rest-base.php (format_person_summary, format_company_summary, format_date)
...
<task type="auto">
  <name>Task 1: Add sanitization helper methods to STADION_REST_Base</name>
  <files>includes/class-rest-base.php</files>
  <action>
Add three protected helper methods to STADION_REST_Base for consistent sanitization:
...
Verify methods exist: `grep -E "function sanitize_(text|rich_content|url)" includes/class-rest-base.php`
  ...
  <done>
- Three new protected sanitization methods added to STADION_REST_Base
- format_person_summary, format_company_summary, format_date use sanitization helpers
...
<task type="auto">
  ...
  <action>
Review and sanitize remaining response fields in STADION_REST_API that don't use the base class formatters.
...
- Uses format_person_summary and format_company_summary - already handled by Task 1
...
- [ ] Manual test: Visit https://cael.is/ and verify:
  - Dashboard loads with people/companies displayed
