...
must_haves:
  ...
  artifacts:
    - path: "includes/class-post-types.php"
      provides: "stadion_feedback CPT registration"
  ...
  key_links:
    - from: "includes/class-post-types.php"
      ...
      pattern: "register_post_type.*stadion_feedback"
    - from: "acf-json/group_feedback_fields.json"
      to: "stadion_feedback"
      via: "ACF location rule"
      pattern: "\"value\":\\s*\"stadion_feedback\""
---

<objective>
Register the `stadion_feedback` custom post type and create the ACF field group for storing feedback metadata.
...
Output:
- `stadion_feedback` CPT registered and visible in WordPress admin
...
<task type="auto">
  <name>Task 1: Register stadion_feedback custom post type</name>
...
Do NOT create custom post statuses (unlike stadion_todo). The status will be managed via ACF select field for simplicity, as noted in the research document.
  </action>
  <verify>
...
3. `wp-json/wp/v2/feedback` endpoint exists (returns empty array or 401 if not logged in)
  ...
  <done>
- stadion_feedback CPT registered
...
Location rule: post_type == stadion_feedback
...
<success_criteria>
- [ ] stadion_feedback CPT registered and visible in admin menu
