...
Output: Working REST endpoint `/prm/v1/people/bulk-update` and `useBulkUpdatePeople` hook.
</objective>
...
- `PRM_Visibility::set_visibility()` sets visibility on a single post
...
<task type="auto">
  ...
  <action>
Add a POST endpoint `/prm/v1/people/bulk-update` in the `register_routes()` method.
...
4. Callback method `bulk_update_people`:
   ...
   - If `visibility` provided: call `PRM_Visibility::set_visibility($id, $visibility)`
...
Test with curl:
```bash
curl -X POST "https://cael.is/wp-json/prm/v1/people/bulk-update" \
...
1. In `src/api/client.js`, add to `prmApi` object:
   ...
   bulkUpdatePeople: (ids, updates) =>
     api.post('/prm/v1/people/bulk-update', { ids, updates }),
   ```
...
- [ ] Bulk update endpoint is registered (check `/wp-json/prm/v1` route listing)
...
- Visibility updates use PRM_Visibility helper
