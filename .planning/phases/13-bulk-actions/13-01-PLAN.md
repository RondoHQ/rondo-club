...
Output: Working REST endpoint `/rondo/v1/people/bulk-update` and `useBulkUpdatePeople` hook.
</objective>
...
- `RONDO_Visibility::set_visibility()` sets visibility on a single post
...
<task type="auto">
  ...
  <action>
Add a POST endpoint `/rondo/v1/people/bulk-update` in the `register_routes()` method.
...
4. Callback method `bulk_update_people`:
   ...
   - If `visibility` provided: call `RONDO_Visibility::set_visibility($id, $visibility)`
...
Test with curl:
```bash
curl -X POST "https://cael.is/wp-json/rondo/v1/people/bulk-update" \
...
1. In `src/api/client.js`, add to `prmApi` object:
   ...
   bulkUpdatePeople: (ids, updates) =>
     api.post('/rondo/v1/people/bulk-update', { ids, updates }),
   ```
...
- [ ] Bulk update endpoint is registered (check `/wp-json/rondo/v1` route listing)
...
- Visibility updates use RONDO_Visibility helper
