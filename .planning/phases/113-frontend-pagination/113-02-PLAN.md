...
<task type="auto">
  <name>Task 3: Integrate pagination into PeopleList</name>
  <files>src/pages/People/PeopleList.jsx</files>
  <action>
Major refactor of PeopleList.jsx to use server-side pagination. This is the core work of the phase.
...
The birth years dropdown currently derives from client-side data. For now, keep it but acknowledge it may not be complete (shows years from current page only). Alternative: remove the dropdown or fetch all years separately. For simplicity, remove dependency on people data:
```javascript
...
**8. Update filter UI for label IDs:**
...
**13. Add loading indicator for page navigation:**
...
**14. Add Pagination component at bottom of list:**
...
**16. Update team fetching:**
The teamIds useMemo currently uses `filteredAndSortedPeople`. Update to use `people`:
```javascript
...
**17. Handle special sort fields:**
For 'organization' and 'labels' sorting, the backend doesn't support these directly. Options:
- Fall back to first_name sorting (already in step 4)
- Keep client-side sorting for these fields only

For now, use the fallback approach - sort by first_name when organization or labels is selected.
Update the sort dropdown to indicate this limitation or remove these options for server-side mode.
...
- For team display, we need to fetch from work_history which is NOT in filtered response

Since filtered endpoint doesn't return ACF data, team display won't work. We have two options:
A) Enhance backend to include current team in response
B) Accept team column won't show data in paginated view

For this plan, choose option B - team column will show '-' for all rows. Document this as a known limitation to address in a future phase (requires backend enhancement to include current_team_id in filtered response).
  </action>
  <verify>
```bash
