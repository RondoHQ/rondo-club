...
<objective>
Add custom ACF field sorting to the `/rondo/v1/people/filtered` endpoint.
...
curl -s "https://[production-url]/wp-json/rondo/v1/people/filtered?orderby=custom_nonexistent" | jq '.code'
...
curl -s "https://[production-url]/wp-json/rondo/v1/people/filtered?orderby=custom_vog_status&order=asc&per_page=5" | jq '.people[].first_name'

# Test with desc order
curl -s "https://[production-url]/wp-json/rondo/v1/people/filtered?orderby=custom_vog_status&order=desc&per_page=5" | jq '.people[].first_name'

# Verify pagination still works with custom sort
curl -s "https://[production-url]/wp-json/rondo/v1/people/filtered?orderby=custom_vog_status&page=2&per_page=10" | jq '.page'
```
  </verify>
  <done>
...
fetch('/wp-json/rondo/v1/people/filtered?orderby=custom_[field_name]&per_page=10')
  .then(r => r.json())
