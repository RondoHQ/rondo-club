...
Purpose: Non-admin users need field metadata (label, type, choices) to properly display custom fields on detail pages. Currently the `/stadion/v1/custom-fields/{post_type}` endpoint requires `manage_options`, blocking non-admins.
...
1. Register new route: `GET /stadion/v1/custom-fields/{post_type}/metadata`
...
fetch('/wp-json/stadion/v1/custom-fields/person/metadata')
  .then(r => r.json())
  .then(console.log)
```
...
- GET `/stadion/v1/custom-fields/person` should still require admin
- POST, PUT, DELETE should still require admin
  </verify>
  <done>
- `/stadion/v1/custom-fields/{post_type}/metadata` endpoint returns field definitions to any logged-in user
