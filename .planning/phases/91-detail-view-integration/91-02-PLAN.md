...
must_haves:
  truths:
    ...
    - "Team detail view shows Custom Fields section when custom fields are defined"
  ...
  key_links:
    - from: "src/components/CustomFieldsSection.jsx"
      to: "/rondo/v1/custom-fields/{post_type}/metadata"
...
Purpose: Users need to view and edit custom field values on Person and Team detail pages. This completes the DISP-01, DISP-02, DISP-03, DISP-04 requirements.
...
- `acfData`: object - the `person.acf` or `team.acf` object
...
4. **Field display rendering** - create a `renderFieldValue` function that handles all 14 types:
   ...
   - **relationship**: Links to related person(s)/team(s)
...
- Link to `/people/{id}` or `/teams/{id}` based on the related post's type
...
3. **Field input rendering** - create `renderFieldInput` for all 14 types:
   ...
   **relationship:**
   Use autocomplete search for people/teams:
   ```jsx
   <Controller
     name={field.name}
     control={control}
     render={({ field: { value, onChange } }) => (
       <RelationshipInput
         value={value}
         onChange={onChange}
         postTypes={fieldDef.post_type || ['person', 'team']}
         max={fieldDef.max || 0}
       />
     )}
   />
...
4. **Form submission**:
   ...
   - onSubmit should merge into existing ACF and save via wpApi.updatePerson/Team
...
<task type="auto">
  <name>Task 3: Integrate CustomFieldsSection into PersonDetail and TeamDetail</name>
...
2. Add mutation for updating team if not exists - check if `updateTeam` mutation exists, if not add it similar to PersonDetail pattern

3. Add section after the main team info, in appropriate location:
   ```jsx
   <CustomFieldsSection
     postType="team"
     postId={parseInt(id)}
...
Note: TeamDetail structure is different from PersonDetail - check where sections are placed and add Custom Fields in a logical location (after the main team info cards).
  </action>
  <verify>
- PersonDetail shows Custom Fields section when fields are defined for person
- TeamDetail shows Custom Fields section when fields are defined for team
...
- CustomFieldsSection integrated into TeamDetail.jsx
...
2. Team detail view shows "Custom fields" section with all defined team fields
...
- DISP-02: Custom Fields section on Team detail view - DONE
