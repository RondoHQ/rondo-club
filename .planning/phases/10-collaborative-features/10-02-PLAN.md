...
Output: MentionInput component, PRM_Mentions class for parsing/rendering, workspace member search API.
</objective>
...
<task type="auto">
  ...
  public function search_workspace_members($request) {
      ...
      foreach ($workspace_ids as $workspace_id) {
          $members = PRM_Workspace_Members::get_workspace_members($workspace_id);
...
<task type="auto">
  <name>Task 3: Create PRM_Mentions class for parsing and rendering</name>
  ...
  2. In functions.php, load the class in prm_init():
  ```php
  require_once get_template_directory() . '/includes/class-mentions.php';
  ```

  3. In class-comment-types.php create_note() method, call PRM_Mentions::save_mentions() after creating the comment, and trigger action hook:
  ```php
  ...
  ```php
  'content' => PRM_Mentions::render_mentions(wp_kses_post($comment->comment_content)),
  ```
  ...
  - Check class-comment-types.php calls PRM_Mentions::save_mentions()
  </verify>
  <done>PRM_Mentions class parses and stores mentions, hook fired on mention save</done>
</task>
...
- [ ] /prm/v1/workspaces/members/search returns matching users
- [ ] PRM_Mentions::parse_mention_ids() extracts user IDs from markup
...
- prm_user_mentioned action fires when mentions saved
