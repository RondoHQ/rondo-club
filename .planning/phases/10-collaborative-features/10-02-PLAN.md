...
Output: MentionInput component, RONDO_Mentions class for parsing/rendering, workspace member search API.
</objective>
...
<task type="auto">
  ...
  ```php
  register_rest_route('rondo/v1', '/workspaces/members/search', [
      'methods' => 'GET',
  ...
  public function search_workspace_members($request) {
      ...
      foreach ($workspace_ids as $workspace_id) {
          $members = RONDO_Workspace_Members::get_workspace_members($workspace_id);
  ...
  searchWorkspaceMembers: async (workspaceIds, query) => {
    const { data } = await api.get('/rondo/v1/workspaces/members/search', {
...
<task type="auto">
  <name>Task 3: Create RONDO_Mentions class for parsing and rendering</name>
  ...
  2. In functions.php, load the class in rondo_init():
  ```php
  require_once get_template_directory() . '/includes/class-mentions.php';
  ```

  3. In class-comment-types.php create_note() method, call RONDO_Mentions::save_mentions() after creating the comment, and trigger action hook:
  ```php
  ...
  ```php
  'content' => RONDO_Mentions::render_mentions(wp_kses_post($comment->comment_content)),
  ```
  ...
  - Check class-comment-types.php calls RONDO_Mentions::save_mentions()
  </verify>
  <done>RONDO_Mentions class parses and stores mentions, hook fired on mention save</done>
</task>
...
- [ ] /rondo/v1/workspaces/members/search returns matching users
- [ ] RONDO_Mentions::parse_mention_ids() extracts user IDs from markup
