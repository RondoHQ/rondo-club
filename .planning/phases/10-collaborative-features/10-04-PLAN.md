...
**From RESEARCH.md:**
- Extend existing RONDO_ICal_Feed with workspace scope
...
- RONDO_ICal_Feed with manual iCal generation
- RONDO_Workspace_Members for membership checks
...
<task type="auto">
  ...
  <action>
  1. In RONDO_ICal_Feed::register_query_vars(), add new query vars:
  ```php
  $vars[] = 'stadion_workspace_ical';
  $vars[] = 'stadion_workspace_id';
  ```

  2. In RONDO_ICal_Feed::register_rewrite_rules(), add workspace calendar rule:
  ```php
  add_rewrite_rule(
  ...
  3. In RONDO_ICal_Feed::maybe_handle_feed_request(), add workspace feed handling BEFORE the existing personal feed check:
  ```php
...
<task type="auto">
  ...
  <files>includes/class-ical-feed.php</files>
  <action>
  Add handle_workspace_feed() method to RONDO_ICal_Feed:
