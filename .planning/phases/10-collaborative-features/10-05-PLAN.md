...
Output: Extended PRM_Reminders with workspace activity section, mentions included in digest emails.
</objective>
...
- Call PRM_Mention_Notifications::get_queued_mentions() to get pending mentions

**Constraining decisions:**
- Phase 10-03: Mentions queued in user meta via PRM_Mention_Notifications
- Existing: PRM_Reminders handles daily digest scheduling per user
</context>

<tasks>

<task type="auto">
  <name>Task 1: Add workspace activity gathering to PRM_Reminders</name>
  ...
  <done>PRM_Reminders collects workspace activity and queued mentions</done>
</task>
...
<task type="auto">
  ...
  <action>
  1. In PRM_Email_Channel::send_notification(), update the email body generation to include new sections.
  ...
  $subject = $has_collab
      ? 'Your Caelis digest (including team activity)'
      : 'Your Caelis digest';
  ```
  ...
  <verify>
  - Check PRM_Email_Channel includes mentions and workspace_activity in email body
