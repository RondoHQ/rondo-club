...
- User preference: caelis_mention_notifications = 'digest' (default) | 'immediate' | 'never'

**Constraining decisions:**
- Phase 10-02: prm_user_mentioned action hook fires when mentions saved
- Existing: PRM_Reminders handles per-user scheduled notifications
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create PRM_Mention_Notifications class</name>
...
<task type="auto">
  ...
  4. In class-rest-api.php, update the settings endpoint to handle `mention_notifications`:
     - GET: return `get_user_meta($user_id, 'caelis_mention_notifications', true)` with default 'digest'
     - POST: `update_user_meta($user_id, 'caelis_mention_notifications', sanitize_text_field($value))`
