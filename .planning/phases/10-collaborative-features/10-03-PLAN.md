...
- Existing: RONDO_Reminders handles per-user scheduled notifications
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create RONDO_Mention_Notifications class</name>
  ...
  2. In functions.php, load and instantiate in rondo_init():
  ```php
  require_once get_template_directory() . '/includes/class-mention-notifications.php';
  new RONDO_Mention_Notifications();
  ```

  Do NOT modify RONDO_Reminders yet - that integration is Plan 05.
  </action>
  <verify>
  - Check class-mention-notifications.php exists with handle_mentions, queue_for_digest methods
  - Check functions.php loads and instantiates RONDO_Mention_Notifications
  </verify>
