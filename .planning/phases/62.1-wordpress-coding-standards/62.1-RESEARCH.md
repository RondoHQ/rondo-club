...
**Primary recommendation:** Install WPCS via Composer, create a `phpcs.xml.dist` configuration with `WordPress` standard (or `WordPress-Extra`), set theme-specific properties (text-domain: `caelis`, prefix: `prm_`), then run `phpcbf` for auto-fixing followed by manual fixes for remaining issues.
</research_summary>
...
1. **Existing code style?**
   - What we know: Caelis has 41 PHP files + 8 test files
