...
updated: 2026-02-16T11:30:00Z
resolved: 2026-02-16T11:30:00Z
---
...
root_cause: Auto-focus feature (originally commit c669a724, enhanced in commit 3bc61bb0) programmatically focuses the main element in two ways: (1) useEffect on route changes, (2) onClick handler on any click in main content area. The onClick handler was the culprit - clicking a dropdown would trigger the handler, which would refocus main, stealing focus from the dropdown and causing it to close immediately. Secondary issue: TabButton missing type="button" causes form submission.
fix:
1. Removed auto-focus feature from Layout.jsx (removed mainRef ref, useEffect focus code, onClick focus handler, tabIndex attribute, focus CSS classes)
2. Added type="button" to TabButton.jsx to prevent form submission
verification: Deployed to production (commit 319cedf6). User should test dropdowns on /feedback, /tuchtzaken, and /financien/instellingen pages.
