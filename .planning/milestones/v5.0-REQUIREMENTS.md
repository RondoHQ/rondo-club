# Requirements Archive: v5.0 Google Contacts Sync
...
# Requirements: Stadion v5.0 Google Contacts Sync

**Defined:** 2026-01-17
**Core Value:** Seamless bidirectional sync between Stadion and Google Contacts with Stadion as source of truth
...
- [x] **IMPORT-02**: Map Google Contact fields to Stadion person fields (names, emails, phones, addresses, URLs, biography)
...
- [x] **IMPORT-09**: Create team from team data and link to work_history

### Export to Google

- [x] **EXPORT-01**: Push new Stadion contacts to Google Contacts
- [x] **EXPORT-02**: Reverse field mapping (Stadion person fields -> Google Contact format)
...
- [x] **SYNC-04**: Detect changes in Stadion (compare post_modified vs last_synced)
...
- [x] **CONFLICT-02**: Default resolution strategy: Stadion wins
- [ ] **CONFLICT-03**: Configurable resolution strategies (stadion/google/newest/manual) — *Deferred to v5.1*
- [x] **CONFLICT-04**: Log all conflict resolutions for audit

### Deletion Handling

- [x] **DELETE-01**: When contact deleted in Stadion, delete corresponding contact in Google
- [x] **DELETE-02**: When contact deleted in Google, unlink in Stadion only (preserve Stadion data)
...
- [ ] **SETTINGS-06**: Conflict resolution strategy preference dropdown — *Deferred to v5.1*
...
- [x] **CLI-01**: `wp stadion google-contacts sync --user-id=ID` trigger sync for user
- [x] **CLI-02**: `wp stadion google-contacts sync --user-id=ID --full` force full resync
- [x] **CLI-03**: `wp stadion google-contacts status --user-id=ID` check sync status
- [x] **CLI-04**: `wp stadion google-contacts conflicts --user-id=ID` list unresolved conflicts
- [x] **CLI-05**: `wp stadion google-contacts unlink-all --user-id=ID` unlink all contacts
...
- **GROUPS-01**: Sync Stadion labels to Google Contact groups
- **GROUPS-02**: Sync Google Contact groups to Stadion labels
...
| Team sync to Google groups | Google doesn't have separate team entities |
...
**Shipped:** 36 of 38 v1 requirements
**Deferred:** 2 requirements (CONFLICT-03, SETTINGS-06) — configurable conflict resolution strategy UI
**Dropped:** 0
