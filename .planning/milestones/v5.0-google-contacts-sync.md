...
Enable bidirectional synchronization between Stadion and Google Contacts, allowing users to:
...
- Syncing teams/teams as separate Google Contact groups (contacts only)
...
- [ ] Map Google Contact fields to Stadion person fields:
...
- [ ] Push new Stadion contacts to Google Contacts
...
- [ ] Detect changes in Stadion (track `post_modified` vs `last_synced`)
...
- [ ] When contact deleted in Google: Option to delete in Stadion or unlink
- [ ] When contact deleted in Stadion: Option to delete in Google or unlink
- [ ] Default: Unlink only (safe default, no data loss)
- [ ] User preference for deletion propagation

#### FR7: Sync Controls
- [ ] Per-contact sync toggle (some contacts may be Stadion-only)
...
'stadion_google_contacts_sync_token'    // string: People API sync token for delta sync
'stadion_google_contacts_last_sync'     // datetime: Last full sync timestamp
'stadion_google_contacts_sync_enabled'  // bool: Master switch for contacts sync
...
'stadion_google_contacts_conflict_mode' // string: 'newest', 'google', 'stadion', 'manual'
...
├── class-google-contacts-mapper.php     # Field mapping between Google and Stadion
...
#### `Stadion\Calendar\GoogleContactsProvider`
...
#### `Stadion\Calendar\GoogleContactsSync`
...
#### `Stadion\Calendar\GoogleContactsMapper`
- Map Google Contact fields → Stadion person fields
- Map Stadion person fields → Google Contact fields
- Handle field type conversions (dates, phones, etc.)
- Normalize data formats

#### `Stadion\Calendar\GoogleContactsConflict`
- Detect conflicts (both sides modified)
- Apply resolution strategy
- Queue manual conflicts for review
- Merge non-conflicting field changes

#### `Stadion\REST\GoogleContacts`
- `GET /rondo/v1/google-contacts/status` - Connection and sync status
- `POST /rondo/v1/google-contacts/connect` - Initiate OAuth flow
- `POST /rondo/v1/google-contacts/disconnect` - Revoke and cleanup
- `POST /rondo/v1/google-contacts/sync` - Trigger manual sync
- `GET /rondo/v1/google-contacts/conflicts` - List pending conflicts
- `POST /rondo/v1/google-contacts/conflicts/{id}/resolve` - Resolve conflict
- `GET /rondo/v1/google-contacts/log` - Sync history
...
|--------|----------|-------------|
| GET | `/rondo/v1/google-contacts/status` | Get connection status, last sync, stats |
| POST | `/rondo/v1/google-contacts/connect` | Start OAuth flow (returns auth URL) |
| GET | `/rondo/v1/google-contacts/callback` | OAuth callback handler |
| DELETE | `/rondo/v1/google-contacts/disconnect` | Revoke access, cleanup |
| POST | `/rondo/v1/google-contacts/sync` | Trigger immediate sync |
| GET | `/rondo/v1/google-contacts/preferences` | Get sync preferences |
| PUT | `/rondo/v1/google-contacts/preferences` | Update sync preferences |
| GET | `/rondo/v1/google-contacts/conflicts` | List unresolved conflicts |
| POST | `/rondo/v1/google-contacts/conflicts/{id}` | Resolve a conflict |
| GET | `/rondo/v1/google-contacts/log` | Get sync history |
| POST | `/rondo/v1/people/{id}/google-sync` | Sync single contact |
| DELETE | `/rondo/v1/people/{id}/google-link` | Unlink contact from Google |
...
| Google People API | Stadion Field | Notes |
|-------------------|--------------|-------|
...
| `teams[0].name` | `work_history[].team` | Create team if needed |
| `teams[0].title` | `work_history[].job_title` | |
| `teams[0].current` | `work_history[].is_current` | |
...
```
┌─────────────┐
│   UNLINKED  │ ←── Contact exists only in Stadion, no Google link
└──────┬──────┘
       │ Export to Google
       ▼
┌─────────────┐
...
add_action('stadion_google_contacts_sync', [$this, 'process_user_sync'], 10, 1);

// Schedule based on user preference
wp_schedule_single_event(
    $next_sync_time,
    'stadion_google_contacts_sync',
    ['user_id' => $user_id]
);
```
...
# Resolve conflict
wp stadion google-contacts resolve --contact-id=123 --strategy=google
...
**Goal:** Push new Stadion contacts to Google

- [ ] Implement `createContact` API call
- [ ] Reverse field mapping (Stadion → Google format)
...
- [ ] Track Stadion changes via `post_modified`
...
- [ ] Implement resolution strategies (newest/google/stadion/manual)
...
3. **Contact groups**: Should Stadion labels map to Google Contact groups? (Proposal: Yes, as a future enhancement after core sync works)
...
5. **Photo sync direction**: Should we push Stadion photos to Google, or only pull? (Proposal: Bidirectional, newest wins)
