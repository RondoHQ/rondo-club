# Requirements Archive: v6.0 Custom Fields

**Archived:** 2026-01-21
**Status:** SHIPPED

This is the archived requirements specification for v6.0.
For current requirements, see `.planning/REQUIREMENTS.md` (created for next milestone).

---

# Requirements: Stadion v6.0 Custom Fields

**Defined:** 2026-01-18
**Core Value:** Admin-defined custom fields for People and Teams using ACF-native storage

## v1 Requirements

Requirements for v6.0 release. Each maps to roadmap phases.

### Field Management

- [x] **MGMT-01**: Admin can create custom field definition for People
- [x] **MGMT-02**: Admin can create custom field definition for Teams
- [x] **MGMT-03**: Admin can edit existing field definition (label, description, options)
- [x] **MGMT-04**: Admin can delete/deactivate field definition (data preserved)
- [x] **MGMT-05**: Admin can reorder fields via drag-and-drop
- [x] **MGMT-06**: Field key auto-generates from label (editable)
- [x] **MGMT-07**: Admin can set validation options (required, unique)
- [x] **MGMT-08**: Admin can set placeholder text for fields

### Field Types

- [x] **TYPE-01**: Text field type (single line)
- [x] **TYPE-02**: Textarea field type (multi-line)
- [x] **TYPE-03**: Number field type (integer/decimal with min/max)
- [x] **TYPE-04**: Email field type (with validation)
- [x] **TYPE-05**: URL field type (with validation)
- [x] **TYPE-06**: Date field type (date picker)
- [x] **TYPE-07**: Select field type (single choice dropdown)
- [x] **TYPE-08**: Checkbox field type (multiple choices)
- [x] **TYPE-09**: True/False field type (boolean toggle)
- [x] **TYPE-10**: Image field type (upload with preview)
- [x] **TYPE-11**: File field type (upload with download link)
- [x] **TYPE-12**: Link field type (URL + display text)
- [x] **TYPE-13**: Color picker field type
- [x] **TYPE-14**: Relationship field type (link to People/Teams)

### Display

- [x] **DISP-01**: Custom Fields section on Person detail view
- [x] **DISP-02**: Custom Fields section on Team detail view
- [x] **DISP-03**: Inline editing of custom field values
- [x] **DISP-04**: Type-appropriate rendering (date pickers, file previews, color swatches, etc.)
- [x] **DISP-05**: Custom field columns in People list view
- [x] **DISP-06**: Custom field columns in Teams list view
- [x] **DISP-07**: Type-appropriate column rendering (truncated text, icons, badges, etc.)

### Search

- [x] **SRCH-01**: Text custom field values included in global search
- [x] **SRCH-02**: Email custom field values included in global search
- [x] **SRCH-03**: URL custom field values included in global search

### Settings UI

- [x] **SETT-01**: Custom Fields subtab in Settings
- [x] **SETT-02**: Toggle between People/Teams field lists
- [x] **SETT-03**: Add Field form/modal with type selector
- [x] **SETT-04**: List view of existing fields with edit/delete actions
- [x] **SETT-05**: Drag-and-drop reordering of fields
- [x] **SETT-06**: "Show in list view" toggle per field
- [x] **SETT-07**: Column order configuration for list view fields

## v2 Requirements

Deferred to future release. Tracked but not in current roadmap.

### Advanced Field Types

- **TYPE-15**: Repeater field type (nested multi-row data)
- **TYPE-16**: Gallery field type (multiple images)
- **TYPE-17**: WYSIWYG field type (rich text editor)

### Extended Relationships

- **REL-01**: Relationship to any post type (not just People/Teams)
- **REL-02**: Bidirectional relationship tracking

### Import/Export

- **IMPEXP-01**: Export custom field definitions to JSON
- **IMPEXP-02**: Import custom field definitions from JSON

## Out of Scope

Explicitly excluded. Documented to prevent scope creep.

| Feature | Reason |
|---------|--------|
| Per-user custom fields | Complexity; global fields cover primary use case |
| Per-workspace custom fields | Complexity; global fields sufficient for v1 |
| Custom tables for field storage | Rule 0: Use WordPress/ACF native storage only |
| Conditional logic between fields | ACF supports this but adds significant UI complexity |
| Calculated/formula fields | High complexity, unclear requirements |

## Traceability

Which phases cover which requirements. Updated during roadmap creation.

| Requirement | Phase | Status |
|-------------|-------|--------|
| MGMT-01 | Phase 87 | Complete |
| MGMT-02 | Phase 87 | Complete |
| MGMT-03 | Phase 87 | Complete |
| MGMT-04 | Phase 87 | Complete |
| MGMT-05 | Phase 94 | Complete |
| MGMT-06 | Phase 87 | Complete |
| MGMT-07 | Phase 94 | Complete |
| MGMT-08 | Phase 94 | Complete |
| TYPE-01 | Phase 89 | Complete |
| TYPE-02 | Phase 89 | Complete |
| TYPE-03 | Phase 89 | Complete |
| TYPE-04 | Phase 89 | Complete |
| TYPE-05 | Phase 89 | Complete |
| TYPE-06 | Phase 89 | Complete |
| TYPE-07 | Phase 89 | Complete |
| TYPE-08 | Phase 89 | Complete |
| TYPE-09 | Phase 89 | Complete |
| TYPE-10 | Phase 90 | Complete |
| TYPE-11 | Phase 90 | Complete |
| TYPE-12 | Phase 90 | Complete |
| TYPE-13 | Phase 90 | Complete |
| TYPE-14 | Phase 90 | Complete |
| DISP-01 | Phase 91 | Complete |
| DISP-02 | Phase 91 | Complete |
| DISP-03 | Phase 91 | Complete |
| DISP-04 | Phase 91 | Complete |
| DISP-05 | Phase 92 | Complete |
| DISP-06 | Phase 92 | Complete |
| DISP-07 | Phase 92 | Complete |
| SRCH-01 | Phase 93 | Complete |
| SRCH-02 | Phase 93 | Complete |
| SRCH-03 | Phase 93 | Complete |
| SETT-01 | Phase 88 | Complete |
| SETT-02 | Phase 88 | Complete |
| SETT-03 | Phase 88 | Complete |
| SETT-04 | Phase 88 | Complete |
| SETT-05 | Phase 94 | Complete |
| SETT-06 | Phase 92 | Complete |
| SETT-07 | Phase 92 | Complete |

**Coverage:**
- v1 requirements: 29 total
- Shipped: 29
- Dropped: 0

---

## Milestone Summary

**Shipped:** 29 of 29 v1 requirements
**Adjusted:** None
**Dropped:** None

---
*Archived: 2026-01-21 as part of v6.0 milestone completion*
