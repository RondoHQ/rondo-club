...
Integrate Gmail with Caelis to automatically track email correspondence with contacts, enabling users to:
...
3. **Reduced Context Switching**: Compose emails without leaving Caelis
4. **Privacy First**: Email content stays in Gmail; Caelis stores metadata and references only
...
#### FR6: Reply from Caelis
- [ ] "Reply" button on email threads shown in Caelis
...
- [ ] Suggest "Add to Caelis" for frequent unmatched senders
...
- Email body content NOT stored in Caelis database
...
'caelis_gmail_enabled'           // bool: Master switch for Gmail integration
'caelis_gmail_auto_log'          // bool: Auto-log emails as activities
'caelis_gmail_auto_log_direction'// string: 'sent', 'received', 'both'
'caelis_gmail_check_frequency'   // string: '15min', '1hour', '6hours', 'daily'
'caelis_gmail_last_history_id'   // string: Gmail history ID for incremental sync
'caelis_gmail_signature'         // string: Email signature for compose
```
...
```
includes/
├── class-gmail-provider.php      # Gmail API client, send/receive/search
├── class-gmail-sync.php          # Background sync, auto-logging orchestration
├── class-gmail-matcher.php       # Match emails to contacts by address
└── class-rest-gmail.php          # REST API endpoints for UI
```

### Class Responsibilities

#### `Caelis\Google\GmailProvider`
...
#### `Caelis\Google\GmailMatcher`
- Match email addresses to Caelis contacts
- Build reverse index: email → person_id
- Handle case normalization
- Integrate with Google Contacts sync data

#### `Caelis\REST\Gmail`
- `GET /prm/v1/gmail/status` - Connection status
- `GET /prm/v1/gmail/threads` - List threads for a contact
- `GET /prm/v1/gmail/threads/{id}` - Get thread details
- `POST /prm/v1/gmail/send` - Send email
- `POST /prm/v1/gmail/threads/{id}/log` - Log thread as activity
- `GET /prm/v1/gmail/search` - Search emails
- `GET /prm/v1/gmail/preferences` - Get preferences
- `PUT /prm/v1/gmail/preferences` - Update preferences
...
├── pages/People/
│   └── PersonDetail/
│       └── EmailsSection.jsx       # Email threads list for contact
├── components/
│   ├── Gmail/
│   │   ├── EmailThreadList.jsx     # List of thread cards
│   │   ├── EmailThreadCard.jsx     # Single thread preview
│   │   ├── EmailComposeModal.jsx   # Compose/reply modal
│   │   ├── EmailLogModal.jsx       # Log email as activity dialog
│   │   └── EmailSearchModal.jsx    # Search emails with contact
│   └── Settings/
│       └── GmailPreferences.jsx    # Gmail-specific settings
```
...
|--------|----------|-------------|
| GET | `/prm/v1/gmail/status` | Gmail connection status, quota usage |
| GET | `/prm/v1/gmail/threads` | List threads (params: emails[], maxResults, pageToken) |
| GET | `/prm/v1/gmail/threads/{id}` | Get full thread with messages |
| POST | `/prm/v1/gmail/send` | Send new email or reply |
| POST | `/prm/v1/gmail/threads/{id}/log` | Log thread as activity |
| GET | `/prm/v1/gmail/search` | Search (params: query, emails[], maxResults) |
| GET | `/prm/v1/gmail/preferences` | Get user's Gmail preferences |
| PUT | `/prm/v1/gmail/preferences` | Update preferences |
| GET | `/prm/v1/people/{id}/emails` | Shortcut: threads for a person |
| POST | `/prm/v1/people/{id}/emails/send` | Shortcut: send to a person |
...
│     sync        │
└────────┬────────┘
         │ For each user with Gmail enabled
         ▼
┌─────────────────┐
│ GmailSync       │
│ history.list()  │
...
Emails logged as activities use the existing `prm_activity` comment type:
...
│  │  • Combined scopes requested incrementally           │   │
│  │  • One refresh token for all services                │   │
...
│  ├── Thread IDs (references only)                           │
│  ├── Cached metadata:                                       │
│  │   ├── Subject lines                                      │
│  │   ├── Snippets (first ~100 chars)                        │
│  │   ├── Dates                                              │
│  │   └── Participant email addresses                        │
│  ├── Activity log entries (type: email)                     │
│  └── User preferences                                       │
│                                                             │
│  NOT stored in Caelis:                                      │
│  ├── Full email body                                        │
│  ├── Attachments                                            │
│  └── Email headers (except minimal threading info)          │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
wp caelis gmail status --user-id=1

# Trigger email sync for user
wp caelis gmail sync --user-id=1

# List recent threads for a contact
wp caelis gmail threads --person-id=123 --limit=10

# Clear cached email data
wp caelis gmail clear-cache --user-id=1
```
...
- [ ] Create `GmailMatcher` for email → contact matching
- [ ] REST endpoint: `GET /prm/v1/people/{id}/emails`
...
**Goal:** Send emails from Caelis

- [ ] `EmailComposeModal` component
- [ ] REST endpoint: `POST /prm/v1/gmail/send`
...
- [ ] REST endpoint: `GET /prm/v1/gmail/search`
...
```
┌─────────────────────────────────────────────────────────────┐
│  Emails with John Smith                    [Compose] [🔍]   │
├─────────────────────────────────────────────────────────────┤
│  ┌─────────────────────────────────────────────────────┐   │
│  │ 📧 Re: Project proposal                              │   │
│  │    Hey John, I've reviewed the proposal and...       │   │
│  │    Today, 2:30 PM                    [Log] [Reply]   │   │
│  └─────────────────────────────────────────────────────┘   │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ 📧 Lunch next week?                                  │   │
│  │    Would you be free for lunch on Tuesday? I...      │   │
│  │    Yesterday                         [Log] [Reply]   │   │
│  └─────────────────────────────────────────────────────┘   │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ ✉️ Contract review (3 messages)                      │   │
│  │    Thanks for sending this over. A few questions...  │   │
│  │    Jan 15                            [Log] [Reply]   │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
│                    [Load more threads...]                   │
...
│  │                                                          │
│  ☎ Phone • Jan 15                                           │
│  │  Quick call about timeline                               │
│  │                                                          │
│  📅 Meeting • Jan 14                                        │
│  │  Weekly sync (from Calendar)                             │
│  │  [View in Calendar →]                                    │
└─────────────────────────────────────────────────────────────┘
```
...
- [ ] Zero email content stored in Caelis database (privacy)
...
2. **Attachment handling**: Should attachments be uploadable from Caelis, or only reference existing Gmail attachments? (Proposal: Support upload for new emails)
...
| `gmail.send` | Send emails from Caelis | Send as user (appears in Sent folder) |
