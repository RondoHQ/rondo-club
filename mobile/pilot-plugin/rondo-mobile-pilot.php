...
class Plugin extends \Rondo\Mobile\Gateway {
	protected const ORIGIN        = 'https://rondo.svawc.nl';
	protected const FLAG          = 'RONDO_MOBILE_PILOT';
	protected const CONFIG_OPTION = 'rondo_mobile_pilot';
	...
	public function no_store( $response, $server, $request ) {
		if ( str_starts_with( $request->get_route(), '/' . static::NS . '/' ) ) {
	...
	public static function settings(): array {
		$value = get_option( static::CONFIG_OPTION, [] );
		return is_array( $value ) ? $value : [];
	}

	public static function enabled(): bool {
		$config = static::settings();
		return defined( static::FLAG ) && constant( static::FLAG ) === true
			&& untrailingslashit( home_url() ) === static::ORIGIN
	...
	protected static function session_policy(): string {
		return hash( 'sha256', wp_json_encode( static::settings() ) );
	}
	...
	protected static function user_allowed( \WP_User $user ): bool {
		...
		if ( ! static::enabled() || ! user_can( $user, 'read' ) || ! $person || $person->post_type !== 'person' || $person->post_status !== 'publish' ) {
			return false;
		}
		// Pin both identities: relinking an account never silently grants another household access.
		foreach ( static::settings()['testers'] as $tester ) {
	...
	public function route_permission( \WP_REST_Request $request ) {
		...
		if ( ! in_array( $request->get_route(), [ '/' . static::NS . '/token', '/' . static::NS . '/wallet' ], true ) ) {
			return true;
		}
		...
		for ( $slot = 0; $slot < 60; ++$slot ) {
			...
			if ( add_option( $key, true, '', false ) ) {
				wp_schedule_single_event( ( $window + 2 ) * MINUTE_IN_SECONDS, static::CLEANUP, [ $key ] );
				return true;
			}
		}
		return static::error( 'rate_limited', 429 );
	}
}
