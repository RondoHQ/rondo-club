...
await writeFile(join(target, 'ios/App/App/Pilot.entitlements'), '<?xml version="1.0" encoding="UTF-8"?>\n<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n<plist version="1.0"><dict><key>com.apple.developer.associated-domains</key><array><string>applinks:rondo.svawc.nl</string><string>webcredentials:rondo.svawc.nl</string><string>applinks:demo.rondo.club</string><string>webcredentials:demo.rondo.club</string></array></dict></plist>\n');
...
await edit('android/app/src/main/AndroidManifest.xml', text => text.replace('<intent-filter>\n                <action android:name="android.intent.action.VIEW"', '<intent-filter android:autoVerify="true">\n                <action android:name="android.intent.action.VIEW"').replace('<data android:scheme="club.rondo.spike" android:host="oauth" android:path="/callback" />', '<data android:scheme="https" android:host="rondo.svawc.nl" android:path="/rondo-app/callback" /><data android:scheme="https" android:host="demo.rondo.club" android:path="/rondo-app/callback" />'));
...
await writeFile(join(root, 'READINESS.json'), JSON.stringify({ appId: 'club.rondo.pilot', clubs: ['https://rondo.svawc.nl', 'https://demo.rondo.club'], scope: 'rondo:pilot:read', appleAssociationPrepared: Boolean(team), androidAssociationPrepared: Boolean(fingerprint), signingConfigured: false, uploaded: false, deployed: false }, null, 2));
