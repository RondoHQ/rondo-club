...
function PassDetail() {
  ...
  return <section><Link className="back-link" to="/passen">‹ Alle passen</Link><QueryState query={household}>{(people) => {
    ...
    return <>{options.length > 1 && <button className="secondary" onClick={() => setParams({})}>Andere pas kiezen</button>}<PassContent personId={person.id} role={role || options[0]?.key || ''} label={options.find((option) => option.key === role)?.label || person.membership_pass.label} wallets={person.membership_pass.wallets || {}} /></>;
  }}</QueryState></section>;
}
...
function More() {
  const { logout, profile, session } = useContext(MemberContext);
  return <section><h1>Meer</h1><p>{profile.name}</p><Link className="action-card" to="/gegevens"><strong>Mijn gegevens</strong><span>›</span></Link><Link className="action-card" to="/clubs"><strong>Mijn clubs</strong><span>›</span></Link><div className="panel"><h2>Over deze proef</h2><p>{PILOT ? session.club.id === 'demo' ? 'Je bekijkt fictieve gezinsgegevens, diensten en ledenpassen. Wallet is bij deze democlub nog niet beschikbaar. Gegevens wijzigen en diensten boeken zijn in deze pilot uitgeschakeld.' : 'Je bekijkt de echte AWC-gegevens van jou en je gezin en kunt je beschikbare passen toevoegen aan Wallet. Gegevens wijzigen en diensten boeken zijn in deze pilot uitgeschakeld.' : 'Je bekijkt gegevens van je club en kunt je aanmelden en afmelden voor vrijwilligersdiensten. Je kunt je eigen contactgegevens en het gezinsadres wijzigen. Je kunt je pas ook toevoegen aan Wallet als je club dit aanbiedt.'}</p><p>Je blijft maximaal {PILOT ? 7 : 30} dagen aangemeld op dit toestel. Via Uitloggen verwijder je je opgeslagen aanmelding.</p></div><button className="secondary" onClick={logout}>Uitloggen</button></section>;
}
...
export default function MemberApp({ session, profile, logout, onExpired, read, changeShift, changeProfile, requestWallet }) {
